...
- The [`stop_details`](../build-with-claude/build-with-claude-handling-stop-reasons.md#refusal-categories) field on refusal responses is now publicly documented; it returns a `category` (`cyber`, `bio`, or `null`) and a human-readable `explanation`, so your application can route different classes of refusal to the right next step. No beta header is required.
