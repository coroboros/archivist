...
<AccordionGroup>
  ...
  <Accordion title="Is turning the Compliance API off in Claude Console recorded anywhere?">
    Yes. When the Compliance API is turned off (or back on) in Claude Console, the change is recorded as an organization settings-updated activity in the [Activity Feed](./manage-claude-compliance-activity-feed.md), so your audit trail shows who changed the setting and when. This activity is an exception to the recording stop: the disable is recorded even though no other activity is recorded while the Compliance API is off.
  </Accordion>
