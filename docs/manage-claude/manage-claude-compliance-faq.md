...
<section title="Why doesn't my parent organization appear in Claude Console when creating an Admin API key?">
  ...
  - **For full Compliance API access ([Activity Feed](./manage-claude-compliance-activity-feed.md) plus chats, files, projects, users, and organization metadata),** the primary owner of the parent organization creates a [Compliance Access Key](./manage-claude-compliance-api-access.md#create-a-compliance-access-key) in claude.ai.
...
<section title="Why does my Admin API key return 403 on chat or file endpoints?">
  ...
  To access content endpoints, the primary owner of your parent organization must [create a Compliance Access Key](./manage-claude-compliance-api-access.md#create-a-compliance-access-key) with `read:compliance_user_data` (and `delete:compliance_user_data` for deletes), or `read:compliance_org_data` for organization, role, and group endpoints. See [Handle Compliance API errors](./manage-claude-compliance-errors.md#403-forbidden) for the full per-endpoint catalog.
...
<section title="How do I get a sandbox to test the Compliance API?">
  ...
  1. **Provision the Claude Enterprise organization.** Contact your Anthropic representative to set up a Claude Enterprise sandbox organization, or to [request Compliance API access](./manage-claude-compliance-api-access.md#request-compliance-api-access) on an existing Claude Enterprise organization.
