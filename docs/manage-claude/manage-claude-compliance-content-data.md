---
title: "WARNING: This operation PERMANENTLY deletes the chat, all of its messages,"
...
<Note>
  The endpoints on this page are available only to Claude Enterprise organizations. They retrieve claude.ai chats, files, projects, and Cowork and Claude Code session transcripts; they delete chats, files, and projects. See [Set up the Compliance API](./manage-claude-compliance-api-access.md).
</Note>
...
<Check>
  ...
  **Prerequisite:** None for listing chats or sessions organization-wide. To filter the chat or remote session lists to specific users, you need user IDs from [List organization users](./manage-claude-compliance-org-data.md#list-organization-users); the local session list has no user filter. The other endpoints on this page take resource IDs directly.
</Check>
...
Use [List chats](../api/api-compliance-apps-chats-list.md) to page through chat metadata, then [Get chat messages](../api/api-compliance-apps-chats-messages-list.md) to fetch the full message content of one chat.
...
The messages endpoint returns the chat's metadata plus a `chat_messages` array sorted by `created_at`. When `limit` is omitted, the full message set is returned in one response; pass `limit`, `after_id`, or `before_id` to page through very long chats. The endpoint also accepts `created_at.*` and `updated_at.*` range bounds (`gt`, `gte`, `lt`, `lte`) and an `order` parameter (`asc` or `desc`). See [Get chat messages](../api/api-compliance-apps-chats-messages-list.md) for the full parameter list. For user messages, `created_at` is when the message was sent; for assistant messages, it is when Claude finished generating the message. Each message carries its text content and, when present, any uploaded files (typically on user messages), any tool-generated files, and any artifacts the assistant produced or updated (typically on assistant messages):
...
| ------------------------------ | --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `claude_file_*` ID             | The file's binary content               | [Download file content](../api/api-compliance-apps-chats-files-download.md)                      |
| `claude_file_*` ID             | The file's metadata only                | [Get file metadata](../api/api-compliance-apps-chats-files-retrieve.md)                          |
| `claude_gen_file_*` ID         | A tool-generated file's binary content  | [Download a Claude-generated file](../api/api-compliance-apps-chats-generated_files-download.md) |
| `claude_gen_file_*` ID         | A tool-generated file's metadata only   | [Get generated-file metadata](../api/api-compliance-apps-chats-generated_files-retrieve.md)      |
| `claude_artifact_version_*` ID | One artifact version's text             | [Download artifact content](../api/api-compliance-apps-artifacts-download.md)                    |
| `claude_artifact_version_*` ID | The artifact version's metadata only    | [Get artifact metadata](../api/api-compliance-apps-artifacts-retrieve.md)                        |
| `claude_proj_doc_*` ID         | A project document's plain-text content | [Get project document content](../api/api-compliance-apps-projects-documents-retrieve.md)        |
| `claude_proj_doc_*` ID         | A project document's metadata only      | [Get project document metadata](../api/api-compliance-apps-projects-documents-metadata.md)       |
...
* [List projects](../api/api-compliance-apps-projects-list.md)
* [Get project details](../api/api-compliance-apps-projects-retrieve.md)
* [List project attachments](../api/api-compliance-apps-projects-attachments-list.md)
* [Get project document content](../api/api-compliance-apps-projects-documents-retrieve.md)
...
The following table summarizes how local sessions differ from the [remote sessions](./manage-claude-compliance-content-data.md#retrieve-remote-sessions) covered later on this page.
...
The list is built from session activity metadata, so it can include sessions whose transcript content was not captured, for example sessions that ran before capture began for your organization (as far back as your retention period allows); every message in such a session's transcript carries `provenance.type` of `content_unavailable` with `reason` of `not_captured` (see [Retrieve a local session transcript](./manage-claude-compliance-content-data.md#retrieve-a-local-session-transcript)).
...
Project instruction files such as `CLAUDE.md` appear as ordinary user-role content. Skill content appears when the client sends it as message content and is not distinguished from other user text. For a coverage summary and a comparison with OpenTelemetry logging for Cowork and Claude Code, see the [Compliance API FAQ](./manage-claude-compliance-faq.md#data-coverage-and-retention).
...
The response embeds a `session` envelope alongside the paginated `data` array. The first record in this example is the marker that stands in for the request's system prompt; its `provenance` is described later in this section. On this endpoint `user.email_address` is always `null`: the messages endpoint does not resolve email addresses, so a `null` here does not mean the user's account was deleted. To attribute a session to an email address, join `user.id` against the [list endpoint](./manage-claude-compliance-content-data.md#retrieve-local-sessions) or the retrieve endpoint (`GET /v1/compliance/apps/sessions/local/{session_id}`).
...
The messages endpoint returns the session's transcript: user prompts, assistant responses, and tool calls and results. Thinking blocks and images are not included. For a coverage summary and a comparison with Cowork's OpenTelemetry logging, see the [Compliance API FAQ](./manage-claude-compliance-faq.md#data-coverage-and-retention).
...
The messages endpoint returns [404 Not Found](./manage-claude-compliance-errors.md#404-not-found) for `pending` sessions, deleted sessions, and sessions in organizations your key cannot read.
...
* [Delete chat](../api/api-compliance-apps-chats-delete.md): also removes the chat's messages and any files attached to those messages.
* [Delete file](../api/api-compliance-apps-chats-files-delete.md): handles both chat files and project files.
* [Delete project document](../api/api-compliance-apps-projects-documents-delete.md): removes a single project document by ID.
* [Delete project](../api/api-compliance-apps-projects-delete.md): see [Detach chats before deleting a project](./manage-claude-compliance-content-data.md#detach-chats-before-deleting-a-project).
...
<CardGroup cols={2}>
  <Card title="API reference" href="../api/api-compliance-apps.md">
    The full request and response schema for every chat, file, project, and artifact endpoint.
  </Card>

  <Card title="List organizations, users, roles, groups, and settings" href="./manage-claude-compliance-org-data.md">
