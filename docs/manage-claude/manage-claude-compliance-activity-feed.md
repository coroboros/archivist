...
Organizations and files do not paginate: [List organizations](./manage-claude-compliance-org-data.md#list-organizations) returns all results in one response, and files are retrieved individually by ID.
...
| _additional fields_ | varies | Type-specific fields, for example `claude_chat_id` on chat events or `filename` on file events. See [Query compliance activities](../api/api-compliance-activities-list.md) in the API reference for the per-type field list. |
