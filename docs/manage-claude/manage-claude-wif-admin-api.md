...
<Steps>
  ...
  <Step title="Exchange the workload's identity token">
    At runtime, the workload exchanges the JWT from its identity provider for a short-lived `org:admin` bearer token using the same [token exchange](./manage-claude-workload-identity-federation.md#authenticate-from-your-workload) as any other federated workload.
  </Step>
