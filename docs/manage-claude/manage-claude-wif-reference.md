...
| `workspace:inference` | The inference endpoints in the rule's workspace: [Messages](../api/api-messages.md) (including streaming and token counting), [Models](https://platform.claude.com/docs/en/api/models-list.md), and the [OpenAI-compatible chat endpoint](https://platform.claude.com/docs/en/cli-sdks-libraries/libraries/openai-sdk.md). Use this for workloads that only need to call Claude and never need to manage Files, Skills, or other resources. |
| `org:manage_tunnels` | The [MCP tunnels API](../agents-and-tools/agents-and-tools-mcp-tunnels-reference.md#tunnels-api): list and get tunnels, register and archive CA certificates, reveal and rotate the tunnel token, and archive tunnels. The Console's create-tunnel modal locks this scope when you create a rule from it. |
