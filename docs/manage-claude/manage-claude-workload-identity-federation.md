...
- **Authorization:** The OAuth `scope` granted on the minted token. At launch this is always `workspace:developer`, which grants the same access as an API key issued for that workspace (see [OAuth scopes](./manage-claude-wif-reference.md#oauth-scopes)). The rule also sets `token_lifetime_seconds` (60 to 86400, default 3600).
...
<Steps>
  ...
  <Step title="Create a federation rule">
    ...
    | Authorization | OAuth scope (`workspace:developer` at launch; see [OAuth scopes](./manage-claude-wif-reference.md#oauth-scopes)) and token lifetime in seconds. |
...
<Warning>
  ...
  runs (container env, CI secrets, shell profiles). The CLI's [`ant auth status`](https://platform.claude.com/docs/en/api/sdks/cli.md#check-authentication-status)
