...
* **Authorization:** The OAuth `scope` granted on the minted token. The default is `workspace:developer`, which grants the same access as a workspace API key. Some products lock the scope when you create a rule from their flow; for example, the [MCP tunnels](../agents-and-tools/agents-and-tools-mcp-tunnels-overview.md) create-tunnel modal creates rules scoped to `workspace:manage_tunnels`. See [OAuth scopes](./manage-claude-wif-reference.md#oauth-scopes). The rule also sets `token_lifetime_seconds` (60 to 86400, default 3600).
