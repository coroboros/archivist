...
**Fix:** Compliance Access Key scopes are immutable after creation. [Create a new Compliance Access Key](./manage-claude-compliance-api-access.md#create-a-compliance-access-key) with `read:compliance_org_data` selected, update your integration to use it, then delete the old key. A key that already carries `read:compliance_org_data` is unaffected by the retirement.
...
**Fix:** [Create a new Compliance Access Key](./manage-claude-compliance-api-access.md#create-a-compliance-access-key) with `delete:compliance_user_data` selected. The delete scope is separate from `read:compliance_user_data` so that read-only audit keys cannot delete content.
