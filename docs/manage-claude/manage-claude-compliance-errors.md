...
Requests to the Compliance API are limited to **600 requests per minute per [parent organization](./manage-claude-compliance-api.md#how-the-compliance-api-works)**. The limit is a single budget shared across every key under the parent (Compliance Access Keys and the Admin API keys of all linked organizations) and across every `/v1/compliance/*` endpoint. Contact your Anthropic representative if your integration needs a higher limit.
