...
An `org:admin` token grants access to the whole organization, regardless of the workspace the underlying profile or federation rule is bound to. To obtain one, see the prerequisites in [Manage WIF with the Admin API](./manage-claude-wif-admin-api.md#prerequisites).
