...
<Note>
  The tool search tool is not compatible with [tool use examples](./agents-and-tools-tool-use-define-tools.md#providing-tool-use-examples). If you need to provide examples of tool usage, use standard tool calling without tool search.
</Note>
...
You can include the tool search tool in the [Messages Batches API](../build-with-claude/build-with-claude-batch-processing.md). Tool search operations through the Messages Batches API are priced the same as those in regular Messages API requests.
...
<CardGroup cols={2}>
  <Card title="Tool reference" icon="list" href="./agents-and-tools-tool-use-tool-reference.md">
    Full tool catalog with model compatibility and parameters.
  </Card>

  <Card title="MCP connector" icon="plug" href="./agents-and-tools-mcp-connector.md">
    Configure MCP toolsets with deferred loading.
  </Card>

  <Card title="Prompt caching" icon="bolt" href="./agents-and-tools-tool-use-tool-use-with-prompt-caching.md">
