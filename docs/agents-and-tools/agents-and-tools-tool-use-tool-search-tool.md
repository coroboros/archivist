...
<Note>
  [Tool use examples](./agents-and-tools-tool-use-define-tools.md#providing-tool-use-examples) work with tool search: when Claude discovers a deferred tool, the API expands its `input_examples` along with its definition.
</Note>
...
You can include the tool search tool in the [Messages Batches API](../build-with-claude/build-with-claude-batch-processing.md).
...
<CardGroup cols={2}>
  ...
  <Card title="Tool use with prompt caching" icon="bolt" href="./agents-and-tools-tool-use-tool-use-with-prompt-caching.md">
