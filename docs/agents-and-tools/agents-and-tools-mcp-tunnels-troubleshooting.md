...
| `curl https://:8080` fails with `wrong version number` | Expected; the listener is plaintext WebSocket. TLS happens inside the WS stream. | Verify through a [Managed Agent or the Messages API](./agents-and-tools-mcp-tunnels-overview.md#use-the-tunneled-mcp-servers) instead. |
