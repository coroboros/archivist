...
Once your stack is running and has one or more MCP servers configured, attach a tunneled MCP server to a Managed Agent session. To call the same servers from the Messages API instead, see [Use the tunneled MCP servers](./agents-and-tools-mcp-tunnels-overview.md#use-the-tunneled-mcp-servers).
