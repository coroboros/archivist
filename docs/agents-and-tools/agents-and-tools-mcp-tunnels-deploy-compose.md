...
This guide deploys the [tunnel stack](./agents-and-tools-mcp-tunnels-concepts.md#components) as hardened containers on a single host. The same configuration can be replicated across multiple hosts for availability.
...
- **A way for the host to authenticate to the Tunnels API.**
  - **Programmatic access (recommended).** Turn on **Set up programmatic access** when creating the tunnel so the [setup component](./agents-and-tools-mcp-tunnels-concepts.md#components) can authenticate through Workload Identity Federation. Record the federation rule ID (`fdrl_...`) and your organization ID.
...
<Steps>
  ...
  <Step title="Provision the tunnel">
    Set the identifiers from the [Console create-tunnel flow](./agents-and-tools-mcp-tunnels-console.md#create-a-tunnel):
    ...
    Set `ANTHROPIC_IDENTITY_TOKEN` to an OIDC JWT from this host's identity provider. Follow the [WIF guide for your provider](../manage-claude/manage-claude-workload-identity-federation.md#identity-providers) to register the issuer, set the rule's subject, and mint the token; the rule's audience must match the audience you request when minting.
    ...
    See [Setup component authentication failures](./agents-and-tools-mcp-tunnels-troubleshooting.md#setup-component-authentication-failures) if it errors.
  ...
  <Step title="Write the proxy config">
    `tunnel_domain` is **required**: the [proxy](./agents-and-tools-mcp-tunnels-concepts.md#components) uses it to strip the domain suffix from incoming hostnames before looking up the subdomain in `routes`. `routes` is a flat map from subdomain to upstream URL, not a list.
...
<Steps>
  ...
  <Step title="Scaffold and generate certificates">
    ...
    The proxy listens on `:8080` over plain WebSocket; the [inner TLS](./agents-and-tools-mcp-tunnels-concepts.md#components) handshake happens **inside** that WebSocket stream using these certificates. Anthropic verifies the inner handshake against the CA you register in the Console. The server certificate's Subject Alternative Name (SAN) must include `*.<tunnel-domain>` per the [certificate requirements](./agents-and-tools-mcp-tunnels-reference.md#certificate-requirements).
  ...
  <Step title="Register the CA certificate in the Console">
    ...
    The tunnel's status flips to **Active** once a certificate is registered. See [Add a CA certificate](./agents-and-tools-mcp-tunnels-console.md#add-a-ca-certificate).
  </Step>
  ...
  <Step title="Write docker-compose.yaml">
    The `network_mode: "service:mcp-proxy"` setting places [cloudflared](./agents-and-tools-mcp-tunnels-concepts.md#components) in the proxy's network namespace so that `localhost:8080` inside the cloudflared container reaches the proxy. The `--url http://localhost:8080` flag gives cloudflared its forwarding target; without that flag, cloudflared has no route for incoming requests and returns a 503.
