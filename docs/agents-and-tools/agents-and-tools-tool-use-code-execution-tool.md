...
featureMetadata:
  ...
  supportedPlatforms:
    ...
    Microsoft Foundry:
      availability: ga
      note: On [Microsoft Foundry](../build-with-claude/build-with-claude-claude-in-microsoft-foundry.md), code execution requires a [Hosted on Anthropic deployment](../build-with-claude/build-with-claude-claude-in-microsoft-foundry.md#additional-features-not-supported-when-hosted-on-azure).
  details:
...
1. **Upload your file** using the [Files API](../build-with-claude/build-with-claude-files.md)
...
When Claude saves files to its output directory during code execution (see [How generated files are captured](./agents-and-tools-tool-use-code-execution-tool.md#how-generated-files-are-captured)), each file's ID appears in the code execution tool result, and you can download it with the [Files API](../build-with-claude/build-with-claude-files.md):
...
A file Claude wrote elsewhere is still in the container, so you can [reuse the container](./agents-and-tools-tool-use-code-execution-tool.md#container-reuse) and ask Claude to copy it into `$OUTPUT_DIR`.
...
* `content`: A list with an entry for each file the command left in `$OUTPUT_DIR` (see [How generated files are captured](./agents-and-tools-tool-use-code-execution-tool.md#how-generated-files-are-captured)). Each entry carries the `file_id` to [retrieve the file](./agents-and-tools-tool-use-code-execution-tool.md#retrieve-generated-files) with the Files API
...
* **Workspace scoping:** Like the [Files API](../build-with-claude/build-with-claude-files.md), containers are scoped to the request's workspace
...
With [streaming](../build-with-claude/build-with-claude-streaming.md) enabled (`"stream": true`), you'll receive code execution events as they occur. The sub-tool input streams as `input_json_delta` events, and each result block arrives whole in a single `content_block_start` event:
...
You can include the code execution tool in the [Messages Batches API](../build-with-claude/build-with-claude-batch-processing.md). Code execution tool calls through the Messages Batches API are priced the same as those in regular Messages API requests.
...
<CardGroup cols={2}>
  <Card title="Advisor tool" icon="compass" href="./agents-and-tools-tool-use-advisor-tool.md">
    Pair a faster executor model with a higher-intelligence advisor model that provides strategic guidance mid-generation.
  </Card>

  <Card title="Programmatic tool calling" icon="code" href="./agents-and-tools-tool-use-programmatic-tool-calling.md">
    Call your own tools from code that runs inside the code execution container.
  </Card>

  <Card title="Files API" icon="file" href="../build-with-claude/build-with-claude-files.md">
    Upload files for analysis and download the files that code execution creates.
  </Card>

  <Card title="Using Agent Skills with the API" icon="book" href="../build-with-claude/build-with-claude-skills-guide.md">
