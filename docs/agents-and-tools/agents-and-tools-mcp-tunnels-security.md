...
- **Limit network reach.** The proxy and cloudflared should only be able to reach the destinations listed in the [network requirements](./agents-and-tools-mcp-tunnels-overview.md#network-requirements). Use NetworkPolicy (Kubernetes) or host firewall rules (Compose).
