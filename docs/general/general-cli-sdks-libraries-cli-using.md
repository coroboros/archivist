...
This page covers the `ant` CLI's input and output mechanics that apply across every endpoint. To install and authenticate, see the [Quickstart](./general-cli-sdks-libraries-cli-quickstart.md). To chain commands and version-control resources, see [CLI scripting and automation](./general-cli-sdks-libraries-cli-scripting.md).
...
| `--workspace-id`                      | Optional. Workspace ID (`wrkspc_...`) to send as the `anthropic-workspace-id` header, for API keys with access to multiple workspaces (equivalent to setting `ANTHROPIC_WORKSPACE_ID`). See [Select a workspace](../manage-claude/manage-claude-authentication.md#select-a-workspace). [Admin API](../manage-claude/manage-claude-admin-api.md) commands take their own `--workspace-id`, which names the workspace they manage instead. |
...
Every API resource the CLI exposes is documented in the [API reference](../api/api-cli-messages-create.md). For a local listing, run `ant --help`, and append `--help` to any subcommand for its flags and parameters.
...
<CardGroup cols={3}>
  ...
  <Card title="API reference" icon="book" href="../api/api-cli-messages-create.md">
    Endpoint-specific parameters, request fields, and response schemas
  </Card>

  <Card title="CLI authentication options" icon="lock" href="./general-cli-sdks-libraries-cli-authentication.md">
