...
To install and authenticate the CLI, see the [CLI quickstart](./general-cli-sdks-libraries-cli-quickstart.md). `ant apply` requires CLI version 1.30.0 or later.
...
The frontmatter holds the agent's configuration (the fields from [Define your agent](../managed-agents/managed-agents-agent-setup.md)) and the body is its system prompt. `ant apply` [infers](./general-cli-sdks-libraries-cli-apply.md#kind-inference) that the file is an agent from its path, here the `agents/` directory.
...
* An [environment](../managed-agents/managed-agents-environments.md) is a YAML file in `environments/`.
* A [memory store](../managed-agents/managed-agents-memory.md) is a YAML file in `memory_stores/`.
* A [deployment](../managed-agents/managed-agents-scheduled-deployments.md) is a Markdown file in `deployments/`: the frontmatter is the request body and the prose becomes the message that starts each session.
* A [skill](../managed-agents/managed-agents-skills.md) is a directory with a `SKILL.md` at its root, conventionally under `skills/`, uploaded as one bundle.
...
* Authenticate with [Workload Identity Federation](../manage-claude/manage-claude-workload-identity-federation.md) rather than a stored API key, as an identity that reaches the organization and workspace recorded in `claude-lock.json`. `ant apply` refuses credentials that resolve to any other organization or workspace.
...
<CardGroup cols={3}>
  <Card title="Start a session" icon="terminal" href="../managed-agents/managed-agents-sessions.md">
    Run the agents you applied, from the CLI or an SDK
  </Card>

  <Card title="Scheduled deployments" icon="clock" href="../managed-agents/managed-agents-scheduled-deployments.md">
    Deployment fields, run history, and pausing
  </Card>

  <Card title="CLI scripting and automation" icon="code" href="./general-cli-sdks-libraries-cli-scripting.md">
