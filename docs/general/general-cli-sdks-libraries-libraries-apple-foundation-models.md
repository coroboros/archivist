...
Tokens are scoped to your workspace, expire after one hour, and authorize only [Messages API](../api/api-messages-create.md) calls. They carry no end-user identity: App Attest identifies your app, not the person using it, so handle any per-user logic in your app.
