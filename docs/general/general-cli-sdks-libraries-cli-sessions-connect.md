...
source: "https://platform.claude.com/docs/en/cli-sdks-libraries/cli/sessions-connect"
category: "general"
generated: true
---
---
...
`ant beta:sessions connect` attaches your terminal to an existing Claude Managed Agents [session](../managed-agents/managed-agents-sessions.md). It loads the session's transcript and follows it live as the agent works. You can also step in: send a message, interrupt the agent, or allow or deny a tool call that is waiting for approval. With `--web`, it opens the session in the Claude Console's session viewer in your browser instead.

The command requires version 1.32.0 or later of the CLI. To install or update the CLI and authenticate, see the [CLI quickstart](./general-cli-sdks-libraries-cli-quickstart.md).
...
Without `--web`, the command needs an interactive terminal. In scripts, use `ant beta:sessions:events stream` and `ant beta:sessions:events send` instead. See [CLI scripting and automation](./general-cli-sdks-libraries-cli-scripting.md).

Press Ctrl+C to detach. The session keeps running, and connecting again loads its full history.

## Follow and steer the session

The terminal view shows the conversation live: messages and tool calls, with each call's duration and outcome. A status bar shows whether the session is running, idle, or waiting for your approval. In [multiagent](../managed-agents/managed-agents-multiagent-orchestration.md) sessions, the view follows the session's primary thread, which includes the messages the coordinator exchanges with the agents it delegates to.
