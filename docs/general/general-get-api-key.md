...
<Steps>
  ...
  <Step title="Create a key">
    Click **Create key**, name the key, choose an [expiration](../manage-claude/manage-claude-authentication.md#key-expiration), and set **Linked account** to yourself or a service account. You can also choose a [workspace](https://platform.claude.com/settings/workspaces) to scope the key to.
  </Step>
...
The [Admin API](../api/api-beta-organization.md) includes endpoints for managing your organization's API keys programmatically, such as [Retrieve API Key](../api/api-beta-organization-api_keys-retrieve.md) and [List API Keys](../api/api-beta-organization-api_keys-list.md). These endpoints are for organization admins automating key management. They accept an [Admin API key](../manage-claude/manage-claude-admin-api-keys.md), an OAuth token with the `org:admin` scope, or a personal or service account key that isn't scoped to a specific workspace; workspace keys don't work there. They never return a key's secret value, only a partially redacted hint.
