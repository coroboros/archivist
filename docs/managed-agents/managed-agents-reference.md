...
| `agent.thread_message_received` | In a [multiagent](./managed-agents-multi-agent.md) session, an agent delivered its result to the coordinator. |
| `agent.thread_message_sent` | In a [multiagent](./managed-agents-multi-agent.md) session, the coordinator sent a follow-up to another agent. |
...
| `session.thread_created` | A [multiagent](./managed-agents-multi-agent.md) thread was created. |
| `session.thread_status_running` | A [multiagent](./managed-agents-multi-agent.md) thread started activity. |
| `session.thread_status_idle` | A [multiagent](./managed-agents-multi-agent.md) thread finished its turn and is awaiting input. Includes `stop_reason`. |
| `session.thread_status_rescheduled` | A [multiagent](./managed-agents-multi-agent.md) thread hit a transient error and is retrying automatically. |
| `session.thread_status_terminated` | A [multiagent](./managed-agents-multi-agent.md) thread was archived or reached a terminal error. |
...
Organization-level [spend limits and tier-based rate limits](../api/api-rate-limits.md) also apply.
