---
title: "spawn.sh: called once per claimed work item"
...
For Zero Data Retention and HIPAA BAA eligibility, see [API and data retention](../manage-claude/manage-claude-api-and-data-retention.md#feature-eligibility).
  ...
  <Tab title="Webhook-triggered (SDK)">
    <Steps>
      ...
      <Step title="Implement the webhook handler">
        ...
        When you hand a claimed work item to `handle_item()` (typescript: `handleItem()`; go: `HandleItem()`) yourself, as this handler does, pass the work item's `secret` along as `work_secret` (typescript: `workSecret`; go: `WorkSecret`) so the session can mount any [memory stores](./managed-agents-self-hosted-sandboxes.md#use-memory-stores) attached to it. A handler like this one runs every claimed item in one process on one host, so two sessions that attach the same memory store cannot run through it at the same time (see [Prepare the host](./managed-agents-self-hosted-sandboxes.md#prepare-the-host)); if your sessions share stores, launch [one sandbox per session](./managed-agents-self-hosted-sandboxes.md#run-one-sandbox-per-session) instead.
...
<Note>
  Self-hosted sandboxes support `memory_store` resources only; see [Use memory stores](./managed-agents-self-hosted-sandboxes.md#use-memory-stores). A session on a self-hosted environment that includes a `file` or `github_repository` resource is rejected with a 400 error:
...
Sessions on a self-hosted environment attach [memory stores](./managed-agents-memory.md) exactly as sessions on cloud environments do: list them in `resources` when you create the session, as shown in [Attach a memory store to a session](./managed-agents-memory.md#attach-a-memory-store-to-a-session). A session accepts up to 8 memory stores. On a self-hosted environment the SDK worker, rather than Anthropic's infrastructure, materializes each store for the agent, so memory stores there require `EnvironmentWorker` (or its `handle_item()` (typescript: `handleItem()`; go: `HandleItem()`) method) from the Python, TypeScript, or Go SDK.

The `ant` CLI worker (`ant beta:worker poll` and `ant beta:worker run`) does not mount memory stores. To combine the CLI poller with memory stores, run the SDK worker inside a per-session sandbox as described in [Run one sandbox per session](./managed-agents-self-hosted-sandboxes.md#run-one-sandbox-per-session).

Memory stores cannot be attached to sessions on self-hosted environments on [Claude Platform on AWS](../build-with-claude/build-with-claude-claude-platform-on-aws.md).
...
Memory stores on self-hosted sandboxes need a POSIX filesystem on the worker host (the Linux host from [Before you begin](./managed-agents-self-hosted-sandboxes.md#before-you-begin)); Windows hosts are not supported, because the worker requires `O_NOFOLLOW` when it opens memory files. A case-sensitive filesystem is recommended, so that memory paths that differ only in case do not collide.
...
| The worker log contains `cannot create the memory store's folder` and `the worker host must make this mount path writable`.             | The user the worker runs as cannot create directories under `/mnt/memory`.                                                                                                                                     | Create `/mnt/memory` and `chown` it to that user; see [Prepare the host](./managed-agents-self-hosted-sandboxes.md#prepare-the-host).                                                                                                                                                             |
| The session sits `idle` with a `requires_action` stop reason and no error event shortly after a worker claimed it.                      | The worker failed the work item because it could not mount a memory store, for one of the preceding reasons.                                                                                                   | Fix the cause on the host, then send a [`user.interrupt`](./managed-agents-events-and-streaming.md#integrating-events) event: the session's work is queued again and the next worker that claims it retries the mount.                                                                            |

## Serve custom tools from your sandbox

[Custom tools](./managed-agents-tools.md#custom-tools) are tools your own code executes: the agent emits an `agent.custom_tool_use` event and waits for a matching `user.custom_tool_result`. The worker can be that code, and because it runs inside your sandbox, the tool reaches the internal services, credentials, and network egress you configured for the sandbox, and nothing more. The environment key authorizes posting custom tool results, so your Claude API key stays off the worker host.
...
<Steps>
  <Step title="Declare the tool on the agent">
    Add a `custom` entry to the agent's `tools` whose `name` matches the tool your worker registers. See [Custom tools](./managed-agents-tools.md#custom-tools) for the full declaration shape.
  ...
  <Step title="Register the implementation with the worker">
    Pass the tool through the worker's `tools` (go: `ToolsFunc`) factory (see [SDK helpers](./managed-agents-self-hosted-sandboxes.md#sdk-helpers)), alongside the built-in toolset:
...
The worker answers only the tools registered with it. A custom tool that is declared on the agent but registered with no worker or client leaves the session paused with a `requires_action` stop reason until something posts its result; see [Handling custom tool calls](./managed-agents-events-and-streaming.md#handling-custom-tool-calls) for the event flow.

### Wrap an MCP server as custom tools

The [MCP connector](./managed-agents-mcp-connector.md) connects to MCP servers from Anthropic's side, so a server must expose an HTTP endpoint that Anthropic can reach, directly or through an [MCP tunnel](../agents-and-tools/agents-and-tools-mcp-tunnels-overview.md). To use a server that only your network can reach, make the worker the MCP client instead and declare the server's tools as custom tools. The MCP server needs no inbound connectivity from outside your network; Anthropic receives the tool definitions you declare on the agent, each call's input, and the result your worker posts back. At runtime the model calls a wrapped tool like any other custom tool:
...
* **Permission policies do not apply to custom tools.** [Permission policies](./managed-agents-permission-policies.md#custom-tools) govern the built-in and MCP toolsets; the worker executes every wrapped tool call the model makes, so put any approval step in your own tool code.
