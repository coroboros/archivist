...
Self-hosted sandboxes are not yet available on [Claude Platform on AWS](../build-with-claude/build-with-claude-claude-platform-on-aws.md).
...
The CLI and SDK both are configurable (see [reference](#reference)), but if you require more control, you can leverage the [Environments Work endpoints](../api/api-beta-environments-work.md) directly and implement your own worker.
