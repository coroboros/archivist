...
- You can also send `user.message` [events](./managed-agents-events-and-streaming.md#user-events) to an outcome-oriented session, to direct the agent's work as it progresses, but these are not as necessary; the agent knows to work until it has exhausted its iterations or achieved the outcome.
...
The agent writes output files to `/mnt/session/outputs/` inside the container. Once the session is idle, fetch them via the [Files API](../build-with-claude/build-with-claude-files.md) scoped to the session:
