...
If a subagent needs something from your client, such as [permission](./managed-agents-events-and-streaming.md#tool-confirmation) to run an `always_ask` tool, or the [result of a custom tool](./managed-agents-events-and-streaming.md#handling-custom-tool-calls), the event is cross-posted to the **primary thread** with `session_thread_id` identifying the originating session thread.
