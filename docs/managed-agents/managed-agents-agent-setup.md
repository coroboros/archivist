...
<Tip>
  To set the model's effort level, pass `model` as an object, for example: `{"id": "claude-opus-4-8", "effort": "high"}`. The `effort` field accepts a level string (`low`, `medium`, `high`, `xhigh`, or `max`) or an object such as `{"type": "high"}`. See [Effort levels](../build-with-claude/build-with-claude-effort.md#effort-levels) for what each level does.
</Tip>
