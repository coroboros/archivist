...
| `model`       | Required. The Claude [model](../general/general-models-overview.md) that powers the agent. Accepts a model ID string or an object, for example `{"id": "claude-opus-5"}`. Claude 4.5 and later models are supported. The object form also accepts `speed`, `effort`, and `inference_geo` fields; see the tips under [Create an agent](./managed-agents-agent-setup.md#create-an-agent), [Effort levels](../build-with-claude/build-with-claude-effort.md#effort-levels), and [Pin the inference geo](./managed-agents-agent-setup.md#pin-the-inference-geo). |
...
The examples use curl, the `ant` CLI, or one of the SDKs. If you haven't set one up, the [quickstart](./managed-agents-quickstart.md#install-the-cli) covers installation and client setup.
...
<Tip>
  To use Claude Opus 5.5, Claude Opus 5, or Claude Opus 4.8 with [fast mode](../build-with-claude/build-with-claude-fast-mode.md), pass `model` as an object, for example: `{"id": "claude-opus-5", "speed": "fast"}`. See the fast mode page's [supported models](../build-with-claude/build-with-claude-fast-mode.md#supported-models).
</Tip>

<Tip>
  To set the model's effort level, pass `model` as an object, for example: `{"id": "claude-opus-5", "effort": "high"}`. The `effort` field accepts a level string (`low`, `medium`, `high`, `xhigh`, or `max`) or an object such as `{"type": "high"}`. See [Effort levels](../build-with-claude/build-with-claude-effort.md#effort-levels) for what each level does.
</Tip>

### Pin the inference geo

Like `speed` and `effort`, `inference_geo` is set through the object form of `model`: pass `model` as an object and set `inference_geo` alongside `id`. The field accepts `"us"` or `"global"`. When it's unset, each model request follows the workspace's default inference geo at the time it's served. See [Data residency](../manage-claude/manage-claude-data-residency.md) for the workspace-level geo controls and pricing.
...
<CardGroup cols={2}>
  <Card title="Tools" icon="tool" href="./managed-agents-tools.md">
    Configure tools available to your agent.
  </Card>

  <Card title="Skills" icon="graduation-cap" href="./managed-agents-skills.md">
    Attach reusable, filesystem-based expertise to your agent for domain-specific workflows.
  </Card>

  <Card title="Start a session" icon="play" href="./managed-agents-sessions.md">
