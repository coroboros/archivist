...
Creating a session provisions the environment's sandbox but does not start any work. To delegate a task, send events to the session using a [user event](./managed-agents-reference.md#event-types). The session acts as a state machine that tracks progress while events drive the actual execution.
...
See [Session statuses](./managed-agents-session-operations.md#session-statuses) for the statuses a session moves through, and [Session operations](./managed-agents-session-operations.md) for retrieving, listing, updating, archiving, and deleting sessions.
