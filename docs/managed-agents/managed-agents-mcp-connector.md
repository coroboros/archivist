...
| `url`  | Required. The endpoint of the remote MCP server (up to 2048 characters). See [Supported MCP server types](./managed-agents-reference.md#supported-mcp-server-types) for transport requirements.                                          |
...
Credentials are matched by URL, so the vault must contain a credential whose `mcp_server_url` exactly matches the `url` declared in `mcp_servers`. If none matches, the connection is attempted unauthenticated. See [Add a credential](./managed-agents-vaults.md#add-a-credential) for the `static_bearer` and `mcp_oauth` credential types.
...
<CardGroup cols={2}>
  <Card title="Permission policies" icon="check" href="./managed-agents-permission-policies.md">
  ...
  <Card title="Supported MCP server types" icon="book" href="./managed-agents-reference.md">
