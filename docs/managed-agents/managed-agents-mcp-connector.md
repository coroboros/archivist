...
Claude Managed Agents connects to [remote MCP servers](../agents-and-tools/agents-and-tools-remote-mcp-servers.md) that expose an HTTP endpoint, or to private MCP servers through [MCP tunnels](../agents-and-tools/agents-and-tools-mcp-tunnels-overview.md). The server must support the MCP protocol's streamable HTTP transport.
