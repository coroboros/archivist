...
To retrieve a single run by ID, call `GET /v1/deployment_runs/{deployment_run_id}`. A [`deployment_run` webhook event](./managed-agents-webhooks.md#supported-event-types) carries the run ID as its `data.id`.
