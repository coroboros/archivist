...
<Tip>
  This page covers how refusals appear in streaming responses. For every `stop_reason` value and how to handle it, see [Stop reasons and fallback](../build-with-claude/build-with-claude-handling-stop-reasons.md). To retry refused requests on another Claude model, see [Refusals and fallback](../build-with-claude/build-with-claude-refusals-and-fallback.md).
</Tip>
...
<Note>
  Usage metrics are still provided in the response, even when the response is refused. Whether a refused request is billed depends on when the refusal arrives and its category; see [How refusals are billed](../build-with-claude/build-with-claude-refusals-and-fallback.md#how-refusals-are-billed).
</Note>

<Tip>
  Resetting context is not the only way to recover. You can also retry the refused request on a different Claude model, and the [Refusals and fallback](../build-with-claude/build-with-claude-refusals-and-fallback.md) page shows how to set that up with server-side fallback, the SDK middleware, or a manual retry.
</Tip>
...
* **Fall back to another model:** Configure [server-side fallback or the SDK middleware](../build-with-claude/build-with-claude-refusals-and-fallback.md) so refused requests are retried on another Claude model instead of surfacing a refusal to the user
* **Redeem fallback credit on manual retries:** If you build the retry yourself, pass the refusal's [fallback credit](../build-with-claude/build-with-claude-fallback-credit.md) token so the retry doesn't pay the prompt-cache cost twice
...
* **Refusals include structured detail.** On every model, a refusal also includes a `stop_details` object that identifies the policy category behind the decline. See [Refusals and fallback](../build-with-claude/build-with-claude-refusals-and-fallback.md#refusal-response) for the full response shape.
...
* **Check batch results for refusals.** A refused request in a [Message Batch](../build-with-claude/build-with-claude-batch-processing.md) is returned as a succeeded result with `stop_reason`: `"refusal"`, not as an errored result.
...
<CardGroup cols={2}>
  <Card title="Refusals and fallback" icon="arrows-clockwise" href="../build-with-claude/build-with-claude-refusals-and-fallback.md">
    Retry refused requests on another Claude model, server-side or in your client.
  </Card>

  <Card title="Stop reasons and fallback" icon="code" href="../build-with-claude/build-with-claude-handling-stop-reasons.md">
  ...
  <Card title="Multilingual support" icon="text-aa" href="../build-with-claude/build-with-claude-multilingual-support.md">
