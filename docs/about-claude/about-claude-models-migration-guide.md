...
<Note>
This guide covers migrating [Messages API](../build-with-claude/build-with-claude-working-with-messages.md) code. If you use [Claude Managed Agents](../managed-agents/managed-agents-overview.md), no changes beyond updating model name are required.
</Note>
...
- [Adaptive thinking](../build-with-claude/build-with-claude-adaptive-thinking.md)
...
- The full set of server-side and client-side [tools](../agents-and-tools/agents-and-tools-tool-use-overview.md) ([bash](../agents-and-tools/agents-and-tools-tool-use-bash-tool.md), [code execution](../agents-and-tools/agents-and-tools-tool-use-code-execution-tool.md), [computer use](../agents-and-tools/agents-and-tools-tool-use-computer-use-tool.md), [text editor](../agents-and-tools/agents-and-tools-tool-use-text-editor-tool.md), [web search](../agents-and-tools/agents-and-tools-tool-use-web-search-tool.md), [web fetch](../agents-and-tools/agents-and-tools-tool-use-web-fetch-tool.md), [MCP connector](../agents-and-tools/agents-and-tools-mcp-connector.md), [memory](../agents-and-tools/agents-and-tools-tool-use-memory-tool.md))

<Tip>
  **Automate this migration with the Claude API skill.** In Claude Code, run `/claude-api migrate` to invoke the bundled [Claude API skill](../agents-and-tools/agents-and-tools-agent-skills-claude-api-skill.md#migrating-to-a-newer-claude-model):
...
1. **Extended thinking removed:** `thinking: {type: "enabled", budget_tokens: N}` is no longer supported on Claude Opus 4.7 or later models and returns a 400 error. Switch to [adaptive thinking](../build-with-claude/build-with-claude-adaptive-thinking.md) (`thinking: {type: "adaptive"}`) and use the [effort parameter](../build-with-claude/build-with-claude-effort.md) to control thinking depth. Adaptive thinking is **off by default** on Claude Opus 4.7: requests with no `thinking` field run without thinking, matching Opus 4.6 behavior. Set `thinking: {type: "adaptive"}` explicitly to enable it.
    ...
    Adaptive thinking is steerable through prompting. For guidance on tuning when the model over- or under-thinks, see [Calibrating effort and thinking depth](../build-with-claude/build-with-claude-prompt-engineering-claude-prompting-best-practices.md#calibrating-effort-and-thinking-depth).
...
4. **Updated token counting:** Claude Opus 4.7 uses a new tokenizer, contributing to its improved performance on a wide range of tasks. The new tokenizer may use roughly 1x to 1.35x as many tokens when processing text compared to previous models (up to ~35% more, varying by content).
    ...
    [`/v1/messages/count_tokens`](../build-with-claude/build-with-claude-token-counting.md) will return a different number of tokens for Claude Opus 4.7 than it did for Claude Opus 4.6. Token efficiency can vary by workload shape.
...
5. **Prefill removal (carried over from Opus 4.6):** Prefilling assistant messages returns a 400 error on Claude Opus 4.7. Use [structured outputs](../build-with-claude/build-with-claude-structured-outputs.md), system prompt instructions, or `output_config.format` instead.
...
6. **Stricter effort calibration:** Meaningfully changing from Claude Opus 4.6, Claude Opus 4.7 respects [effort levels](../build-with-claude/build-with-claude-effort.md) strictly, especially at the low end. At `low` and `medium`, the model scopes its work to what was asked rather than going above and beyond.
...
2. **Audit token-count expectations:** Any code path that estimates tokens client-side or assumes a fixed token-to-character ratio should be re-tested against Claude Opus 4.7. Use the [Token counting endpoint](../build-with-claude/build-with-claude-token-counting.md) to verify.
...
- [ ] Replace `thinking: {type: "enabled", budget_tokens: N}` with `thinking: {type: "adaptive"}` plus the [effort parameter](../build-with-claude/build-with-claude-effort.md).
...
2. **Update tool versions**
   ...
   - **Text editor:** Use `text_editor_20250728` and `str_replace_based_edit_tool`. See [Text editor tool documentation](../agents-and-tools/agents-and-tools-tool-use-text-editor-tool.md) for details.
   - **Code execution:** Upgrade to `code_execution_20250825`. See [Code execution tool documentation](../agents-and-tools/agents-and-tools-tool-use-code-execution-tool.md#upgrade-to-latest-tool-version) for migration instructions.
...
1. **Prefilling assistant messages is no longer supported**
   ...
   Prefilling assistant messages returns a `400` error on Sonnet 4.6. Use [structured outputs](../build-with-claude/build-with-claude-structured-outputs.md), system prompt instructions, or `output_config.format` instead.

   **Common prefill use cases and migrations:**

   - **Controlling output formatting** (forcing JSON/YAML output): Use [structured outputs](../build-with-claude/build-with-claude-structured-outputs.md) or tools with enum fields for classification tasks.
...
If you're using extended thinking with `budget_tokens` on Sonnet 4.5, it is still functional on Sonnet 4.6 but is deprecated. Migrate to [adaptive thinking](../build-with-claude/build-with-claude-adaptive-thinking.md) with the [effort parameter](../build-with-claude/build-with-claude-effort.md).

##### Migrating to adaptive thinking

[Adaptive thinking](../build-with-claude/build-with-claude-adaptive-thinking.md) is the recommended replacement for `budget_tokens` on Sonnet 4.6. It is particularly well suited to the following workload patterns:
...
- [ ] Review and update prompts following [prompting best practices](../build-with-claude/build-with-claude-prompt-engineering-claude-prompting-best-practices.md)
- [ ] **Recommended:** Migrate from `thinking: {type: "enabled", budget_tokens: N}` to `thinking: {type: "adaptive"}` with the [effort parameter](../build-with-claude/build-with-claude-effort.md) (`budget_tokens` is deprecated and will be removed in a future release)
- [ ] Test in development environment before production deployment
...
Extended thinking is deprecated in Claude 4.6 or newer models. If using newer models, use [adaptive thinking](../build-with-claude/build-with-claude-adaptive-thinking.md) instead.
</Note>
