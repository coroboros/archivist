...
**Enforce coding discipline via rules, not skills.** Distill [Andrej Karpathy's observations on LLM coding pitfalls](https://x.com/karpathy/status/2015883857489522876) (see also: [forrestchang's CLAUDE.md compilation](https://github.com/forrestchang/andrej-karpathy-skills/blob/main/CLAUDE.md)) into always-on rules — behavioral guardrails that auto-load every session, rather than skills requiring manual invocation. Only the rules that add to Claude Code's built-in guidelines are worth including — ~70% of typical advice is already handled natively by the system prompt (simplicity, no speculative features, surgical changes).
