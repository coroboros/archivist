# Claude Insights Docs

Manually curated documentation extracted from **Anthropic team members** and **official Anthropic online resources** (posts, talks, examples, threads, etc.).

This folder is maintained manually and is **not part of the automated official docs mirror**.

---

## Claude Code

- [Best Practices for Using Claude Opus 4.7 with Claude Code](./claude-code-best-practices-opus-4-7.md)
- [Claude Code — Key Practices](./claude-code-key-practices.md)
- [Hidden and Under-Utilized Features in Claude Code](./claude-code-hidden-features.md)
- [How Boris Cherny Uses Claude Code (Creator Setup Guide)](./claude-code-workflow-boris-cherny.md)
- [Using Claude Code: The Unreasonable Effectiveness of HTML](./claude-code-html-effectiveness.md)

## Claude Project

- [Best Practices for Claude Project Knowledge Bases](./claude-project-knowledge-bases-best-practices.md)

## Skills

- [Lessons from Building Claude Code: How We Use Skills](./skills-how-anthropic-uses-skills.md)
- [The Complete Guide to Building Skills for Claude](./skills-complete-guide-to-building-skills-for-claude.md)

## MCP

- [Code execution with MCP: Building more efficient agents](./mcp-code-execution.md)
