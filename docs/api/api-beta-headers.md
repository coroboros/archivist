...
The SDKs' `beta` namespaces add these headers automatically. Add them yourself only when making raw HTTP requests. See the [Managed Agents overview](../managed-agents/managed-agents-overview.md) and the [MCP tunnels reference](../agents-and-tools/agents-and-tools-mcp-tunnels-reference.md#tunnels-api) for details.
