...
<Note>
**[Claude Platform on AWS](../build-with-claude/build-with-claude-claude-platform-on-aws.md):** The rate limits on this page apply. Billing and spend limits differ: spend limits are not available, and billing is through AWS Marketplace (not Anthropic credit purchases). Organizations start at Tier 1. Rate limit increases go through your Anthropic account representative; there is no automatic tier advancement, and per-workspace rate limit configuration is not available. [Fast mode](../build-with-claude/build-with-claude-fast-mode.md) is not available on Claude Platform on AWS.
</Note>
...
For rate limit purposes on most models, only `input_tokens` + `cache_creation_input_tokens` count toward your ITPM limit, making [prompt caching](../build-with-claude/build-with-claude-prompt-caching.md) an effective way to increase your effective throughput.
</Note>
...
For all models without the † marker, cached input tokens do not count towards rate limits and are billed at a reduced rate (10% of base input token price). This means you can achieve significantly higher effective throughput by using [prompt caching](../build-with-claude/build-with-claude-prompt-caching.md).
</Note>

<Tip>
**Maximize your rate limits with prompt caching**

To get the most out of your rate limits, use [prompt caching](../build-with-claude/build-with-claude-prompt-caching.md) for repeated content like:
...
You can check your current rate limits and behavior in the [Claude Console](/settings/limits), or read the configured limits programmatically with the [Rate Limits API](../manage-claude/manage-claude-rate-limits-api.md).
...
| Claude Haiku 3.5 ([retired, except on Bedrock and Vertex AI](../about-claude/about-claude-model-deprecations.md)) | 50                                | 50,000<sup>†</sup>                     | 10,000                                  |
...
| Claude Haiku 3.5 ([retired, except on Bedrock and Vertex AI](../about-claude/about-claude-model-deprecations.md)) | 1,000                             | 100,000<sup>†</sup>                    | 20,000                                  |
...
| Claude Haiku 3.5 ([retired, except on Bedrock and Vertex AI](../about-claude/about-claude-model-deprecations.md)) | 2,000                             | 200,000<sup>†</sup>                    | 40,000                                  |
...
| Claude Haiku 3.5 ([retired, except on Bedrock and Vertex AI](../about-claude/about-claude-model-deprecations.md)) | 4,000                             | 400,000<sup>†</sup>                    | 80,000                                  |
...
_<sup>* - Opus rate limit is a total limit that applies to combined traffic across Claude Opus 4.8, Opus 4.7, Opus 4.6, Opus 4.5, Opus 4.1 ([deprecated](../about-claude/about-claude-model-deprecations.md)), and Opus 4 ([deprecated](../about-claude/about-claude-model-deprecations.md)).</sup>_

_<sup>** - Sonnet 4.x rate limit is a total limit that applies to combined traffic across Sonnet 4.6, Sonnet 4.5, and Sonnet 4 ([deprecated](../about-claude/about-claude-model-deprecations.md)).</sup>_
...
[Claude Managed Agents](../managed-agents/managed-agents-overview.md) endpoints are rate-limited per organization. These limits are separate from the Messages API rate limits above.
...
When using [fast mode](../build-with-claude/build-with-claude-fast-mode.md) (research preview) with `speed: "fast"` on Claude Opus 4.8, Opus 4.7, or Opus 4.6, dedicated rate limits apply that are separate from standard Opus rate limits. When fast mode rate limits are exceeded, the API returns a `429` error with a `retry-after` header.

The response includes `anthropic-fast-*` headers that indicate your fast mode rate limit status. See [Fast mode](../build-with-claude/build-with-claude-fast-mode.md#rate-limits) for details on these headers.
...
For more about workspaces, see [Workspaces](../manage-claude/manage-claude-workspaces.md).
...
To read your current organization and workspace rate limits programmatically, use the [Rate Limits API](../manage-claude/manage-claude-rate-limits-api.md).
