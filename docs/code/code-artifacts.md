...
<Note>
  Artifacts are available on Pro, Max, Team, and Enterprise plans and require a session signed in with [`/login`](./code-setup.md#authenticate). See [Availability](#availability) for the full set of requirements.
</Note>
...
As of Claude Code v2.1.183, Claude applies a built-in design skill when it builds an artifact, so pages get a deliberate palette, typography, and layout without extra prompting. That skill also looks for an existing design system in your project before choosing its own. To keep artifacts consistent with your product's branding, record your design tokens where Claude can find them, such as the project's [CLAUDE.md](./code-memory.md) or a theme file in your repository:
...
| Authentication      | The session is backed by a claude.ai account: sign in with `/login` in the CLI or desktop app. Claude Tag sessions are signed in through the agent's identity, so no step is needed there. Sessions using an API key, [gateway token](./code-llm-gateway.md), or cloud-provider credential cannot publish.                                                                                                                                          |
| Model provider      | Anthropic API. Not available on [Amazon Bedrock](./code-amazon-bedrock.md), [Google Cloud's Agent Platform](./code-google-vertex-ai.md), or [Microsoft Foundry](./code-microsoft-foundry.md).                                                                                                                                                                                                                                                                   |
| Organization policy | Customer-managed encryption keys (CMEK), HIPAA, and [Zero Data Retention](./code-zero-data-retention.md) are not enabled for the organization.                                                                                                                                                                                                                                                                                                      |
| Surface             | Claude Code CLI version 2.1.183 or later, or the Claude desktop app version 1.13576.0 or later. [Claude Tag](https://claude.com/docs/claude-tag/overview) sessions can also publish artifacts when both Claude Tag and artifacts are enabled for the organization. Off by default in [Agent SDK](./code-agent-sdk/overview.md), GitHub Action, and MCP-server contexts, and when [`CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`](./code-env-vars.md) is set. |
...
| Method                               | Setting                              |
| :----------------------------------- | :----------------------------------- |
| [Settings file](./code-settings.md)        | `"disableArtifact": true`            |
| [Environment variable](./code-env-vars.md) | `CLAUDE_CODE_DISABLE_ARTIFACT=1`     |
| [Permission rule](./code-permissions.md)   | Add `Artifact` to `permissions.deny` |
...
The viewer on claude.ai loads each artifact from a sandboxed `*.claudeusercontent.com` origin. If your organization restricts outbound network access, add that domain to your allowlist alongside `claude.ai`. See [Network access requirements](./code-network-config.md#network-access-requirements) for the full list.
...
* Browse [prompting patterns and workflows](./code-prompt-library.md) that pair with artifacts
* Turn an artifact prompt you reuse into a [skill](./code-skills.md) so you can invoke it as a command
* [Connect MCP servers](./code-mcp.md) so Claude can pull data into an artifact while it builds the page
