...
Claude applies a built-in design skill when it builds an artifact, so pages get a deliberate palette, typography, and layout without extra prompting. That skill also looks for an existing design system in your project before choosing its own. To keep artifacts consistent with your product's branding, record your design tokens where Claude can find them, such as the project's [CLAUDE.md](./code-memory.md) or a theme file in your repository:
...
| Authentication      | Signed in to claude.ai with `/login`. Sessions using an API key, [gateway token](./code-llm-gateway.md), or cloud-provider credential cannot publish.                                                                                                                                    |
...
| Surface             | Claude Code CLI, or the Claude desktop app version 1.13576.0 or later. Off by default in [Agent SDK](./code-agent-sdk/overview.md), GitHub Action, and MCP-server contexts, and when [`CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`](./code-env-vars.md) is set.                                  |
