...
* **Corporate HTTP proxies**: an `HTTPS_PROXY` sits between Claude Code and every server it talks to, including the gateway. If your network requires one, [configure the proxy](./code-network-config.md) in addition to the gateway. For a Claude apps gateway you host, [sign-in checks that the proxy host is also on a private network](./code-claude-apps-gateway.md#prerequisites); if it isn't, add the gateway host to `NO_PROXY` so the CLI connects to it directly.
