...
* [Claude Code](./code-quickstart.md#step-1-install-claude-code)
...
In [plan mode](./code-permission-modes.md#analyze-before-you-edit-with-plan-mode), browser tool calls that only read the page or browser state run without a permission prompt, and calls that change state prompt for approval.
...
* **Permissions**: Claude can upload a file only when the session is allowed to read it, so [permission rules](./code-settings.md#permission-settings) that deny `Read` access to a file also block uploading it.
