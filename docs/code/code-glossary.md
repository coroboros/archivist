...
Learn more: [Run agent teams](./code-agent-teams.md)
...
Learn more: [How Claude Code works](./code-how-claude-code-works.md)
...
Learn more: [Auto memory](./code-memory.md#auto-memory)
...
Learn more: [Start faster with bare mode](./code-headless.md#start-faster-with-bare-mode)
...
Learn more: [Bundled skills](./code-skills.md#bundled-skills)
...
Learn more: [Channels](./code-channels.md)
...
Learn more: [Checkpointing](./code-checkpointing.md)
...
Learn more: [The `.claude` directory](./code-claude-directory.md)
...
Learn more: [CLAUDE.md files](./code-memory.md#claude-md-files)
...
Two other uses of the word are unrelated: `claude` CLI subcommands such as `claude mcp add`, listed in the [CLI reference](./code-cli-reference.md#cli-commands), and the `command` field of a stdio [MCP server](#mcp-server) entry, which specifies the executable Claude Code launches to start the server.

Learn more: [Commands](./code-commands.md) · [Skills](./code-skills.md)
...
Learn more: [What survives compaction](./code-context-window.md#what-survives-compaction) · [When context fills up](./code-how-claude-code-works.md#when-context-fills-up)
...
Learn more: [Use MCP servers from claude.ai](./code-mcp.md#use-mcp-servers-from-claude-ai)
...
Learn more: [Explore the context window](./code-context-window.md)
...
Learn more: [Sessions from Dispatch](./code-desktop.md#sessions-from-dispatch)
...
Learn more: [Use extended thinking](./code-model-config.md#extended-thinking)
...
Learn more: [Get started with hooks](./code-hooks-guide.md) · [Hooks reference](./code-hooks.md)

## M

### Managed settings

Settings enforced org-wide by IT or DevOps, delivered from Anthropic's servers through the admin console or deployed to devices at an OS-level path outside `~/.claude`. User and project settings cannot override managed settings. Server-managed delivery applies on [eligible configurations](./code-server-managed-settings.md#platform-availability); see [Security considerations](./code-server-managed-settings.md#security-considerations). Use this for security policies, compliance requirements, or standardized tooling across a fleet.
...
Learn more: [Model Context Protocol](./code-mcp.md)
...
Learn more: [Scale with MCP Tool Search](./code-mcp.md#scale-with-mcp-tool-search)

## N

### Non-interactive mode

A mode that executes a single prompt and exits without an interactive prompt, invoked with `-p` or `--print`. Used for CI, scripts, and piping. The run is still saved as a resumable session unless you pass `--no-session-persistence`. The [Agent SDK](./code-agent-sdk/overview.md) is the Python and TypeScript equivalent. Formerly called headless mode.

Learn more: [Run Claude Code programmatically](./code-headless.md)
...
Learn more: [Output styles](./code-output-styles.md)
...
Learn more: [Choose a permission mode](./code-permission-modes.md)
...
Learn more: [Configure permissions](./code-permissions.md)
...
Learn more: [Analyze before you edit with plan mode](./code-permission-modes.md#analyze-before-you-edit-with-plan-mode)

### Plugin

A bundle of skills, hooks, subagents, and MCP servers packaged as a single installable unit. Plugin skills are namespaced as `plugin-name:skill-name` so multiple plugins coexist. Distribute plugins across teams via a [marketplace](./code-plugin-marketplaces.md).

Learn more: [Plugins](./code-plugins.md)
...
Learn more: [Remote Control](./code-remote-control.md)
...
Learn more: [Organize rules with `.claude/rules/`](./code-memory.md#organize-rules-with-claude/rules/)
...
Learn more: [Sandboxing](./code-sandboxing.md)
...
Learn more: [Work with sessions](./code-how-claude-code-works.md#work-with-sessions)
...
Learn more: [Settings files](./code-settings.md#settings-files)
...
Learn more: [Extend Claude with skills](./code-skills.md)
...
Learn more: [Create custom subagents](./code-sub-agents.md)

### Surface

Any place you access Claude Code: the CLI, VS Code, JetBrains, Desktop, or claude.ai. All surfaces share the same engine. Sessions on your machine read your local CLAUDE.md, settings, and skills; [cloud sessions](./code-claude-code-on-the-web.md#what’s-available-in-cloud-sessions) start from a fresh clone of your repository and don't read `~/.claude/` on your machine. Slack and the Chrome extension are integrations that connect to a surface rather than surfaces themselves.

Learn more: [Platforms and integrations](./code-platforms.md)
...
Learn more: [From web to terminal](./code-claude-code-on-the-web.md#from-web-to-terminal)
...
Learn more: [Tools available to Claude](./code-tools-reference.md)
...
How a session knows the work is actually done rather than just plausible. You give Claude a check it can run, such as a test suite, a build, or a screenshot comparison, and Claude iterates until the check passes instead of stopping after one attempt. A verification loop is the prerequisite for [`/goal`](./code-goal.md), unattended runs, and [dynamic workflows](./code-workflows.md): without one, the only thing deciding the agent is finished is the agent itself.

Learn more: [Give Claude a way to verify its work](./code-best-practices.md#give-claude-a-way-to-verify-its-work)
...
Learn more: [Run parallel sessions with git worktrees](./code-worktrees.md)
