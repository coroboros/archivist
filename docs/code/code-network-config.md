...
Put the same variables in the `env` block of `~/.claude/settings.json` or [managed settings](./code-settings.md) instead. Every variable on this page can be set there, and settings are the only configuration that reaches every background session on every machine.
...
Set the [`processWrapper`](./code-settings.md#available-settings) setting to prefix the supervisor, its workers, and the other background processes listed under [What the launcher covers](./code-corporate-launcher.md#what-the-launcher-covers) with your launcher. The equivalent [`CLAUDE_CODE_PROCESS_WRAPPER`](./code-env-vars.md) environment variable takes precedence when both are set, and it is subject to the same rule: deliver it through managed settings or `~/.claude/settings.json`, not a shell export. [Run Claude Code behind a corporate launcher](./code-corporate-launcher.md) covers the contract the launcher must satisfy, what it does and doesn't reach, and how to roll it out.

<Note>
  An already-running supervisor keeps the launch configuration it started with. After deploying the launcher setting, run [`claude daemon stop --any`](./code-agent-view.md#the-supervisor-process) so the next `claude agents` or `--bg` starts a supervisor that honors it. An installed service takes `claude daemon stop` without `--any`.
</Note>
...
| `mcp-proxy.anthropic.com`      | [MCP connectors from claude.ai](./code-mcp.md#use-mcp-servers-from-claude-ai), including connectors an organization administrator configures. Connector traffic routes through this proxy; connectors are enabled by default for claude.ai-authenticated users. To disable, set [`ENABLE_CLAUDEAI_MCP_SERVERS=false`](./code-env-vars.md) or the [`disableClaudeAiConnectors`](./code-settings.md#available-settings) setting |
| `downloads.claude.ai`          | Plugin executable downloads; native installer and native auto-updater                                                                                                                                                                                                                                                                                                                                       |
| `storage.googleapis.com`       | Install counts and plugin metadata shown in `/plugin`. Signed [artifact](./code-artifacts.md) uploads try this host first; publishing falls back to `api.anthropic.com` when it is blocked                                                                                                                                                                                                                        |
| `storage.googleapis.com`       | {/* max-version: 2.1.115 */}Native installer and native auto-updater on versions prior to 2.1.116                                                                                                                                                                                                                                                                                                           |
| `bridge.claudeusercontent.com` | [Claude in Chrome](./code-chrome.md) extension WebSocket bridge                                                                                                                                                                                                                                                                                                                                                   |
| `*.claudeusercontent.com`      | Viewing [artifacts](./code-artifacts.md) on claude.ai. The viewer loads each artifact's content from a sandboxed subdomain of this origin. Required in the viewer's browser, not by the CLI itself                                                                                                                                                                                                                |
| `raw.githubusercontent.com`    | Changelog feed for [`/release-notes`](./code-commands.md) and the release notes shown after updating                                                                                                                                                                                                                                                                                                              |
