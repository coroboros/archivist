...
| `raw.githubusercontent.com`    | Changelog feed for [`/release-notes`](./code-commands.md) and the release notes shown after updating; plugin marketplace install counts                                                            |
...
When using [Amazon Bedrock](./code-amazon-bedrock.md), [Google Vertex AI](./code-google-vertex-ai.md), [Microsoft Foundry](./code-microsoft-foundry.md), or a signed-in [Claude apps gateway](./code-claude-apps-gateway.md) session, model traffic and authentication go to your provider or gateway instead of `api.anthropic.com`, `claude.ai`, or `platform.claude.com`. The WebFetch tool still calls `api.anthropic.com` for its [domain safety check](./code-data-usage.md#webfetch-domain-safety-check) unless you set `skipWebFetchPreflight: true` in [settings](./code-settings.md).
