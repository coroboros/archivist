...
To prevent registration entirely, set [`disableDeepLinkRegistration`](./code-settings.md) to `"disable"` in `settings.json`. To enforce this across an organization so users cannot re-enable it, set it in [managed settings](./code-server-managed-settings.md) instead.

## Open a VS Code tab instead of a terminal

The VS Code extension registers its own handler at `vscode://anthropic.claude-code/open`, which opens a Claude Code editor tab rather than a terminal window. See [Launch a VS Code tab from other tools](./code-vs-code.md#launch-a-vs-code-tab-from-other-tools) for that URL's parameters.
...
* [Skills](./code-skills.md): store a long runbook prompt as a `/skill` in the repo so the deep link's `q` parameter only has to name it
* [Non-interactive mode](./code-headless.md): run Claude from a script and capture the output without opening a terminal
