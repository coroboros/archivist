...
| :------------------------------------------- | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`CLAUDE_CODE_MAX_RETRIES`](./code-env-vars.md)    | 10      | Number of retry attempts. {/* min-version: 2.1.186 */}Capped at 15 as of v2.1.186. Lower it to surface failures faster in scripts.                                   |
| [`CLAUDE_CODE_RETRY_WATCHDOG`](./code-env-vars.md) | unset   | Set to `1` in unattended sessions such as CI jobs to retry `429` and `529` capacity errors indefinitely instead of failing after `CLAUDE_CODE_MAX_RETRIES` attempts. |
...
The model that [auto mode](./code-permission-modes.md#eliminate-prompts-with-auto-mode) uses to classify actions could not produce a decision, so auto mode did not approve the action automatically. The message you see depends on why the classifier failed.
...
* This is transient and unrelated to [auto mode eligibility](./code-permission-modes.md#eliminate-prompts-with-auto-mode); you do not need to change settings
...
In an interactive session, auto mode falls back to a normal permission prompt for that action so you can approve or deny it manually. In [non-interactive mode](./code-headless.md) the run aborts because the transcript only grows and retrying cannot succeed.
...
Environment variables and `apiKeyHelper` take precedence over `/login`, so running `/login` alone does not help while either is still supplying a key. See [Authentication precedence](./code-authentication.md#authentication-precedence).
...
* Use an alias such as `sonnet` or `opus` instead of a full versioned ID. Aliases resolve to a maintained default so they do not go stale. See [Model configuration](./code-model-config.md).
