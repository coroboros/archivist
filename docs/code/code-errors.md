...
| :------------------------------------------- | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`CLAUDE_CODE_MAX_RETRIES`](./code-env-vars.md)    | 10      | Number of retry attempts. {/* min-version: 2.1.186 */}Capped at 15 as of v2.1.186; {/* min-version: 2.1.199 */}as of v2.1.199 `CLAUDE_CODE_RETRY_WATCHDOG` raises the default and removes the cap. Lower it to surface failures faster in scripts.                                                                                                                                                                                                                               |
| [`CLAUDE_CODE_RETRY_WATCHDOG`](./code-env-vars.md) | unset   | Set to `1` in unattended sessions such as CI jobs to retry `429` and `529` capacity errors indefinitely instead of failing after `CLAUDE_CODE_MAX_RETRIES` attempts. {/* min-version: 2.1.199 */}As of v2.1.199 it also raises the default retry count for other transient errors, such as server errors, timeouts, and dropped connections, to 300, roughly three hours of backoff, and removes the cap of 15 on `CLAUDE_CODE_MAX_RETRIES` if you set that variable explicitly. |
| [`API_TIMEOUT_MS`](./code-env-vars.md)             | 600000  | Per-request timeout in milliseconds. Raise it for slow networks or proxies.                                                                                                                                                                                                                                                                                                                                                                                                      |
...
The model that [auto mode](./code-permission-modes.md#eliminate-prompts-with-auto-mode) uses to classify actions couldn't produce a decision, so auto mode didn't approve the action automatically. The message you see depends on why the classifier failed.
...
* This is transient and unrelated to [auto mode eligibility](./code-permission-modes.md#eliminate-prompts-with-auto-mode); you don't need to change settings
...
* Switch to a different [permission mode](./code-permission-modes.md) so you can approve the action when prompted, or start a fresh conversation without the triggering content
...
In an interactive session, auto mode falls back to a normal permission prompt for that action so you can approve or deny it manually. In [non-interactive mode](./code-headless.md) the run aborts because the transcript only grows and retrying can't succeed.
...
{/* min-version: 2.1.199 */}A [subagent](./code-sub-agents.md)'s API request failed terminally, for example because a usage limit was reached or retries for a server error ran out, so the subagent stopped before finishing its task. This message requires Claude Code v2.1.199 or later; before then the API error text was returned to Claude as if it were the subagent's result.
...
* Once the underlying error clears, ask Claude to retry the task or [resume the subagent](./code-sub-agents.md#resume-subagents)

When a rate limit, overload, or server error interrupts a foreground subagent that already produced text output, Claude receives that partial output marked as incomplete instead of this error. {/* min-version: 2.1.200 */}A subagent whose only output was tool calls gets this error too; in v2.1.199 that shape returned an empty partial result instead. See [API errors in subagents](./code-sub-agents.md#api-errors-in-subagents).
...
* Reduce concurrency: lower [`CLAUDE_CODE_MAX_TOOL_USE_CONCURRENCY`](./code-env-vars.md), avoid running many parallel subagents, or switch to a smaller model with `/model` for high-volume scripted runs
...
* Set per-workspace spend caps in the Console to prevent a single project from draining the org balance. See [Manage costs effectively](./code-costs.md).
...
* See [Authentication precedence](./code-authentication.md#authentication-precedence) to understand which credential Claude Code uses when several are present
...
The session reached the API client without any credential. This appears in [background sessions](./code-agent-view.md), cloud sessions, and Agent SDK contexts where the interactive login check doesn't run before the first request.
...
* If the key comes from an [`apiKeyHelper`](./code-settings.md#available-settings) script, run the script directly to confirm it prints a valid key on stdout
* Run `/status` to confirm which credential source Claude Code is actually using

### Your apiKeyHelper script is failing

The command configured in the [`apiKeyHelper`](./code-settings.md#available-settings) setting exited with an error, timed out, or printed nothing to stdout. Without a key from the script, the request reaches the API with a placeholder credential, and the API rejects it with `401`.
...
Running `/login` doesn't help here: the helper's output [takes precedence](./code-authentication.md#authentication-precedence) over a saved login for as long as the setting is present.
...
* Fix the command so it prints the key to stdout and exits with code 0. See [rotate credentials with apiKeyHelper](./code-llm-gateway-connect.md#rotate-credentials-with-apikeyhelper) for a working setup.
...
Environment variables and `apiKeyHelper` take precedence over `/login`, so running `/login` alone doesn't help while either is still supplying a key. See [Authentication precedence](./code-authentication.md#authentication-precedence).
...
* If the message names `apiKeyHelper`, remove the [`apiKeyHelper`](./code-settings.md#available-settings) setting from your `settings.json`
...
* Authenticate with a Console API key instead of your subscription. See [Claude Console authentication](./code-authentication.md#claude-console-authentication) for setup.
...
An Owner in your Team or Enterprise organization has turned off routines at the organization level. The error appears when you try to create or run a routine, including from `/schedule` and the [Routines](./code-routines.md) UI on claude.ai/code.
...
The session isn't talking to the Anthropic API directly, so there is no claude.ai backend for [Remote Control](./code-remote-control.md) to pair with.

```text theme={null}
Remote Control is only available when using Claude via api.anthropic.com.
```

This appears on Amazon Bedrock, Google Cloud's Agent Platform, and Microsoft Foundry. {/* min-version: 2.1.196 */}As of v2.1.196 it also appears when [`ANTHROPIC_BASE_URL`](./code-env-vars.md) points at a host other than `api.anthropic.com`, such as an [LLM gateway](./code-llm-gateway.md) or proxy, even when you sign in with claude.ai.
...
* For this and the other Remote Control startup messages, see [Troubleshoot Remote Control](./code-remote-control.md#troubleshooting)
...
In [non-interactive mode](./code-headless.md) (`-p`) and the [Agent SDK](./code-agent-sdk/overview.md), the message reads as follows, and the structured error code is `authentication_failed`:
...
Sessions authenticated with an API key, [`CLAUDE_CODE_OAUTH_TOKEN`](./code-env-vars.md), or a third-party provider don't use the saved login and never see this message.

{/* min-version: 2.1.210 */}You can check for this state before a request fails: [`/status`](./code-commands.md) shows a `Login` row reading `Expired — log in again`, plus the organization and email it has saved for the expired login. The row appears only when the saved login is your active credential and can no longer be refreshed. Sessions authenticated another way don't show the row, even if an expired login remains saved. Before v2.1.210, `/status` gave no indication in this state that a login had ever existed, because the cleared credential left it nothing to report.

**What to do:**

* Run `/login` to sign in again. Retrying without signing in shows the same message on every request.
* In non-interactive mode, run `claude` in the same environment, complete `/login`, then rerun your command. For automation that can't sign in interactively, authenticate with `ANTHROPIC_API_KEY` or [generate a long-lived token with `claude setup-token`](./code-authentication.md#generate-a-long-lived-token).
* If signing in keeps failing, see [Login and authentication](./code-troubleshoot-install.md#login-and-authentication)
...
* In an interactive session, run `/login`, choose **3rd-party platform**, then select **Claude Platform on AWS · refresh credentials** under **Using 3rd-party platforms** to run the same command without restarting Claude Code. See [Configure AWS credentials](./code-claude-platform-on-aws.md#1-configure-aws-credentials)
...
{/* min-version: 2.1.198 */}This message requires Claude Code v2.1.198 or later and only appears when [`awsAuthRefresh`](./code-amazon-bedrock.md#advanced-credential-configuration) is set in your settings file. Your AWS provider returned a 403, or [Amazon Bedrock](./code-amazon-bedrock.md) returned a 401.
...
* If your credentials are current, confirm the IAM permissions in [IAM configuration](./code-amazon-bedrock.md#iam-configuration) are attached to the identity you're using and that the selected model is enabled for your account and region
...
The AWS default credential provider chain didn't produce credentials within 60 seconds, so Claude Code stopped the resolve and failed the request. The failure is local credential resolution: the request never reached [Amazon Bedrock](./code-amazon-bedrock.md), [Claude Platform on AWS](./code-claude-platform-on-aws.md), or the [Mantle endpoint](./code-amazon-bedrock.md#use-the-mantle-endpoint). Claude Code clears its [credential cache](./code-amazon-bedrock.md#credential-caching-and-resolution-timeout) and retries before this error surfaces, so by the time you see it the chain has stalled on repeated attempts.
...
* If your chain runs an interactive sign-in that legitimately needs more than 60 seconds, such as SSO with MFA through a wrapper like `aws-vault`, raise the limit in milliseconds with [`CLAUDE_CODE_AWS_CHAIN_RESOLVE_TIMEOUT_MS`](./code-env-vars.md)
...
* If you are behind a corporate proxy, set `HTTPS_PROXY` before launching Claude Code and see [Network configuration](./code-network-config.md)
* If you route through an LLM gateway or relay, set [`ANTHROPIC_BASE_URL`](./code-env-vars.md) to its address. See [Connect Claude Code to an LLM gateway](./code-llm-gateway-connect.md) for setup.
...
A gateway or proxy between Claude Code and [Amazon Bedrock](./code-amazon-bedrock.md) is transforming the streaming response body or its `Content-Type` header. Amazon Bedrock streams responses as `application/vnd.amazon.eventstream`, and Claude Code rejects a successful streaming response that reports a different content-type instead of decoding a body it can't read. The request isn't retried.
...
* If the gateway rewrites only the header and passes the binary body through intact, set [`CLAUDE_CODE_DISABLE_BEDROCK_CONTENT_TYPE_GUARD=1`](./code-env-vars.md) to skip the check until the gateway is fixed. See [Streaming errors behind a gateway or proxy](./code-amazon-bedrock.md#streaming-errors-behind-a-gateway-or-proxy).
...
* Trim large `CLAUDE.md` memory files, or move instructions into [path-scoped rules](./code-memory.md#path-specific-rules) that load only when relevant
...
* Auto-compact is on by default and normally prevents this error. If you have set [`DISABLE_AUTO_COMPACT`](./code-env-vars.md), re-enable it or run `/compact` manually before the window fills.

See [Explore the context window](./code-context-window.md) for an interactive view of how context fills up.
...
* Configure your gateway to forward the `anthropic-beta` header. See [feature pass-through](./code-llm-gateway-protocol.md#feature-pass-through) for what gateways must forward.
* As a fallback, set [`CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1`](./code-env-vars.md) before launching. This disables features that require the beta header so requests succeed through a gateway that cannot forward it.
...
* **Non-interactive mode (`-p`)**: pass `--model` with a valid alias or ID, or set [`ANTHROPIC_MODEL`](./code-env-vars.md). The error text shows `Run --model` on this surface.
* **Agent SDK**: the error text omits the hint because the model is set programmatically. Set [`model` on `Options`](./code-agent-sdk/typescript.md#options) in TypeScript or [`ClaudeAgentOptions(model=...)`](./code-agent-sdk/python.md#claudeagentoptions) in Python, and handle the structured `model_not_found` error to surface your own retry or model picker.
* Use an alias such as `sonnet` or `opus` instead of a full versioned ID. Aliases resolve to a maintained default so they don't go stale. See [Model configuration](./code-model-config.md).
* If the wrong model keeps coming back in the CLI, a stale ID is set somewhere. Check in [priority order](./code-model-config.md#setting-your-model): the `--model` flag, the `ANTHROPIC_MODEL` environment variable, then the `model` field in `.claude/settings.local.json`, your project's `.claude/settings.json`, and `~/.claude/settings.json`. Remove the stale value and Claude Code falls back to your account default.
...
* For Google Cloud's Agent Platform deployments, see [Google Cloud's Agent Platform troubleshooting](./code-google-vertex-ai.md#troubleshooting).
...
Claude Code produces this error locally at the moment the switch is requested, before any API request is made. It applies when a model is set through the [Agent SDK](./code-agent-sdk/typescript.md) `setModel()` method or by an app such as the [Desktop app](./code-desktop.md) that runs the Claude Code CLI for you.
...
* The check runs only on the Anthropic API. On Amazon Bedrock, Google Cloud's Agent Platform, Microsoft Foundry, [Claude Platform on AWS](./code-claude-platform-on-aws.md), and behind an [LLM gateway](./code-llm-gateway.md) or a custom `ANTHROPIC_BASE_URL`, your provider or gateway defines the model names, so Claude Code accepts any string and passes it through.
...
Your organization admin has disabled this model in the claude.ai admin console, or it is excluded by an [`availableModels`](./code-model-config.md#restrict-model-selection) allowlist in managed settings. When the restricted model was set with `--model`, `ANTHROPIC_MODEL`, or the `model` setting, Claude Code substitutes an allowed model and continues. Typing `/model <name>` for a restricted model is rejected with `Run /model to choose a different model.` and the session keeps its current model.

```text theme={null}
Model "claude-opus-4-8" is restricted by your organization's settings. Using claude-sonnet-4-6 instead.
```

Claude Code treats a model family alias, one of `opus`, `sonnet`, `haiku`, or `fable`, as a request for that family rather than for its newest version. On the Anthropic API and on [Claude Platform on AWS](./code-claude-platform-on-aws.md), a restricted family alias resolves to the newest version of the family that your organization and the `availableModels` allowlist permit, and the substitution notice names that version. Claude Code rejects `/model <alias>` only when every version of the family is restricted. Before v2.1.205, a family alias was substituted or rejected based on its newest version alone, even when an older version of the same family was allowed.
...
* If you need access to the restricted model, ask your organization admin to enable it. See [Organization model restrictions](./code-model-config.md#organization-model-restrictions).
...
* {/* min-version: agent-sdk@0.3.197 */}If you hit this in the [Agent SDK](./code-agent-sdk/overview.md), upgrade the SDK package instead. Opus 4.8 needs TypeScript SDK v0.3.154 or later and Python SDK v0.2.88 or later. Sonnet 5 needs TypeScript SDK v0.3.197 or later
...
Claude Code adjusts these values automatically on the Anthropic API. You typically see this error on Amazon Bedrock or Google Cloud's Agent Platform when [`MAX_THINKING_TOKENS`](./code-env-vars.md) is set higher than the provider's output limit, or when plan mode raises the thinking budget.
...
* Run `/rewind`, or press Esc twice, to step back to a checkpoint before the corrupted turn and continue from there. See [Checkpointing](./code-checkpointing.md) for how checkpoints are created and restored.
...
The check evaluates the full conversation, not only your latest prompt, so sending a new message in the same session usually re-triggers the same refusal. The same applies after exiting and reopening the session with `--continue` or `--resume`, since the transcript on disk still contains the triggering content. On [Amazon Bedrock](./code-amazon-bedrock.md), [Google Cloud's Agent Platform](./code-google-vertex-ai.md), and [Microsoft Foundry](./code-microsoft-foundry.md), this message also covers requests the model's safety measures flagged as a cybersecurity topic. See [Safety measures flagged a cybersecurity topic](#safety-measures-flagged-a-cybersecurity-topic).
...
* On [Amazon Bedrock](./code-amazon-bedrock.md), [Google Cloud's Agent Platform](./code-google-vertex-ai.md), and [Microsoft Foundry](./code-microsoft-foundry.md), a cybersecurity flag produces the [Usage Policy refusal](#usage-policy-refusal) message instead.
* [Non-interactive mode](./code-headless.md) omits the `/feedback` sentence.
...
* To keep working in the same session, press Esc twice or run `/rewind` to step back to a checkpoint before the turn that triggered the flag, then take a different approach. See [Checkpointing](./code-checkpointing.md).

## Installation errors

These errors appear while installing or updating Claude Code, from the [install script](./code-setup.md#install-claude-code), `claude install`, or `claude update`. For `command not found`, PATH, permission, and TLS problems during setup, see [Troubleshoot installation and login](./code-troubleshoot-install.md).
...
* If your network requires a proxy, set `HTTPS_PROXY` before running the installer or `claude update`. See [Check network connectivity](./code-troubleshoot-install.md#check-network-connectivity).
* If a corporate proxy keeps closing the transfer, ask your network team to allow the full download from `downloads.claude.ai`. See [Network access requirements](./code-network-config.md#network-access-requirements).
...
This message requires Claude Code v2.1.198 or later. You combined `--bg` with `-p` or `--print` in the same `claude` invocation. `--bg` starts a [background session](./code-agent-view.md#from-your-shell) that you later attach to with `claude agents`, while `--print` runs [non-interactively](./code-headless.md) and never starts the interactive session that `claude agents` attaches to. Before v2.1.198 this combination silently created a background job that could never be attached to.
...
* Drop `-p` or `--print`. `--bg` takes the prompt as its positional argument, so `claude --bg "<task>"` is the complete command. See [Dispatch new agents from your shell](./code-agent-view.md#from-your-shell).
...
The schema you passed to [`--json-schema`](./code-cli-reference.md#cli-flags) in [non-interactive mode](./code-headless.md#get-structured-output) failed JSON Schema compilation, so `claude` exits with code 1 instead of running the prompt. Before v2.1.205, an invalid schema produced unstructured output with no error, and any schema that used the `format` keyword was treated as invalid.
...
* See [Get structured output](./code-headless.md#get-structured-output) for a working schema and command
...
* Add that server directly with `claude mcp add` or `claude mcp add-json` under a valid name. See [Import MCP servers from Claude Desktop](./code-mcp.md#import-mcp-servers-from-claude-desktop).

### MCP permission prompt tool not found

The tool you passed to [`--permission-prompt-tool`](./code-cli-reference.md#cli-flags) wasn't among the connected MCP tools when the run first needed a permission decision, either because its server never connected or because no connected server exposes a tool by that name. Claude Code still sends your prompt: the [non-interactive](./code-headless.md) run exits with this error, and exit code 1, on the first tool call that needs approval, so it produces no answer even though the request was made. Before the first prompt, Claude Code waits up to the per-server connection timeout of 30 seconds set by [`MCP_TIMEOUT`](./code-env-vars.md) for that server to connect. {/* min-version: 2.1.206 */}Before v2.1.206, startup didn't wait for the server to finish connecting, so a slow-starting but healthy server produced this error too.
...
* If the server needs longer than 30 seconds to start, raise [`MCP_TIMEOUT`](./code-env-vars.md)

## Plugin errors

These errors come from [plugin](./code-plugins.md) and [marketplace](./code-plugin-marketplaces.md) configuration. For plugin problems that don't produce one of the messages on this page, such as a marketplace URL that doesn't load or a plugin that installs but doesn't appear, see [Plugin troubleshooting](./code-discover-plugins.md#troubleshooting).

### Marketplace is registered from an untrusted source

The marketplace is registered under a name that is [reserved for official Anthropic marketplaces](./code-plugin-marketplaces.md#marketplace-schema), but its registered source isn't an `anthropics` GitHub repository. Claude Code re-checks reserved names every time it loads or refreshes a marketplace, so the marketplace and the plugins installed from it stop loading. Before v2.1.205, the name was checked only when the marketplace was added, so an entry registered before its name became reserved kept loading.
...
* See the reserved name list under [Marketplace schema](./code-plugin-marketplaces.md#marketplace-schema)

<h3 id="plugin-command-references-user-config">
  Plugin command references user\_config in a shell command
</h3>

A plugin hook, [monitor](./code-plugins-reference.md#monitors), or MCP [`headersHelper`](./code-mcp.md#use-dynamic-headers-for-custom-authentication) command references a `${user_config.KEY}` [plugin option](./code-plugins-reference.md#user-configuration), and the substituted string would be passed to a shell. A configured value containing `$(...)`, backticks, or `;` would run as code there, so Claude Code refuses to start the component instead of substituting the value. The check runs on the command template, so the error appears even when no value is configured yet. Before v2.1.207, the value was substituted into the shell command.
...
* For a hook, add an `args` array so it runs in [exec form](./code-hooks.md#exec-form-and-shell-form), where each `${user_config.KEY}` becomes one argument with no shell in between. Or drop the reference and read the `$CLAUDE_PLUGIN_OPTION_<KEY>` environment variable inside the script
...
Nothing in a [subagent's `tools` list](./code-sub-agents.md#supported-frontmatter-fields) resolved to a tool, so Claude Code refuses to launch the subagent rather than start one that can't act. The message groups the entries by why they didn't resolve: not a recognized tool, a tool that isn't available to subagents, or recognized but matching no tool in the current session. Omitting the `tools` field never triggers this refusal. An MCP server pattern such as `mcp__github__*` isn't exempt: when no connected tool comes from that server, the launch is refused with the pattern in the matched-nothing group. Before v2.1.208, the subagent launched with no tools and returned an empty or confusing result.
...
* Correct each entry the error names against the [tools available to subagents](./code-sub-agents.md#available-tools)
...
* If Claude should be able to edit the file, remove or narrow the `Read` deny rule in `/permissions` or in [settings](./code-settings.md#permission-settings)
...
Claude wrote to the [auto memory](./code-memory.md#auto-memory) index `MEMORY.md` and left it over one of its read limits: 200 lines or 25KB. The write succeeded, but only the first 200 lines or 25KB, whichever comes first, load at the start of a session, so everything past the limit is dropped each time the index is read. Before v2.1.210, an over-limit index was silently truncated on the next load with no write-time signal.
...
* To trim the index yourself, see [Audit and edit your memory](./code-memory.md#audit-and-edit-your-memory)

## Background session errors

[Background sessions](./code-agent-view.md) run without an interactive terminal of their own, so commands that need one behave differently there. These messages appear in the transcript of a background session, in the terminal that attaches to one, or in the session or shell you dispatch from; where a message is specific to one surface, its entry says so.
...
[`CLAUDE_CODE_PROCESS_WRAPPER`](./code-corporate-launcher.md) is set, and its value can't be used, so Claude Code refuses to start the affected process rather than run it without the launcher. Configuration problems are reported with a message that starts with the variable name and states the reason, for example:
...
* Set the variable to the absolute path of an executable that ends by calling `exec "$@"`. See [the launcher contract](./code-corporate-launcher.md#the-launcher-contract) for the full contract
...
* After fixing the value in the `env` block of [settings](./code-corporate-launcher.md#set-up-the-launcher), restart the background service with `claude daemon stop --any` so the next dispatch starts a wrapped one

### EUNKNOWN when starting a background session

Windows refused to start a program with an error code that has no standard name, so the failure surfaces as `EUNKNOWN`. The usual trigger is a software restriction policy, such as Group Policy or AppLocker, blocking the program being started. The error appears when you start a [background session](./code-agent-view.md) with `/background` or `claude --bg`:
...
Claude Code found `permissions.allow` rules or `permissions.additionalDirectories` entries in the project's `.claude/settings.json` or `.claude/settings.local.json` and didn't apply them, because [allow rules from project settings require workspace trust](./code-permissions.md#project-allow-rules-and-workspace-trust). The count, the setting name, and the file named in the message vary with your configuration. `deny` and `ask` rules aren't affected.
...
* **Effort level**: run `/effort` to check the current reasoning level and raise it for hard debugging or design work. Defaults vary by model, so check before assuming you are below the maximum. See [Adjust effort level](./code-model-config.md#adjust-effort-level) for per-model defaults and the `ultrathink` shortcut.
* **Context pressure**: run `/context` to see how full the window is. If it is near capacity, run `/compact` at a natural breakpoint or `/clear` to start fresh. See [Explore the context window](./code-context-window.md) for how auto-compact affects earlier turns.
...
When a response goes wrong, rewinding usually works better than replying with corrections. Press Esc twice or run `/rewind` to step back to before the bad turn, then rephrase the prompt with more specifics. Correcting in-thread keeps the wrong attempt in context, which can anchor later answers to it. See [Checkpointing](./code-checkpointing.md).
...
* Run `/feedback` inside Claude Code to send the transcript and a description to Anthropic. The command also offers to open a prefilled GitHub issue. Sending to Anthropic requires [authentication](./code-authentication.md). On Amazon Bedrock, Google Cloud's Agent Platform, Microsoft Foundry, and other third-party providers, or when no Anthropic credentials are configured, `/feedback` saves a local archive you can send to your Anthropic account representative instead.
