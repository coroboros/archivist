...
When a rate limit, overload, or server error interrupts a foreground subagent that already produced output, Claude receives that partial output marked as incomplete instead of this error. See [API errors in subagents](./code-sub-agents.md#api-errors-in-subagents).
...
This appears on Amazon Bedrock, Google Vertex AI, and Microsoft Foundry. {/* min-version: 2.1.196 */}As of v2.1.196 it also appears when [`ANTHROPIC_BASE_URL`](./code-env-vars.md) points at a host other than `api.anthropic.com`, such as an [LLM gateway](./code-llm-gateway.md) or proxy, even when you sign in with claude.ai.
...
Claude Code adjusts these values automatically on the Anthropic API. You typically see this error on Amazon Bedrock or Google Vertex AI when [`MAX_THINKING_TOKENS`](./code-env-vars.md) is set higher than the provider's output limit, or when plan mode raises the thinking budget.
