...
The [desktop app](./code-desktop.md#work-in-parallel-with-sessions), [Claude Code on the web](./code-claude-code-on-the-web.md), and the [VS Code extension](./code-vs-code.md#resume-past-conversations) each maintain their own session history. This page covers the CLI.
...
* Model: the session continues on the model it was using. The model isn't restored when it has been retired or isn't allowed by `availableModels`, when a `--model` flag or `ANTHROPIC_MODEL`-family environment variable picks one at launch, or on providers that use provider-specific deployment IDs, such as [Amazon Bedrock, Google Cloud's Agent Platform, and Microsoft Foundry](./code-third-party-integrations.md); see [model configuration](./code-model-config.md#setting-your-model) for the resolution order.
* Permission mode: the mode the session was in. `plan` and `bypassPermissions` are never restored; [bypassing permissions](./code-permission-modes.md#skip-all-checks-with-bypasspermissions-mode) must be enabled again at launch, with one of its launch flags or `permissions.defaultMode: "bypassPermissions"` in [settings](./code-settings.md#permission-settings). `auto` is restored only when your account still meets the [auto mode requirements](./code-permission-modes.md#eliminate-prompts-with-auto-mode). Pass `--permission-mode` to override the restored mode.
* Active goal: a [goal](./code-goal.md#resume-with-an-active-goal) that was still active when the session ended carries over; its turn count, timer, and token-spend baseline reset.
* Scheduled tasks: [tasks that haven't expired](./code-scheduled-tasks.md#limitations) are restored. Background Bash and monitor tasks aren't.
...
| On plan accept          | Accepting a plan in [plan mode](./code-permission-modes.md#analyze-before-you-edit-with-plan-mode) names the session from the plan content unless you've already set one |
...
If you don't name a session, Claude Code generates a session title for it: a short summary of your first prompt, written by a background request to the small/fast model, normally a Haiku-class model. Naming the session with `--name` or `/rename` replaces the generated title. You see the generated title in the [session picker](#use-the-session-picker) and in the statusline [`session_name`](./code-statusline.md) field when no name is set; like the default display name, it isn't a resume handle.
...
If you omit the name, Claude Code names the new branch after the first prompt in the conversation. As of v2.1.198 this also applies after [compaction](./code-how-claude-code-works.md#when-context-fills-up); earlier versions fell back to the literal name `Branched conversation` instead of looking past the compaction summary to the original first prompt.
...
For checkpoint-based rewind within a single session, see [Checkpointing](./code-checkpointing.md).

## Manage context within a session

These commands control what's in the context window without leaving the session:

* **`/clear`**: start fresh with an empty context. Claude Code saves the previous conversation; resume it with `/resume`, or, in the same Claude Code process, {/* min-version: 2.1.191 */}from [the rewind menu's previous-session entry](./code-checkpointing.md#rewind-past-a-cleared-conversation). You keep a name you set with `--name` or `/rename` in the new conversation, but not an AI-generated session title
...
For how compaction interacts with CLAUDE.md, skills, and rules, see the [context window guide](./code-context-window.md). For strategies on when to clear versus compact, see [Best practices](./code-best-practices.md#manage-your-session).
...
* **Run Claude once and capture the result**: invoke `claude -p` with [`--output-format json` or `stream-json`](./code-headless.md#get-structured-output) to capture the result, session ID, usage, and cost of a non-interactive run as structured JSON.
* **Ask an existing session a question**: pass a session ID to [`claude -p --resume`](./code-headless.md#continue-conversations) to send a follow-up prompt, such as a summary request, and capture the structured response.
* **React to session events**: read the `transcript_path` field that [hooks](./code-hooks.md#common-input-fields) and [status line commands](./code-statusline.md#available-data) receive as input. A `SessionEnd` hook can archive the transcript when a session ends.
* **Embed Claude in a TypeScript or Python app**: use the [Agent SDK](./code-agent-sdk/overview.md) to receive each message programmatically.
...
| To                                          | Set                                                    | Where                     |
| ------------------------------------------- | ------------------------------------------------------ | ------------------------- |
| Move storage off `~/.claude`                | [`CLAUDE_CONFIG_DIR`](./code-env-vars.md)                    | Environment variable      |
| Change the 30-day retention                 | [`cleanupPeriodDays`](./code-settings.md#available-settings) | `settings.json`           |
| Suppress transcript writes in all modes     | [`CLAUDE_CODE_SKIP_PROMPT_HISTORY`](./code-env-vars.md)      | Environment variable      |
| Suppress writes for one non-interactive run | [`--no-session-persistence`](./code-cli-reference.md)        | CLI flag with `claude -p` |

## See also

These pages cover related session and parallelism mechanics:

* [Worktrees](./code-worktrees.md): run isolated parallel sessions on separate branches
* [Checkpointing](./code-checkpointing.md): rewind code and conversation to an earlier point
* [Context window](./code-context-window.md): what fills context and what survives compaction
* [Non-interactive mode](./code-headless.md): session behavior under `claude -p`
