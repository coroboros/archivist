...
Sessions created with [`claude -p`](./code-headless.md) or the [Agent SDK](./code-agent-sdk/overview.md) do not appear in the session picker, but you can still resume one by passing its session ID to `claude --resume <session-id>`.
...
Transcripts are stored as JSONL at `~/.claude/projects/<project>/<session-id>.jsonl`, where `<project>` is derived from your working directory path. Each line is a JSON object for a message, tool use, or metadata entry. To store sessions somewhere other than `~/.claude`, set [`CLAUDE_CONFIG_DIR`](./code-env-vars.md). These local files are removed after 30 days by default; change this with [`cleanupPeriodDays`](./code-settings.md#available-settings).

To suppress transcript writes entirely, set [`CLAUDE_CODE_SKIP_PROMPT_HISTORY`](./code-env-vars.md), or in non-interactive mode use `--no-session-persistence`.
