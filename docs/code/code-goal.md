...
| [`/loop`](./code-scheduled-tasks.md#run-a-prompt-repeatedly-with-%2Floop) | A time interval elapses    | You stop it, or Claude decides the work is done |
| [Stop hook](./code-hooks-guide.md#prompt-based-hooks)                     | The previous turn finishes | Your own script or prompt decides               |
...
<Tip>
  The approaches above keep the current session running. You can also schedule work that runs independent of any open session, such as nightly tests or morning triage. See [scheduling options](./code-scheduled-tasks.md#compare-scheduling-options) for cloud routines and desktop scheduled tasks.
</Tip>
...
A goal doesn't change permissions. In the default permission mode, Claude still asks before tool calls that your settings don't already allow, such as the test command above. To let goal turns run unattended, pair `/goal` with [auto mode](./code-auto-mode-config.md).
...
`/goal` is a wrapper around a session-scoped [prompt-based Stop hook](./code-hooks.md#prompt-based-hooks). Each time Claude finishes a turn, the condition and the conversation so far are sent to your configured [small fast model](./code-model-config.md), which defaults to Haiku. The model returns a yes-or-no decision and a short reason. A "no" tells Claude to keep working and includes the reason as guidance for the next turn. A "yes" clears the goal and records an achieved entry in the transcript.
...
`/goal` runs only in workspaces where you have accepted the trust dialog, because the evaluator is part of the hooks system. `/goal` is also unavailable when [`disableAllHooks`](./code-hooks.md#disable-or-remove-hooks) is set at any settings level or when [`allowManagedHooksOnly`](./code-settings.md#hook-configuration) is set in managed settings. In each case, the command tells you why instead of silently doing nothing.

## See also

* [Run a prompt repeatedly with `/loop`](./code-scheduled-tasks.md#run-a-prompt-repeatedly-with-%2Floop): re-run on a time interval instead of until a condition holds
* [Prompt-based hooks](./code-hooks-guide.md#prompt-based-hooks): write your own Stop hook when you need custom evaluation logic
* [Auto mode](./code-auto-mode-config.md): approve tool calls automatically so each goal turn runs unattended
* [Scheduling comparison](./code-scheduled-tasks.md#compare-scheduling-options): run work on a schedule independent of any open session
