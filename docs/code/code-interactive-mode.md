...
| `Shift+Tab` or `Alt+M` (some configurations)              | Cycle permission modes                                                                                                                                     | Cycle through `default` (labeled Manual in the mode indicator), `acceptEdits`, `plan`, and any modes you have enabled, such as `auto` or `bypassPermissions`. See [permission modes](./code-permission-modes.md).                                                                                                                        |
...
When the transcript viewer is open (toggled with `Ctrl+O`), these shortcuts are available. In [fullscreen rendering](./code-fullscreen.md), press `?` to show the full shortcut reference panel inside the viewer. `Ctrl+E` can be rebound via [`transcript:toggleShowAll`](./code-keybindings.md).
...
In print mode they are off by default. Pass [`--prompt-suggestions`](./code-cli-reference.md#cli-flags) with `--output-format stream-json --verbose` to emit a `prompt_suggestion` message after each turn instead.
