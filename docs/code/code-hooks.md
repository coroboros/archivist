...
| [`UserPromptExpansion`](./code-hooks.md#userpromptexpansion) | No         | Yes            | A user-typed command, or an MCP prompt, expands into a prompt before it reaches Claude. Doesn't fire when Claude invokes a skill itself | Block a command from direct invocation or add context when a skill is typed |
...
Use matchers to filter when your callbacks fire. The `matcher` field matches against a different value depending on the hook event type. For example, tool-based hooks match against the tool name, while `Notification` hooks match against the notification type. See the [Claude Code hooks reference](./code-hooks.md#matcher-patterns) for the full list of matcher values for each event type.

SDK matchers follow the same rules as [matchers in settings files](./code-hooks.md#matcher-patterns). A matcher containing only letters, digits, `_`, `-`, spaces, `,`, and `|` is compared as an exact string, with alternatives separated by `|` or `,` and optional surrounding whitespace, so `Write|Edit` and `Write, Edit` each match exactly those two tools and `code-reviewer` matches only that agent type. A matcher of `*`, an empty string, or omitting the matcher entirely matches every occurrence of the event.
...
<Tip>
  **Discovering tool names:** See [Tool Input Types](./code-agent-sdk/typescript.md#tool-input-types) for the full list of built-in tool names, or add a hook without a matcher to log all tool calls your session makes.
...
* **`hookSpecificOutput`** controls the current operation. The fields inside depend on the hook event type. For `PreToolUse` hooks, this is where you set `permissionDecision` (`"allow"`, `"deny"`, `"ask"`, or `"defer"`), `permissionDecisionReason`, and `updatedInput`. Returning `"defer"` ends the query so you can [resume it later](./code-hooks.md#defer-a-tool-call-for-later). For `PostToolUse` hooks, you can set `additionalContext` to append information to the tool result. To replace the tool's output before Claude sees it, set `updatedToolOutput`, which works for any tool in both SDKs. The older `updatedMCPToolOutput` field replaces MCP tool output only and is deprecated.

Return `{}` to allow the operation without changes. SDK callback hooks use the same JSON output format as [Claude Code shell command hooks](./code-hooks.md#json-output), which documents every field and event-specific option. For the SDK type definitions, see the [TypeScript](./code-agent-sdk/typescript.md#synchookjsonoutput) and [Python](./code-agent-sdk/python.md#synchookjsonoutput) SDK references.
...
Use `SubagentStop` hooks to monitor when subagents finish their work. See the full input type in the [TypeScript](./code-agent-sdk/typescript.md#hookinput) and [Python](./code-agent-sdk/python.md#hookinput) SDK references. This example logs a summary each time a subagent completes:
...
* For non-tool hooks that support matchers, like `Notification` and `SubagentStop`, matchers match against different fields, and `Stop` ignores matchers entirely (see [matcher patterns](./code-hooks.md#matcher-patterns))
* Hooks may not fire when the agent hits the [`max_turns`](./code-agent-sdk/python.md#claudeagentoptions) limit because the session ends before hooks can execute
...
`SessionStart` and `SessionEnd` can be registered as SDK callback hooks in TypeScript, but aren't available in the Python SDK because its `HookEvent` type omits them. In Python, they are only available as [shell command hooks](./code-hooks.md#hook-events) defined in settings files such as `.claude/settings.json`. To load shell command hooks from your SDK application, include the appropriate setting source with [`setting_sources`](./code-agent-sdk/python.md#settingsource) or [`settingSources`](./code-agent-sdk/typescript.md#settingsource):
...
The `systemMessage` field shows a message to the user, not the model. By default the SDK surfaces hook output in the message stream only for `SessionStart` and `Setup` hooks, so a message from any other hook event doesn't appear unless you set `includeHookEvents` (`include_hook_events` in Python). To pass context to the model instead, return [`additionalContext`](./code-hooks.md#add-context-for-claude).

If you need to surface hook decisions to your application reliably, log them separately or use a dedicated output channel.

## Related resources

* [Claude Code hooks reference](./code-hooks.md): full JSON input/output schemas, event documentation, and matcher patterns
* [Claude Code hooks guide](./code-hooks-guide.md): shell command hook examples and walkthroughs
* [TypeScript SDK reference](./code-agent-sdk/typescript.md): hook types, input/output definitions, and configuration options
* [Python SDK reference](./code-agent-sdk/python.md): hook types, input/output definitions, and configuration options
* [Permissions](./code-agent-sdk/permissions.md): control what your agent can do
* [Custom tools](./code-agent-sdk/custom-tools.md): build tools to extend agent capabilities
