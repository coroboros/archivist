...
One exception applies: a small set of [cross-source lock keys](./code-settings.md#settings-precedence), such as the sandbox allowlist locks, is honored when any admin-controlled managed source sets them; the user-writable HKCU registry tier is excluded.
