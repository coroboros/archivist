...
[Subagents](./code-sub-agents.md), [agent view](./code-agent-view.md), [agent teams](./code-agent-teams.md), and [dynamic workflows](./code-workflows.md) each parallelize work in a different way. The right one depends on whether you want to stay in each conversation yourself, hand tasks off and check back later, or have Claude coordinate a group of workers for you.
...
* [Worktrees](./code-worktrees.md) give each session a separate git checkout, so parallel sessions never edit the same files. Use them for sessions you run yourself. Agent view moves each dispatched session into its own worktree automatically, and subagents you spawn can each get one too.
* [`/batch`](./code-commands.md) is a [skill](./code-skills.md) that has Claude split one large change into 5 to 30 worktree-isolated subagents that each open a pull request. It's a packaged use of subagents and worktrees, not a separate coordination style.
...
* A [background bash command](./code-interactive-mode.md#background-bash-commands) runs one shell command without blocking the conversation. It doesn't spawn an agent.
* A [forked subagent](./code-sub-agents.md#fork-the-current-conversation), started with `/subtask`, is a subagent that inherits your full conversation context instead of starting fresh. It's a way to spawn a subagent, not a separate surface. To copy the whole session into a new [background session](./code-agent-view.md#from-inside-a-session) that runs alongside it, use `/fork`. With [agent view turned off](./code-agent-view.md#turn-off-agent-view), the forked-subagent command is `/fork` instead and `/subtask` isn't available.
* A [routine](./code-routines.md) runs a session on a schedule in Anthropic's cloud, not in parallel on your machine.
...
* **Who coordinates the work?**
  * Claude delegates and collects results inside one conversation: [subagents](./code-sub-agents.md)
  * You hand off independent tasks and check back later: [agent view](./code-agent-view.md)
  * Claude plans, assigns, and supervises a group of workers: [agent teams](./code-agent-teams.md), experimental and disabled by default
  * A script holds the plan instead of Claude's turn-by-turn judgment: [dynamic workflows](./code-workflows.md). See [how workflows compare to subagents and skills](./code-workflows.md#when-to-use-a-workflow)
...
* [Orchestrate dynamic workflows](./code-workflows.md): run a bundled workflow or have Claude write one that runs many subagents and verifies their findings against each other.
