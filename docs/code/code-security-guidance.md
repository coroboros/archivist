...
In a Claude Code session, install from the [official Anthropic marketplace](./code-discover-plugins.md#official-anthropic-marketplace):
...
User-scoped plugins do not carry into [Claude Code on the web](./code-claude-code-on-the-web.md), because those sessions run on Anthropic infrastructure rather than your machine. To enable the plugin there, or to turn it on for everyone who clones a repository, declare it in the project's checked-in settings:
...
Administrators can enable the plugin organization-wide by setting [`enabledPlugins`](./code-settings.md#plugin-settings) in [managed settings](./code-admin-setup.md).
...
If the plugin was enabled through a project's `.claude/settings.json`, disabling it from `/plugin` writes an override to your `.claude/settings.local.json` rather than editing the checked-in file, so the plugin stays off for you while teammates are unaffected. If it was enabled through [managed settings](./code-admin-setup.md), only an administrator can disable it.

## How the plugin integrates with Claude Code

The plugin is built entirely on [hooks](./code-hooks.md), the mechanism for running your own code at specific points in Claude's loop. It registers:
...
| On demand       | [`/security-review`](./code-commands.md#all-commands)           | One-time security pass on the current branch, run when you ask                                   |
| On pull request | [Code Review](./code-code-review.md), Team and Enterprise plans | Multi-agent correctness and security review with full codebase context                           |
