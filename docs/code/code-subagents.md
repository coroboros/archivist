...
* **Programmatically**: use the `agents` parameter in your `query()` options. See the [TypeScript](./code-agent-sdk/typescript.md#agentdefinition) and [Python](./code-agent-sdk/python.md#agentdefinition) references
* **Filesystem-based**: define agents as markdown files in `.claude/agents/` directories. See [defining subagents as files](./code-sub-agents.md)
...
In the Python SDK, multi-word field names such as `disallowedTools` and `mcpServers` keep their camelCase spelling to match the wire format rather than following Python's snake\_case convention. See the [`AgentDefinition` reference](./code-agent-sdk/python.md#agentdefinition) for details.

Two subagent behaviors changed in Claude Code v2.1.198:

* Subagents run in the background by default. An Agent tool call that omits the [`run_in_background`](./code-agent-sdk/typescript.md) input launches a background subagent, and Claude sets `run_in_background: false` when it needs the result before continuing. Before v2.1.198, omitting `run_in_background` ran the subagent synchronously. Set the `background` field to `true` to force background execution for a specific agent regardless of what Claude requests.
...
<Note>
  {/* min-version: 2.1.172 */}As of Claude Code v2.1.172, subagents can spawn their own subagents. A subagent five levels below the main agent can't spawn further subagents, regardless of whether it runs in the foreground or background. To prevent a subagent from spawning others, omit `Agent` from its `tools` array or add it to `disallowedTools`. See [nested subagents](./code-sub-agents.md#spawn-nested-subagents) for the full depth rules.
</Note>

### Filesystem-based definition (alternative)

You can also define subagents as markdown files in `.claude/agents/` directories. See the [Claude Code subagents documentation](./code-sub-agents.md) for details on this approach. Programmatically defined agents take precedence over filesystem-based agents with the same name.
...
{/* min-version: 2.1.206 */}A subagent that has the [`SendMessage`](./code-tools-reference.md) tool starts with a list of the other named agents running in the session, so it knows which names it can send messages to. Claude Code adds the list to the subagent's first turn automatically. A [fork](./code-sub-agents.md#fork-the-current-conversation) doesn't get the list because it inherits the parent conversation instead. The list requires Claude Code v2.1.206 or later.
...
| Project CLAUDE.md (loaded via [`settingSources`](./code-agent-sdk/claude-code-features.md#control-filesystem-settings-with-settingsources)) | Preloaded skill content, unless listed in `AgentDefinition.skills` |
...
<Note>
  ...
  {/* min-version: 2.1.210 */}In v2.1.210 and later, Claude Code [scans the final message for instruction-shaped patterns](./code-sub-agents.md#subagent-output-scanning) before the parent reads it. The scan treats three kinds of pattern differently:
...
Subagents work well for a few delegated tasks per turn. For runs that coordinate dozens to hundreds of agents, use the `Workflow` tool, which moves the orchestration into a script the runtime executes outside the conversation context. See [dynamic workflows](./code-workflows.md) for how workflows differ from turn-by-turn subagent delegation.

The `Workflow` tool is available in the TypeScript Agent SDK v0.3.149 and later. Include `Workflow` in `allowedTools` to auto-approve workflow runs. The tool input and output schemas are listed in the [TypeScript reference](./code-agent-sdk/typescript.md#workflow).
...
For the file format, see [how to write subagent files](./code-sub-agents.md#write-subagent-files).
...
* [Claude Code subagents](./code-sub-agents.md): comprehensive subagent documentation including filesystem-based definitions
* [Dynamic workflows](./code-workflows.md): orchestrate many subagents from a script for jobs too large for one conversation
* [SDK overview](./code-agent-sdk/overview.md): getting started with the Claude Agent SDK
