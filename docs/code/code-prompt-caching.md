...
* **Effort level**: not part of the cache key or the prompt, so [changing it](./code-model-config.md#adjust-effort-level) mid-session has no effect on the cache.
...
Tool definitions sit in the system prompt layer, so the cache invalidates when the set of MCP tools available to Claude changes between turns. The most common cause is an [MCP server](./code-mcp.md) connecting or disconnecting mid-session, which can happen without any action on your part: a stdio server's process exits, an HTTP session expires, or a server [reconnects automatically after a transient failure](./code-mcp.md#automatic-reconnection). A connected server can also push a [dynamic tool update](./code-mcp.md#dynamic-tool-updates) that changes its tool list.
...
[MCP tool search](./code-mcp.md#scale-with-mcp-tool-search) reduces how much each tool contributes to the prefix by deferring full tool definitions, but the set of tool names still has to stay stable for the cache to remain valid.
