...
An [LLM gateway](./code-llm-gateway.md) is a proxy your organization runs between Claude Code and the model provider. When your organization uses one, Claude Code authenticates to the gateway with a credential your organization issues instead of your personal claude.ai login.
...
<Note>
  * To deploy a gateway for your organization, see [Roll out an LLM gateway](./code-llm-gateway-rollout.md)
  * For what Claude Code sends to a gateway, see the [gateway protocol reference](./code-llm-gateway-protocol.md)
</Note>

## Check for an existing configuration

Administrators can distribute the gateway address and credential through [managed settings](./code-settings.md#settings-files), device management, or an [`apiKeyHelper`](#rotate-credentials-with-apikeyhelper), so Claude Code picks them up at startup with nothing for you to set. To check whether your organization already did this:
...
If you export the gateway only in your shell, it doesn't reliably reach background agents hosted by the [supervisor](./code-agent-view.md#how-background-sessions-are-hosted); see [how each background session sources its gateway](./code-agent-view.md#the-supervisor-process). Use a settings file for any gateway that background agents must always route through.

#### Set in a settings file

To make the configuration apply everywhere Claude Code runs, including [background agents](./code-agent-view.md#how-background-sessions-are-hosted), set the variables in the `env` block of a [settings file](./code-settings.md) instead of relying on your shell. Settings files have different scopes:
...
* **Distributed by an administrator**: if your organization has [deployed the configuration](./code-llm-gateway-rollout.md#distribute-through-managed-settings), the desktop app routes through the gateway with no setup on your part
...
With the gateway configuration active, the desktop app runs sessions on your local machine only: the environment picker doesn't offer SSH sessions or Anthropic-hosted cloud environments, and [Remote Control](./code-remote-control.md) is unavailable. To use Claude Code on a remote host through the gateway, run the CLI on that host with [`ANTHROPIC_BASE_URL` and the gateway credential](#set-the-base-url-and-credential) set there.
...
[Claude Code GitHub Actions](./code-github-actions.md) reads `ANTHROPIC_BASE_URL` and `ANTHROPIC_CUSTOM_HEADERS` from the workflow's `env` block. Pass the credential as the action's `anthropic_api_key` input; the action sets it as `ANTHROPIC_API_KEY`, so it reaches the gateway in the `x-api-key` header.
...
For the action's other authentication options, including `CLAUDE_CODE_OAUTH_TOKEN` and workload identity federation, see [Claude Code GitHub Actions](./code-github-actions.md) and the action's [README](https://github.com/anthropics/claude-code-action#readme).

### Agent SDK

The [Agent SDK](./code-agent-sdk/overview.md) has no gateway-specific options; it passes environment variables to the Claude Code process it spawns. Each SDK accepts an `env` option that sets the spawned process's environment, and the TypeScript and Python SDKs treat it differently:
...
[Claude Code in Slack](./code-slack.md) and [Claude Code on the web](./code-claude-code-on-the-web.md) are Anthropic-hosted products that always use Anthropic's API; they aren't part of a gateway deployment. Gateway variables set in a cloud session's environment configuration are not applied. If your traffic must stay on the gateway, don't enable these surfaces for those users.

[Remote Control](./code-remote-control.md) and [voice dictation](./code-voice-dictation.md) both rely on a claude.ai identity: Remote Control to pair a live session with your account, and voice dictation to reach the claude.ai transcription endpoint. They are unavailable while `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, or an `apiKeyHelper` is active. {/* min-version: 2.1.196 */}As of v2.1.196, Remote Control is also disabled while `ANTHROPIC_BASE_URL` points at a non-Anthropic host, so signing in with claude.ai isn't enough on its own.
...
Some gateways route or tag requests using a custom header in addition to the credential, for example a tenant identifier or a routing key. To send one, set [`ANTHROPIC_CUSTOM_HEADERS`](./code-env-vars.md) with one `Name: Value` pair per line. The example below adds a routing header named `X-Org-Route`:
...
The helper is any shell command that prints the current credential to stdout. Claude Code runs it through your system shell, so on Windows it can be an executable or a PowerShell invocation. Write the script, make it executable, and reference it from `apiKeyHelper` in your [settings file](./code-settings.md):
...
* It suppresses the [fast mode](./code-fast-mode.md) availability check. Unless a previous check already enabled fast mode on the machine, `/fast` reports that fast mode is unavailable.
...
* The WebFetch tool's [domain safety check](./code-data-usage.md#webfetch-domain-safety-check) isn't affected and still calls `api.anthropic.com`. Turn it off separately with `skipWebFetchPreflight: true` in [settings](./code-settings.md) if your network blocks that host.
* For each telemetry stream and the variable that controls it, see [telemetry services](./code-data-usage.md#telemetry-services).
...
See [Claude Platform on AWS](./code-claude-platform-on-aws.md) for the workspace ID.
...
| `Your apiKeyHelper script is failing`                                                                                                                                                                                                                                                                           | The command in the [`apiKeyHelper`](./code-settings.md#available-settings) setting exited with an error, timed out, or printed nothing, so requests carry a placeholder key                                                                                                                                                                     | Run the command directly to see why it fails, and re-authenticate with your credential provider if it reports an expired session; see [the error reference](./code-errors.md#your-apikeyhelper-script-is-failing)                                                                                                                                                                              |
...
| Models missing from the `/model` picker                                                                                                                                                                                                                                                                         | Gateway model names aren't in Claude Code's built-in list                                                                                                                                                                                                                                                                                 | Enable [gateway model discovery](#add-gateway-models-to-the-model-picker) or add names with the [model configuration](./code-model-config.md) variables                                                                                                                                                                                                                                        |
| `/fast` reports `Fast mode unavailable due to network connectivity issues` while inference requests work                                                                                                                                                                                                        | The [fast mode](./code-fast-mode.md) availability check goes directly to `api.anthropic.com` and doesn't follow `ANTHROPIC_BASE_URL`, so blocked direct egress fails the check. The same message appears on an open network when the check presents a gateway-issued key from `ANTHROPIC_API_KEY` or an `apiKeyHelper` and Anthropic rejects it | Allowlist `api.anthropic.com` if egress is blocked, or set a skip variable; for a rejected gateway key only the skip variables help. See [use fast mode behind proxies and LLM gateways](./code-fast-mode.md#use-fast-mode-behind-proxies-and-llm-gateways)                                                                                                                                    |
| `/fast` reports `Fast mode has been disabled by your organization` in a session authenticated with `ANTHROPIC_AUTH_TOKEN`, even though the organization has fast mode enabled                                                                                                                                   | The availability check requires a claude.ai login or an Anthropic API key; with only a bearer token, Claude Code treats fast mode as disabled without sending the check                                                                                                                                                                   | Set `CLAUDE_CODE_SKIP_FAST_MODE_ORG_CHECK=1`; see [use fast mode behind proxies and LLM gateways](./code-fast-mode.md#use-fast-mode-behind-proxies-and-llm-gateways)                                                                                                                                                                                                                           |
...
| Certificate or TLS errors such as `SSL certificate verification failed` or `Self-signed certificate detected`, when the [curl test](#verify-the-connection) succeeds                                                                                                                                            | Claude Code's runtime isn't trusting the same certificate authority that `curl` uses. Common behind corporate TLS-inspection proxies                                                                                                                                                                                                      | Set `NODE_EXTRA_CA_CERTS` to the CA bundle path; see [CA certificate store](./code-network-config.md#ca-certificate-store)                                                                                                                                                                                                                                                                     |

If Claude Code prompts you to log in repeatedly after removing gateway configuration, the cause is usually credential storage rather than the gateway; see [authentication errors](./code-errors.md#authentication-errors).

## Related resources

* [LLM gateways overview](./code-llm-gateway.md): what a gateway is and how it interacts with claude.ai subscriptions
* [Roll out an LLM gateway for your organization](./code-llm-gateway-rollout.md): the admin-facing checklist for deploying and distributing gateway configuration
* [Gateway protocol reference](./code-llm-gateway-protocol.md): what Claude Code sends to a gateway, including the headers and fields the gateway must forward
* [Settings](./code-settings.md): where settings files live and how the `env` block is read
* [Authentication](./code-authentication.md): how credential variables, `apiKeyHelper`, and OAuth login interact
