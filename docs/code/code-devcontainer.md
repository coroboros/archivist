...
<Warning>
  ...
  When executed with `--dangerously-skip-permissions`, dev containers do not prevent a malicious project from exfiltrating anything accessible inside the container, including the Claude Code credentials stored in [`~/.claude`](./code-claude-directory.md).
...
<Accordion title="How dev containers work with your editor">
  ...
  Claude Code runs inside the container, so it sees the same files, dependencies, and tools as the rest of your project's toolchain. In VS Code you can use either the [Claude Code extension panel](./code-vs-code.md) or run `claude` in the integrated terminal; both run inside the container and share the same `~/.claude` configuration.
</Accordion>
...
* **[Amazon Bedrock, Google Cloud's Agent Platform, or Microsoft Foundry](./code-third-party-integrations.md)**: Claude Code uses your cloud provider credentials, with no browser prompt

For cloud providers, pass credentials into the container as environment variables through `containerEnv`, a Codespaces secret, or your cloud's workload identity rather than mounting credential files from the host. See [Amazon Bedrock](./code-amazon-bedrock.md), [Google Cloud's Agent Platform](./code-google-vertex-ai.md), or [Microsoft Foundry](./code-microsoft-foundry.md) for the credential chain Claude Code reads.

See [Choose your API provider](./code-admin-setup.md#choose-your-api-provider) to decide which path fits your organization.
...
By default, the container's home directory is discarded on rebuild, so engineers must sign in again each time. Claude Code stores its authentication token, user settings, and session history under [`~/.claude`](./code-claude-directory.md). Mount a named volume at that path to keep this state across rebuilds.
...
Replace `/home/node` with the home directory of your container's `remoteUser`. If you mount the volume somewhere other than `~/.claude`, set [`CLAUDE_CONFIG_DIR`](./code-env-vars.md) to the mount path so Claude Code reads and writes there.
...
Claude Code reads `/etc/claude-code/managed-settings.json` on Linux and applies it at the highest precedence in the [settings hierarchy](./code-settings.md#how-scopes-interact), so values there override anything an engineer sets in `~/.claude` or the project's `.claude/` directory. Copy the file into place from your Dockerfile:
...
Because the Dockerfile lives in the repository, anyone with write access can change or remove this step. For policy that engineers cannot bypass by editing repository files, deliver managed settings through [server-managed settings](./code-server-managed-settings.md) or your MDM instead. See [managed settings files](./code-settings.md#settings-files) for the available keys and the other delivery paths.

To set [environment variables](./code-env-vars.md) that apply to every Claude Code session in the container, add them to `containerEnv` in your `devcontainer.json`. The following example opts out of telemetry and error reporting and prevents Claude Code from auto-updating after install:
...
For the full list of policy controls including permission rules, tool restrictions, and MCP server allowlists, see [Set up Claude Code for your organization](./code-admin-setup.md).

To make [MCP servers](./code-mcp.md) available inside the container, define them at [project scope](./code-mcp.md#mcp-installation-scopes) in a `.mcp.json` file at the repository root so they are checked in alongside your dev container configuration. Install any binaries that local stdio servers depend on in your Dockerfile, and add remote server domains to your network allowlist.

## Restrict network egress

You can limit the container's outbound traffic to only the domains Claude Code needs. See [Network access requirements](./code-network-config.md#network-access-requirements) for the inference and authentication domains, and [Telemetry services](./code-data-usage.md#telemetry-services) for the optional telemetry and error reporting connections and how to disable them.
...
* [Set up Claude Code for your organization](./code-admin-setup.md): choose an authentication provider, decide how policy reaches devices, and plan the rollout
* [Server-managed settings](./code-server-managed-settings.md): deliver managed policy from the Claude.ai admin console so engineers cannot bypass it by editing repository files
* [Monitor usage and audit activity](./code-monitoring-usage.md): export OpenTelemetry metrics and review what your team is running
* [Network access requirements](./code-network-config.md#network-access-requirements): the full domain allowlist for proxies and firewalls
* [Telemetry services and opt-out](./code-data-usage.md#telemetry-services): what Claude Code sends by default and the environment variables that disable it
* [Explore the `.claude` directory](./code-claude-directory.md): what the volume mount holds, including credentials, settings, and session history
* [Sandbox environments](./code-sandbox-environments.md): compare dev containers with the built-in Bash sandbox, custom containers, and VMs
* [Security model](./code-security.md): how Claude Code's permission system, sandboxing, and prompt-injection protections fit together
* [Permission modes](./code-permission-modes.md): the full range from plan mode to auto mode to bypass, and when to use each
