...
<Note>
  Forked subagents are experimental and require Claude Code v2.1.117 or later. Behavior and configuration may change in future releases. Enable them by setting the [`CLAUDE_CODE_FORK_SUBAGENT`](./code-env-vars.md) environment variable to `1`. The variable is honored in interactive mode and via the SDK or `claude -p`.
</Note>
...
* The `/fork` command spawns a fork instead of acting as an alias for [`/branch`](./code-commands.md).
...
Setting `CLAUDE_CODE_FORK_SUBAGENT=1` enables fork mode in interactive sessions, [non-interactive mode](./code-headless.md), and the Agent SDK. A fork cannot spawn further forks.
