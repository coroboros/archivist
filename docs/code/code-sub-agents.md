...
<Note>
  Forked subagents are experimental and require Claude Code v2.1.117 or later. Behavior and configuration may change in future releases. Enable them by setting the [`CLAUDE_CODE_FORK_SUBAGENT`](./code-env-vars.md) environment variable to `1`.
</Note>
...
Fork mode works only in interactive sessions. It is disabled in [non-interactive mode](./code-headless.md), which includes the Agent SDK. A fork cannot spawn further forks.
