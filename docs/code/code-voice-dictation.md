...
Dictation also works in [agent view](./code-agent-view.md#peek-and-reply). Hold or tap your push-to-talk key while the dispatch input or a peek-panel reply is focused to dictate to a background session.
...
* **A local microphone**: voice dictation does not work in remote environments such as [Claude Code on the web](./code-claude-code-on-the-web.md) or SSH sessions.
...
Transcription does not consume Claude messages or tokens and does not count toward the limits shown in `/usage`. See [data usage](./code-data-usage.md) for how Anthropic handles your data.
...
Some keys are not delivered to terminal applications and can't be bound at all. For example, `Caps Lock` shows an error if you try to bind it. See [customize keyboard shortcuts](./code-keybindings.md) for the full keybinding syntax and the list of reserved shortcuts.
