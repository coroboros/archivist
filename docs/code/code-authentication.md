...
Admins can restrict interactive login with the [`forceLoginMethod` and `forceLoginOrgUUID`](./code-settings.md#available-settings) managed settings. When either is set, sessions authenticated by `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, or `apiKeyHelper` are blocked at startup; cloud provider sessions aren't affected.
