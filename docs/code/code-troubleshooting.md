...
| Update or install download fails with `The connection dropped while downloading the update` or `aborted`                                             | [Error reference](./code-errors.md#the-connection-dropped-while-downloading-the-update)        |
...
| `model not found` or `you may not have access to it`                                                                                                 | [Error reference](./code-errors.md#theres-an-issue-with-the-selected-model)                    |
...
4. Restart with [`claude --safe-mode`](./code-cli-reference.md#cli-flags) to check whether a plugin, MCP server, or hook is the source. It disables all customizations for the session; if usage drops, see [Debug your configuration](./code-debug-your-config.md#test-against-a-clean-configuration) to find which one
...
3. Move the large-file work to a [subagent](./code-sub-agents.md) so it runs in a separate context window
...
<Tabs>
  ...
  <Tab title="Alpine">
    ...
    `ripgrep` is in Alpine's community repository. If `apk` reports that the package is missing, see [Alpine Linux setup](./code-setup.md#alpine-linux-and-musl-based-distributions).
  </Tab>
...
Then set `USE_BUILTIN_RIPGREP=0` in your [environment](./code-env-vars.md). To confirm the switch took effect, run `claude doctor` in your terminal and check that the Search line shows the path of your system ripgrep instead of `OK (bundled)`.
