...
Check where the model is set, in [priority order](./code-model-config.md#setting-your-model):
...
To browse models available to your account, start `claude` interactively and run `/model` to open the picker. For Vertex AI deployments, see [the Vertex AI troubleshooting section](./code-google-vertex-ai.md#troubleshooting).
...
1. See the [Error reference](./code-errors.md) for `API Error: 5xx`, `529 Overloaded`, `429`, and request validation errors that appear during a session
