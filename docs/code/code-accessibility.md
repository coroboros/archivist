...
* For every session on the machine: add `"axScreenReader": true` to your user [settings file](./code-settings.md). This covers any terminal, including the VS Code integrated terminal.

<Note>
  The methods are listed in precedence order: the [`--ax-screen-reader`](./code-cli-reference.md#cli-flags) flag overrides the [`CLAUDE_AX_SCREEN_READER`](./code-env-vars.md) environment variable, which overrides the [`axScreenReader`](./code-settings.md#available-settings) setting.
</Note>
...
Screen reader mode renders as plain scrolling text, even if you've turned on [fullscreen rendering](./code-fullscreen.md) with the [`tui` setting](./code-settings.md#available-settings); the setting has no effect while the mode is active. Attached background sessions still render fullscreen; see [Known limitations](#known-limitations).
...
| `Cost:`                | The session cost summary when Claude Code exits, if your account [shows costs](./code-costs.md) |
...
{/* min-version: 2.1.210 */}Cycling [permission modes](./code-permission-modes.md) with `Shift+Tab` announces the mode you land on, such as `[plan mode on]` or `[accept edits on]`. Claude Code prints the announcement once and doesn't repeat it on later redraws. Requires Claude Code v2.1.210 or later.
...
The bell is your terminal's standard alert. To silence it, change the bell setting in your terminal application. The bell doesn't require screen reader mode: outside the mode, set [`preferredNotifChannel`](./code-settings.md#available-settings) to `"terminal_bell"` for similar alerts when Claude is waiting on you. See [Get a terminal bell or notification](./code-terminal-config.md#get-a-terminal-bell-or-notification).
...
* The `CLAUDE_CODE_ACCESSIBILITY` [environment variable](./code-env-vars.md) is for screen magnifiers. Set `CLAUDE_CODE_ACCESSIBILITY=1` to keep the native terminal cursor visible so that magnifiers, such as macOS Zoom, can track the cursor position.
* The `prefersReducedMotion` [setting](./code-settings.md#available-settings) reduces or disables spinners, shimmer, and other animations without changing the rest of the interface.
* The `theme` [setting](./code-settings.md#available-settings) selects the interface colors, including the colorblind-friendly `dark-daltonized` and `light-daltonized` themes.
...
* Claude Code doesn't announce a permission mode change made in any way other than cycling with `Shift+Tab`, such as entering [plan mode](./code-permission-modes.md#analyze-before-you-edit-with-plan-mode) from a command.
* Attaching to a [background session](./code-agent-view.md) with `claude attach` or from agent view enters the terminal's alternate screen, which has no native scrollback. This is the [same behavior as other attached sessions](./code-fullscreen.md). To get back out, press Left Arrow on an empty prompt, or Ctrl+Z if a dialog has focus.
* Claude Code announces costs in the summary it prints at exit, not per turn.
* Screen reader mode doesn't change [non-interactive mode](./code-headless.md) with the `-p` flag. Non-interactive mode already writes plain text and remains an alternative for scripting.
...
* [Settings](./code-settings.md#available-settings): the `axScreenReader`, `prefersReducedMotion`, `theme`, and `preferredNotifChannel` entries
* [Environment variables](./code-env-vars.md): the `CLAUDE_AX_SCREEN_READER` and `CLAUDE_CODE_ACCESSIBILITY` entries
* [CLI reference](./code-cli-reference.md#cli-flags): the `--ax-screen-reader` flag
* [Terminal configuration](./code-terminal-config.md): bells, notifications, and themes outside screen reader mode
* [Non-interactive mode](./code-headless.md): scripted `claude -p` runs, which write plain text without screen reader mode
