...
| :------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Define a tool                                | Use [`@tool`](./code-agent-sdk/python.md#tool) (Python) or [`tool()`](./code-agent-sdk/typescript.md#tool) (TypeScript) with a name, description, schema, and handler. See [Create a custom tool](#create-a-custom-tool). |
...
| Scale to many tools                          | Use [tool search](./code-agent-sdk/tool-search.md) to load tools on demand.                                                                                                                                         |

## Create a custom tool

A tool is defined by four parts, passed as arguments to the [`tool()`](./code-agent-sdk/typescript.md#tool) helper in TypeScript or the [`@tool`](./code-agent-sdk/python.md#tool) decorator in Python:
...
After defining a tool, wrap it in a server with [`createSdkMcpServer`](./code-agent-sdk/typescript.md#createsdkmcpserver) (TypeScript) or [`create_sdk_mcp_server`](./code-agent-sdk/python.md#create_sdk_mcp_server) (Python). The server runs in-process inside your application, not as a separate process.
...
See the [`tool()`](./code-agent-sdk/typescript.md#tool) TypeScript reference or the [`@tool`](./code-agent-sdk/python.md#tool) Python reference for full parameter details, including JSON Schema input formats and return value structure.
...
Every tool in this array consumes context window space on every turn. If you're defining dozens of tools, see [tool search](./code-agent-sdk/tool-search.md) to load them on demand instead.
...
See `ToolAnnotations` in the [TypeScript](./code-agent-sdk/typescript.md#toolannotations) or [Python](./code-agent-sdk/python.md#toolannotations) reference.
...
| allowed tools             | Permission   | Listed tools run without a permission prompt. Unlisted tools remain available; calls go through the [permission flow](./code-agent-sdk/permissions.md).                                                               |
...
To remove a built-in entirely, omit it from `tools` or list its bare name in `disallowedTools` (Python: `disallowed_tools`); both keep the tool out of context so Claude never attempts it. A scoped `disallowedTools` rule blocks matching calls but leaves the tool visible, so Claude may waste a turn trying it. See [Configure permissions](./code-agent-sdk/permissions.md) for the full evaluation order.
...
<Note>
  The Python `@tool` decorator forwards only `content` and `is_error` from the handler's return dict. To return `structuredContent` from Python, run a [standalone MCP server](./code-agent-sdk/mcp.md) instead of an in-process SDK server.
</Note>
...
* If your server grows to dozens of tools, see [tool search](./code-agent-sdk/tool-search.md) to defer loading them until Claude needs them.
* To connect to external MCP servers (filesystem, GitHub, Slack) instead of building your own, see [Connect MCP servers](./code-agent-sdk/mcp.md).
* To control which tools run automatically versus requiring approval, see [Configure permissions](./code-agent-sdk/permissions.md).

## Related documentation

* [TypeScript SDK Reference](./code-agent-sdk/typescript.md)
* [Python SDK Reference](./code-agent-sdk/python.md)
* [MCP Documentation](https://modelcontextprotocol.io)
* [SDK Overview](./code-agent-sdk/overview.md)
