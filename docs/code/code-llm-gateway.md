...
If your gateway uses model names that do not match the discovery filter, use the environment variables documented in [Model configuration](./code-model-config.md) to add them manually.
...
Route to a gateway that forwards to the [Claude Platform on AWS](./code-claude-platform-on-aws.md) endpoint:
