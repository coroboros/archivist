...
<Note>
  This page covers MCP configuration for the Agent SDK. To add MCP servers to the Claude Code CLI so they load in every project, see [MCP installation scopes](./code-mcp.md#mcp-installation-scopes).
</Note>
...
To restore a bounded startup wait for every server, set the [`MCP_CONNECTION_NONBLOCKING`](./code-env-vars.md) environment variable to `0`. The wait is capped at 5 seconds by [`MCP_CONNECT_TIMEOUT_MS`](./code-env-vars.md), and servers still pending at that deadline keep connecting in the background.

To make one server's tools available before the first turn, set `alwaysLoad: true` on its config. Startup then waits for that server to connect, capped at the same 5-second startup deadline, while other servers keep connecting in the background. The `alwaysLoad` field requires Claude Code v2.1.121 or later. See [Exempt a server from deferral](./code-mcp.md#exempt-a-server-from-deferral) for the `alwaysLoad` field's effect on tool search.
...
Define custom tools directly in your application code instead of running a separate server process. See the [custom tools guide](./code-agent-sdk/custom-tools.md) for implementation details.

{/* min-version: 2.1.210 */}An SDK MCP server registered by an [`initialize` control request](./code-agent-sdk/typescript.md#sdkcontrolinitializeresponse) begins connecting as soon as Claude Code processes the request.
...
Tool search is enabled by default. See [Tool search](./code-agent-sdk/tool-search.md) for configuration options, best practices, and using tool search with custom SDK tools.
...
A `"pending"` status means the server is still connecting, not that it failed. To get updated statuses later in the session, call the query's `mcpServerStatus()` method in the TypeScript SDK, or [`ClaudeSDKClient.get_mcp_status()`](./code-agent-sdk/python.md#methods) in Python.
...
MCP server connections time out after 30 seconds by default. If your server takes longer to start, the connection fails. Raise the limit with the [`MCP_TIMEOUT`](./code-env-vars.md) environment variable, in milliseconds. For servers that need more startup time, also consider:
...
The SDK applies the same MCP output limit as Claude Code. When a tool result is larger than 25,000 tokens, the full output is saved to a file and the tool result is replaced with an error message that names the file path, so the agent can read the output back in portions. Raise the limit with the [`MAX_MCP_OUTPUT_TOKENS`](./code-env-vars.md) environment variable. See [MCP output limits and warnings](./code-mcp.md#mcp-output-limits-and-warnings) for the full behavior, including how a server can declare a higher per-tool limit.

## Related resources

* **[Custom tools guide](./code-agent-sdk/custom-tools.md)**: Build your own MCP server that runs in-process with your SDK application
* **[Permissions](./code-agent-sdk/permissions.md)**: Control which MCP tools your agent can use with `allowedTools` and `disallowedTools`
* **[MCP output limits and warnings](./code-mcp.md#mcp-output-limits-and-warnings)**: How the SDK handles tool results that exceed `MAX_MCP_OUTPUT_TOKENS`, including the persist-to-disk fallback and the `anthropic/maxResultSizeChars` per-tool annotation
* **[TypeScript SDK reference](./code-agent-sdk/typescript.md)**: Full API reference including MCP configuration options
* **[Python SDK reference](./code-agent-sdk/python.md)**: Full API reference including MCP configuration options
