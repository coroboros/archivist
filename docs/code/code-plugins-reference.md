...
* Agents appear in the [@-mention typeahead](./code-sub-agents.md#invoke-subagents-explicitly) under their scoped name, such as `my-plugin:code-reviewer`, once the plugin is enabled
...
Plugin monitors use the same mechanism as the [Monitor tool](./code-tools-reference.md#monitor-tool) and share its availability constraints. They run only in interactive CLI sessions, run unsandboxed at the same trust level as [hooks](#hooks), and are skipped on hosts where the Monitor tool is unavailable.
...
| :-------------------------------------------- | :---------------------------------------------------------------------------------- |
| `<skills-dir>/foo/SKILL.md` with no manifest  | A plain [skill](./code-skills.md) named `foo`                                             |
...
| `<cwd>/.claude/skills/` | project  | Only after you accept the workspace [trust dialog](./code-settings.md) for that folder |
...
* MCP servers it declares go through the [same per-server approval](./code-mcp.md) as a project `.mcp.json`
...
| :----- | :----- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------- |
| `name` | string | Unique identifier (kebab-case, no spaces). When a [marketplace entry](./code-plugin-marketplaces.md#plugin-entries) lists the plugin under a different name, the marketplace entry name is what `enabledPlugins` keys and `/plugin` use | `"deployment-tools"` |
...
* **A dependency requirement**: when a plugin is required by another one that is active, Claude Code writes `true` for it at install or enable time. That gives it an explicit setting, so its own default no longer applies. See [Enable or disable a plugin with dependencies](./code-plugin-dependencies.md#enable-or-disable-a-plugin-with-dependencies).

The same field can appear in a plugin's marketplace entry, where it takes precedence over the value in `plugin.json`. See [Optional plugin fields](./code-plugin-marketplaces.md#optional-plugin-fields).
...
| `experimental.monitors` | string\|array         | Background [Monitor](./code-tools-reference.md#monitor-tool) configurations that start automatically when the plugin is active. See [Monitors](#monitors)                           | `"./monitors.json"`                                  |
...
| `dependencies`          | array                 | Other plugins this plugin requires, optionally with semver version constraints. See [Constrain plugin dependency versions](./code-plugin-dependencies.md)                           | `[{ "name": "secrets-vault", "version": "~2.1.0" }]` |
...
MCP servers can also call the `roots/list` request to read the session's working directories at runtime. See [what `roots/list` returns and when Claude Code notifies the server of changes](./code-mcp.md#option-3-add-a-local-stdio-server).
...
| **Settings**      | `settings.json`              | Default configuration applied when the plugin is enabled. Only the [`agent`](./code-sub-agents.md) and [`subagentStatusLine`](./code-statusline.md#subagent-status-lines) keys are currently supported |
...
| `channel`      | An MCP-based [channel](./code-channels.md): a stdio server (`server.ts`), its `.mcp.json`, and a `package.json` |

The scaffolded plugin uses the `@skills-dir` source rather than a marketplace. Admins can block this source with `strictKnownMarketplaces` or by adding `{"source": "skills-dir"}` to `blockedMarketplaces` in [managed settings](./code-plugin-marketplaces.md#managed-marketplace-restrictions). When blocked, `plugin init` fails before writing.
...
Remove auto-installed plugin dependencies that are no longer required by any installed plugin. Dependencies that Claude Code pulled in to satisfy another plugin's [`dependencies`](./code-plugin-dependencies.md) field are removed; plugins you installed directly are never touched.
...
Enable a disabled plugin. If the plugin declares [dependencies](./code-plugin-dependencies.md), Claude Code enables them transitively at the same scope, and the command fails when a dependency is not installed.
...
Disable a plugin without uninstalling it. Fails when another enabled plugin [depends on](./code-plugin-dependencies.md#enable-or-disable-a-plugin-with-dependencies) the target. The error message includes a chained command that disables every dependent first.
...
Create a release git tag for a plugin. By default the command tags the plugin in the current directory; pass a path to tag a plugin elsewhere. See [Tag plugin releases](./code-plugin-dependencies.md#tag-plugin-releases-for-version-resolution).
