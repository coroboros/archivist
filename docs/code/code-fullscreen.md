...
Run `/tui fullscreen` inside any Claude Code conversation. The CLI saves the [`tui` setting](./code-settings.md#available-settings) and relaunches into fullscreen with your conversation intact, so you can switch mid-session without losing context. Run `/tui default` to switch back to the classic renderer, or `/tui` with no argument to print which renderer is active.

The relaunched session keeps the conversation as it appears on screen. If you ran [`/rewind`](./code-checkpointing.md#rewind-and-summarize) earlier in the session, the relaunch resumes from the rewound point rather than the longer transcript saved on disk. Before v2.1.207, switching renderers after a rewind restored the conversation the rewind had removed.
...
These actions are rebindable. See [Scroll actions](./code-keybindings.md#scroll-actions) for the full list of action names, including half-page and full-page variants that have no default binding.
...
Some terminals block OSC 52 by default. iTerm2 blocks it until you turn on Settings → General → Selection → Applications in terminal may access clipboard; running [`/terminal-setup`](./code-terminal-config.md) in iTerm2 enables this for you.
...
Set [`CLAUDE_CODE_ALT_SCREEN_FULL_REPAINT=1`](./code-env-vars.md) to repaint every cell on every frame instead of sending incremental updates.
...
On Windows, Claude Code already enables full repaint automatically for background sessions and [agent view](./code-agent-view.md), so you only need to set the variable for an interactive fullscreen session you launched directly.
