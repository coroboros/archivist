...
This page collects short recipes for everyday development. For higher-level guidance on prompting and context management, see [Best practices](./code-best-practices.md).
...
For configuring Claude Code in a monorepo or large codebase, see [Monorepos and large repos](./code-large-codebases.md).
...
When you create a PR using `gh pr create`, the session is automatically linked to that PR. To return to it later, run `claude --from-pr 123`, replacing 123 with the PR number, or paste the PR URL into the [`/resume` picker](./code-sessions.md#use-the-session-picker) search.
...
| :----------------------------------------------------- | :-------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Routines](./code-routines.md)                               | Anthropic-managed infrastructure  | Tasks that should run even when your computer is off. Can also trigger on API calls or GitHub events in addition to a schedule. Configure at [claude.ai/code/routines](https://claude.ai/code/routines). |
...
| [`/loop`](./code-scheduled-tasks.md)                         | The current CLI session           | Quick polling while a session is open. Tasks stop when you start a new conversation; `--resume` and `--continue` restore unexpired ones.                                                                 |
...
This resumes the most recent session in the current directory; if there isn't one yet, it prints `No conversation found to continue` and exits. Use `claude --resume` to choose from a list, or `/resume` from inside a running session. See [Manage sessions](./code-sessions.md) for naming, branching, and the full picker reference.
...
Run the same command with a different name in a second terminal to start an isolated parallel session. See [Worktrees](./code-worktrees.md) for cleanup, `.worktreeinclude`, and non-git VCS support. To monitor parallel sessions from one screen instead of separate terminals, see [background agents](./code-agent-view.md).
...
You can also press `Shift+Tab` mid-session to toggle into plan mode. See [Plan mode](./code-permission-modes.md#analyze-before-you-edit-with-plan-mode) for the approval flow and editing the plan in your text editor.
...
The subagent reads files in its own context window and reports a summary. See [Subagents](./code-sub-agents.md) for defining custom agents with their own tools and prompts.
...
See [Non-interactive mode](./code-headless.md) for output formats, permission flags, and fan-out patterns.
