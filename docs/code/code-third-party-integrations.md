...
If you deploy through [Bedrock](./code-amazon-bedrock.md), [Vertex AI](./code-google-vertex-ai.md), [Foundry](./code-microsoft-foundry.md), or [Claude Platform on AWS](./code-claude-platform-on-aws.md), pin specific model versions using `ANTHROPIC_DEFAULT_OPUS_MODEL`, `ANTHROPIC_DEFAULT_SONNET_MODEL`, and `ANTHROPIC_DEFAULT_HAIKU_MODEL`. Without pinning, model aliases resolve to the latest version, which may not yet be enabled in your account when Anthropic releases an update. Pinning lets you control when your users move to a new model. See [Model configuration](./code-model-config.md#pin-models-for-third-party-deployments) for what each provider does when the latest version is unavailable.
