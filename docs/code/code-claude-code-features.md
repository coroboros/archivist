...
For a conceptual overview of what each feature does and when to use it, see [Extend Claude Code](./code-features-overview.md).
...
| :----------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Managed policy settings                                            | Endpoint-managed policy, such as an MDM plist, registry policy, or managed settings file, loads from the host. [Server-managed settings](./code-server-managed-settings.md) are fetched on an [eligible configuration](./code-server-managed-settings.md#platform-availability) when the session authenticates with an organization OAuth login or a directly configured API key | Endpoint policy: remove the managed settings file, plist, or registry policy from the host. Server-managed settings: controlled by your org admin; cannot be disabled from the SDK |
...
| [claude.ai MCP connectors](./code-mcp.md#use-mcp-servers-from-claude-ai) | Loaded when the session authenticates with your claude.ai login. Not loaded when `CLAUDE_CODE_OAUTH_TOKEN` holds a token from [`claude setup-token`](./code-authentication.md#generate-a-long-lived-token), which can only make model requests. Passing `mcpServers: {}` does not suppress the connectors                                                                  | Set `strictMcpConfig: true`, [`disableClaudeAiConnectors: true`](./code-mcp.md#disable-claude-ai-connectors) in settings, or `ENABLE_CLAUDEAI_MCP_SERVERS=false` in `env`                |

<Warning>
  Do not rely on default `query()` options for multi-tenant isolation. Because the inputs above are read regardless of `settingSources`, an SDK process can pick up host-level configuration and per-directory memory. For multi-tenant deployments, run each tenant in its own filesystem and set `settingSources: []` plus `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` in `env`. [Server-managed settings](./code-server-managed-settings.md) are fetched when the process authenticates with an organization credential; filesystem isolation does not remove them. See [Secure deployment](./code-agent-sdk/secure-deployment.md).
</Warning>
...
<Tip>
  You can also inject context directly via `systemPrompt` without using CLAUDE.md files. See [Modify system prompts](./code-agent-sdk/modifying-system-prompts.md). Use CLAUDE.md when you want the same context shared between interactive Claude Code sessions and your SDK agents.
</Tip>

For how to structure and organize CLAUDE.md content, see [Manage Claude's memory](./code-memory.md).
...
<Note>
  Skills must be created as filesystem artifacts (`.claude/skills/<name>/SKILL.md`). The SDK does not have a programmatic API for registering skills. See [Agent Skills in the SDK](./code-agent-sdk/skills.md) for full details.
</Note>

For more on creating and using skills, see [Agent Skills in the SDK](./code-agent-sdk/skills.md).

## Hooks

The SDK supports two ways to define hooks, and they run side by side:

* **Filesystem hooks:** shell commands defined in `settings.json`, loaded when `settingSources` includes the relevant source. These are the same hooks you'd configure for [interactive Claude Code sessions](./code-hooks-guide.md).
* **Programmatic hooks:** callback functions passed directly to `query()`. These run in your application process and can return structured decisions. See [Control execution with hooks](./code-agent-sdk/hooks.md).
...
<Note>
  The TypeScript SDK supports additional hook events beyond Python, including `SessionStart`, `SessionEnd`, `TeammateIdle`, and `TaskCompleted`. See the [hooks guide](./code-agent-sdk/hooks.md) for the full event compatibility table.
</Note>

For full details on programmatic hooks, see [Control execution with hooks](./code-agent-sdk/hooks.md). For filesystem hook syntax, see [Hooks](./code-hooks.md).
...
| Give Claude structured tool access to an external service                                         | [MCP](./code-agent-sdk/mcp.md)                      | `mcpServers` parameter                                                                                                                                         |

<Tip>
  **Subagents versus agent teams:** Subagents are ephemeral and isolated: fresh conversation, one task, summary returned to parent. Agent teams coordinate multiple independent Claude Code instances that share a task list and message each other directly. Agent teams are a CLI feature. See [What subagents inherit](./code-agent-sdk/subagents.md#what-subagents-inherit) and the [agent teams comparison](./code-agent-teams.md#compare-with-subagents) for details.
</Tip>

Every feature you enable adds to your agent's context window. For per-feature costs and how these features layer together, see [Extend Claude Code](./code-features-overview.md#understand-context-costs).

## Related resources

* [Extend Claude Code](./code-features-overview.md): Conceptual overview of all extension features, with comparison tables and context cost analysis
* [Skills in the SDK](./code-agent-sdk/skills.md): Full guide to using skills programmatically
* [Subagents](./code-agent-sdk/subagents.md): Define and invoke subagents for isolated subtasks
* [Hooks](./code-agent-sdk/hooks.md): Intercept and control agent behavior at key execution points
* [Permissions](./code-agent-sdk/permissions.md): Control tool access with modes, rules, and callbacks
* [System prompts](./code-agent-sdk/modifying-system-prompts.md): Inject context without CLAUDE.md files
