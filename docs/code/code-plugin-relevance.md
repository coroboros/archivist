...
Marketplace-declared suggestions are opt-in per marketplace through [managed settings](./code-settings.md#settings-files). No marketplace's `relevance` declarations produce suggestions until an administrator adds it to the allowlist, including the official Anthropic marketplace. Claude Code also includes one built-in suggestion that is independent of this allowlist; that tip and all marketplace-declared tips are disabled when [`spinnerTipsEnabled`](./code-settings.md#available-settings) is set to `false`.

{/* min-version: 2.1.152 */}This feature requires Claude Code v2.1.152 or later. Older clients ignore the `relevance` field.

This page is for marketplace operators and enterprise administrators. If you are looking to install plugins, see [Discover and install plugins](./code-discover-plugins.md).
...
Declaring `relevance` in `marketplace.json` is not enough on its own. An administrator must allowlist the marketplace in [managed settings](./code-settings.md#settings-files) before its suggestions appear to users.
...
See the [settings reference](./code-settings.md) for `pluginSuggestionMarketplaces` and [`extraKnownMarketplaces`](./code-settings.md#extraknownmarketplaces) for full configuration details.
...
* [Create and distribute a plugin marketplace](./code-plugin-marketplaces.md): build the marketplace that hosts your plugins
* [Recommend your plugin from your CLI](./code-plugin-hints.md): prompt users from your own CLI instead of from Claude Code's session signals
* [Settings](./code-settings.md): full reference for `pluginSuggestionMarketplaces` and `extraKnownMarketplaces`
