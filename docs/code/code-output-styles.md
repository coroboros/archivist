...
For instructions about your project, conventions, or codebase, use [CLAUDE.md](./code-memory.md) instead.
...
Run `/config` and select **Output style** to pick a style from a menu. Your selection is saved to `.claude/settings.local.json` at the [local project level](./code-settings.md).
...
Output style is part of the system prompt, which Claude Code reads once at session start. Changes take effect after `/clear` or a new session. See [How Claude Code uses prompt caching](./code-prompt-caching.md#changing-output-style) for what an output style change does to the cache.
...
<Steps>
  <Step title="Create a Markdown file">
    ...
    * Managed policy: `.claude/output-styles` inside the [managed settings directory](./code-settings.md#settings-files)
...
[Plugins](./code-plugins-reference.md) can also ship output styles in an `output-styles/` directory.
...
Output styles apply to the main conversation only: a [subagent runs its own system prompt](./code-sub-agents.md#what-loads-at-startup), so styles don't change how subagents respond. A [fork](./code-sub-agents.md#fork-the-current-conversation) is the exception, because it inherits the parent's full system prompt.
...
| [CLAUDE.md](./code-memory.md)  | Adds a user message after the system prompt                  | Claude should always know your project conventions and codebase context |
...
| [Agents](./code-sub-agents.md) | Runs a subagent with its own system prompt, model, and tools | You want a separately scoped helper for a focused task                  |
| [Skills](./code-skills.md)     | Loads task-specific instructions when invoked or relevant    | You have a reusable workflow                                            |

## Related resources

* [Settings](./code-settings.md): where the `outputStyle` field lives and how settings precedence works
* [Permission modes](./code-permission-modes.md): how the Proactive style compares to auto mode
* [Plugins](./code-plugins.md): package and distribute output styles alongside skills, hooks, and agents
* [Debug your configuration](./code-debug-your-config.md): diagnose why an output style isn't taking effect
