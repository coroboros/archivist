...
A dynamic workflow is a JavaScript script that orchestrates [subagents](./code-sub-agents.md) at scale. Claude writes the script for the task you describe, and a runtime executes it in the background while your session stays responsive.
...
[Subagents](./code-sub-agents.md), [skills](./code-skills.md), [agent teams](./code-agent-teams.md), and workflows can all run a multi-step task. The difference is who holds the plan:
...
Claude Code highlights the keyword in your input and Claude writes a workflow script for the task instead of working through it turn by turn. The keyword only chooses how Claude structures the work: a workflow started this way runs inside the session's existing [permission mode](./code-permission-modes.md), and its agents' tool calls receive the same permission checks and [sandboxing](./code-sandboxing.md) as any other tool call in the session.
...
The keyword is an opt-in only in a prompt you type yourself: at the interactive prompt, in an IDE extension panel, in a [Remote Control](./code-remote-control.md) client, or in an Agent SDK application that stamps your keyboard input's [`origin`](./code-agent-sdk/typescript.md#sdkmessageorigin) as `{ kind: "human" }`. It doesn't start a workflow when it reaches the session another way:
...
Ultracode is a Claude Code setting that combines `xhigh` [reasoning effort](./code-model-config.md#adjust-effort-level) with automatic workflow orchestration. With it on, Claude plans a workflow for each substantive task instead of waiting for you to ask.
...
Ultracode lasts for the current session and resets when you start a new one. Drop back with `/effort high` when you return to routine work. It's available on models that support `xhigh` [effort](./code-model-config.md#adjust-effort-level); on other models the `/effort` menu doesn't offer it.
...
Whether you see this prompt depends on your [permission mode](./code-permission-modes.md):
...
Your permission mode controls only the launch prompt above. The subagents the workflow spawns always run in `acceptEdits` mode and inherit your [tool allowlist](./code-settings.md#permission-settings), regardless of your session's mode. File edits are auto-approved.
...
* `~/.claude/workflows/` in your home directory: available in every project, visible only to you. If you set [`CLAUDE_CONFIG_DIR`](./code-env-vars.md), this location is the `workflows/` directory under that path.
...
Every agent in a workflow uses your session's model unless the script routes a stage to a different one or the [`CLAUDE_CODE_SUBAGENT_MODEL`](./code-model-config.md#environment-variables) environment variable is set, which overrides both. To control the model cost:
...
Workflows are available in the CLI, the Desktop app, the IDE extensions, [non-interactive mode](./code-headless.md) with `claude -p`, and the [Agent SDK](./code-agent-sdk/overview.md). The same disable settings apply on every surface.
...
To turn workflows off for your whole organization, set `"disableWorkflows": true` in [managed settings](./code-server-managed-settings.md), or use the toggle on the [Claude Code admin settings](https://claude.ai/admin-settings/claude-code) page.
...
* [Run agents in parallel](./code-agents.md): compare subagents, agent view, agent teams, and workflows
* [Create custom subagents](./code-sub-agents.md): the worker primitive workflows orchestrate
* [Manage costs](./code-costs.md): how multi-agent runs count toward usage limits
