...
Claude Code includes several security features that address common concerns. See the [security documentation](./code-security.md) for full details.

* **Permissions system**: Every tool and bash command can be configured to allow, block, or prompt the user for approval. Use glob patterns to create rules like "allow all npm commands" or "block any command with sudo". Organizations can set policies that apply across all users. See [permissions](./code-permissions.md).
...
* **Sandbox mode**: Bash commands can run in a sandboxed environment that restricts filesystem and network access. See the [sandboxing documentation](./code-sandboxing.md) for details.
...
* [Claude Code security documentation](./code-security.md)
* [Hosting the Agent SDK](./code-agent-sdk/hosting.md)
* [Handling permissions](./code-agent-sdk/permissions.md)
