...
| `CLAUDE_CODE_SUBAGENT_MODEL`     | The model to use for all [subagents](./code-sub-agents.md#choose-a-model) and [agent teams](./code-agent-teams.md). Overrides the per-invocation `model` parameter and the subagent definition's `model` frontmatter. Set to `inherit` to use normal model resolution instead |
