...
* **`advisorModel` setting**: configure a persistent default in your [settings file](./code-settings.md)
...
<Note>
  To use Fable 5 as the advisor, you need Claude Code v2.1.170 or later and [Fable 5 access](./code-model-config.md#work-with-fable-5) for your organization.
</Note>
...
Your selection is saved to `advisorModel` in your user settings and persists across sessions. If your organization's [`availableModels`](./code-model-config.md#restrict-model-selection) allowlist excludes the saved advisor model, the advisor is not invoked until you pick an allowed model with `/advisor`. If your current main model does not support the advisor, the selection is still saved and activates when you switch to a [compatible main model](#choose-an-advisor-model) with [`/model`](./code-model-config.md#setting-your-model).
...
The flag takes precedence over the `advisorModel` setting for that session. It exits with an error if the session's main model does not support the advisor, or if the requested advisor model is excluded by your organization's [`availableModels`](./code-model-config.md#restrict-model-selection) allowlist.
...
Claude calls the advisor at decision points rather than on every turn, so pairing a faster main model with a stronger advisor typically costs less than running the stronger model throughout. Advisor usage counts toward the session totals shown by [`/usage`](./code-costs.md#track-your-costs).
...
Enabling or disabling the advisor mid-session does not invalidate your main model's [prompt cache](./code-prompt-caching.md). Unlike [changing model or effort level](./code-prompt-caching.md#actions-that-invalidate-the-cache), toggling `/advisor` keeps the cached prefix intact, and the advisor's returned guidance is cached as part of the transcript on later turns.
...
* **Anthropic API only**: the advisor is a server-executed tool. It is not available on Amazon Bedrock, Google Vertex AI, or Microsoft Foundry. Through an [LLM gateway](./code-llm-gateway.md) configured with `ANTHROPIC_BASE_URL`, availability depends on whether the gateway forwards the request intact to the Anthropic API.
...
To disable the advisor tool entirely, including the `/advisor` command and the `--advisor` flag, set `CLAUDE_CODE_DISABLE_ADVISOR_TOOL=1`. See [Environment variables](./code-env-vars.md).
...
| [`opusplan`](./code-model-config.md#opusplan-model-setting)       | During plan mode when [allowed by `availableModels`](./code-model-config.md#restrict-model-selection), then switches to Sonnet for execution | You enter plan mode                          |
| [Subagents](./code-sub-agents.md#choose-a-model) with `model` set | For the entire delegated subtask                                                                                                       | Claude delegates, or you invoke the subagent |
| [`/model`](./code-model-config.md#setting-your-model)             | For all subsequent turns                                                                                                               | You switch models                            |

## See also

* [Model configuration](./code-model-config.md): switch models, set effort levels, and use `opusplan`
* [Manage costs effectively](./code-costs.md): track token usage across models
