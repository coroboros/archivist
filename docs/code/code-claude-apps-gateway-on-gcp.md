...
* **Cloud SQL for PostgreSQL** instance, private IP only, for the gateway's [store](./code-claude-apps-gateway-config.md#store)
...
* A Google Workspace OAuth 2.0 web-application client with redirect URI `https://<gateway-host>/oauth/callback`; see [Identity provider setup](./code-claude-apps-gateway-deploy.md#identity-provider-setup)
...
<Steps>
  ...
  <Step title="Build and push the image to Artifact Registry">
    Build the image per the [container image requirements](./code-claude-apps-gateway-deploy.md#container-image), using the `linux-x64` glibc binary, and push it:
  ...
  <Step title="Write gateway.yaml">
    The `upstreams` block points at Google Cloud's Agent Platform with `auth: {}`, so the gateway authenticates via Application Default Credentials from the runtime service account. See the [configuration reference](./code-claude-apps-gateway-config.md) for every field.
    ...
    * `trusted_proxies`: the front end's source ranges. The gateway honors `X-Forwarded-For` only when the TCP peer is in this list, then walks the chain past trusted hops, so per-IP sign-in rate limits and audit events record developer IPs instead of the load balancer's.
    ...
    Set `trusted_proxies` to match your front end. An external GKE Ingress of class `gce` isn't listed: it provisions a public forwarding-rule address, which the `/login` [private-network check](./code-claude-apps-gateway.md#prerequisites) rejects.
    ...
    <Note>
      Google id\_tokens carry no `groups` claim. To use group-based policies in [`managed.policies`](./code-claude-apps-gateway-config.md#managed) with Google Workspace as the IdP, configure [`oidc.google_groups`](./code-claude-apps-gateway-config.md#oidc), which looks up each user's groups through the Admin SDK Directory API using a service account with domain-wide delegation. Without it, match on `email_domain` instead.
    </Note>
  </Step>
    ...
    <Tabs>
      <Tab title="Cloud Run">
        ...
        * `--allow-unauthenticated`: grants `allUsers` the `run.invoker` role; use it if your organization doesn't allow `--no-invoker-iam-check`
        ...
        By default the Cloud Run `*.run.app` URL resolves to a public address, which the `/login` [private-network check](./code-claude-apps-gateway.md#prerequisites) rejects. Two topologies give developers a privately resolvable hostname, and Cloud Run provisions neither for you:
      ...
      <Tab title="GKE">
        ...
        Deploy the gateway as a standard Deployment plus a Service and an internal Ingress, class `gce-internal`, as described in [Kubernetes deployment](./code-claude-apps-gateway-deploy.md#kubernetes), with:
        ...
        * the readiness probe pointed at `GET /readyz`
        ...
        Don't apply an egress NetworkPolicy that blocks `169.254.169.254` on a Workload Identity cluster; the pod must reach the metadata server for credentials. The gateway's built-in [SSRF guard](./code-claude-apps-gateway-deploy.md#threat-model-summary) is the defense there.
  ...
  <Step title="Push the gateway URL to developer machines">
    The gateway is now running, but developers can't reach it from `/login` until the gateway URL is on their machines. Deploy the full [managed settings snippet](./code-claude-apps-gateway.md#set-the-gateway-url), with `forceLoginMethod`, `forceLoginGatewayUrl`, and the `parentSettingsBehavior: "merge"` opt-in, to each device via MDM. There is no gateway option in the login picker for a developer to select manually.
  </Step>
</Steps>
...
For gateway boot and login errors, see the platform-agnostic [troubleshooting table](./code-claude-apps-gateway-deploy.md#troubleshooting). The entries below are specific to Google Cloud.
...
* [Configuration reference](./code-claude-apps-gateway-config.md): every `gateway.yaml` option, including `managed.policies` and `telemetry`
* [Deployment and operations](./code-claude-apps-gateway-deploy.md): IdP setup, health checks, JWT secret rotation, upgrades, and the security model
* [Claude apps gateway overview](./code-claude-apps-gateway.md): quickstart and connecting developers
