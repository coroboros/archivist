...
If installation fails or you can't sign in, find your error below. For runtime issues after Claude Code is working, see [Troubleshooting](./code-troubleshooting.md). For configuration problems such as settings not applying or hooks not firing, see [Debug your configuration](./code-debug-your-config.md).
...
| `API Error: 500`, `529 Overloaded`, `429`, or other 4xx and 5xx errors not listed above               | See the [Error reference](./code-errors.md)                                                                                                         |

If your issue isn't listed, work through the diagnostic checks below to narrow down the cause.

<Tip>
  If you'd rather skip the terminal entirely, the [Claude Code Desktop app](./code-desktop-quickstart.md) lets you install and use Claude Code through a graphical interface. Download it for [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code\&utm_medium=docs) or [Windows](https://claude.com/download?utm_source=claude_code\&utm_medium=docs) and start coding without any command-line setup. On Linux, install the app with apt by following the [Linux install instructions](./code-desktop-linux.md).
</Tip>
...
<Note>
  The [VS Code extension](./code-vs-code.md) does not place `claude` at this location. It bundles a private copy of the CLI inside the extension directory for its own chat panel and does not add it to PATH. If you have only installed the extension, `~/.local/bin/claude` will not exist. Run the [standalone install](./code-setup.md) to use `claude` from a terminal, then continue below.
</Note>
...
<Tabs>
  <Tab title="macOS/Linux">
    ...
    A native install shows a symlink into `~/.local/share/claude/versions/`. A script or a symlink you created yourself at this path is a custom launcher, which [auto-update leaves in place](./code-setup.md#auto-updates).
...
If Homebrew installs an older Claude Code version than you expect, the same stale index is usually the cause. The `claude-code` cask tracks the stable channel and is typically about one week behind the latest release; for the newest version run `brew install --cask claude-code@latest` instead. See [Configure release channel](./code-setup.md#configure-release-channel) for the difference between the two casks.
...
Installing needs roughly 512 MB of free memory, and running Claude Code needs more. See the [system requirements](./code-setup.md#system-requirements).
...
Git for Windows is optional. Claude Code uses the [PowerShell tool](./code-tools-reference.md#powershell-tool) when Git Bash is absent, so this error means neither shell was found.
...
**If Git is already installed** but Claude Code can't find it, set the path in your [settings.json file](./code-settings.md):
...
If this prints `False`, you are on a 32-bit edition of Windows. Claude Code requires a 64-bit operating system. See the [system requirements](./code-setup.md#system-requirements).
...
3. **If you're actually on musl**, such as Alpine Linux, install the required packages:
   ...
   ```
   On Alpine, `ripgrep` is in the community repository. If `apk` reports that the package is missing, see [Alpine Linux setup](./code-setup.md#alpine-linux-and-musl-based-distributions).
...
These issues apply if you installed Claude Code with `npm install -g` inside WSL. If you used the [native installer](./code-setup.md), skip this section.
...
* **Unsupported platform.** Prebuilt binaries are published for `darwin-arm64`, `darwin-x64`, `linux-x64`, `linux-arm64`, `linux-x64-musl`, `linux-arm64-musl`, `win32-x64`, and `win32-arm64`. Claude Code does not ship a binary for other platforms; see the [system requirements](./code-setup.md#system-requirements). {/* min-version: 2.1.205 */}On FreeBSD, the installer reports the platform as unsupported. Before v2.1.205, it treated FreeBSD as Linux and downloaded a binary that couldn't run.
...
See [Amazon Bedrock](./code-amazon-bedrock.md), [Google Cloud's Agent Platform](./code-google-vertex-ai.md), or [Microsoft Foundry](./code-microsoft-foundry.md) for full provider setup.
