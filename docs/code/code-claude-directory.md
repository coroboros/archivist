...
On Windows, `~/.claude` resolves to `%USERPROFILE%\.claude`. If you set [`CLAUDE_CONFIG_DIR`](./code-env-vars.md), every `~/.claude` path on this page lives under that directory instead.
...
| Installed plugins       | `~/.claude/plugins`        | Cloned marketplaces, installed plugin versions, and per-plugin data, managed by `claude plugin` commands. Orphaned versions are deleted 7 days after a plugin update or uninstall. See [plugin caching](./code-plugins-reference.md#plugin-caching-and-file-resolution). |
...
| :------------------------------------------------- | :--------------------------------------- | :---------------- | :------------------------------------------------- |
| Give Claude project context and conventions        | `CLAUDE.md`                              | project or global | [Memory](./code-memory.md)                               |
| Allow or block specific tool calls                 | `settings.json` `permissions` or `hooks` | project or global | [Permissions](./code-permissions.md), [Hooks](./code-hooks.md) |
| Run a script before or after tool calls            | `settings.json` `hooks`                  | project or global | [Hooks](./code-hooks.md)                                 |
| Set environment variables for the session          | `settings.json` `env`                    | project or global | [Settings](./code-settings.md#available-settings)        |
| Keep personal overrides out of git                 | `settings.local.json`                    | project only      | [Settings scopes](./code-settings.md#settings-files)     |
| Add a prompt or capability you invoke with `/name` | `skills/<name>/SKILL.md`                 | project or global | [Skills](./code-skills.md)                               |
| Define a specialized subagent with its own tools   | `agents/*.md`                            | project or global | [Subagents](./code-sub-agents.md)                        |
| Orchestrate many subagents from a script           | `workflows/*.js`                         | project or global | [Dynamic workflows](./code-workflows.md)                 |
| Connect external tools over MCP                    | `.mcp.json`                              | project only      | [MCP](./code-mcp.md)                                     |
| Change how Claude formats responses                | `output-styles/*.md`                     | project or global | [Output styles](./code-output-styles.md)                 |
...
| [`settings.local.json`](#ce-settings-local-json)    | Project only       |        | Your personal overrides, gitignored when Claude Code creates it                                               | [Settings scopes](./code-settings.md#settings-files)                  |
...
| [`workflows/*.js`](#ce-workflows)                   | Project and global | ✓      | Dynamic workflow scripts written by Claude and saved from `/workflows`; each file becomes a `/<name>` command | [Dynamic workflows](./code-workflows.md)                              |
...
If a setting, hook, or file isn't taking effect, see [Debug your configuration](./code-debug-your-config.md) for the inspection commands and a symptom-first lookup table.
...
Files in the paths below are deleted on startup once they're older than [`cleanupPeriodDays`](./code-settings.md#available-settings). The default is 30 days.

| Path under `~/.claude/`                      | Contents                                                                                                                                                                                                                                                |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `projects/<project>/<session>.jsonl`         | Full conversation transcript: every message, tool call, and tool result                                                                                                                                                                                 |
| `projects/<project>/<session>/subagents/`    | [Subagent](./code-sub-agents.md) conversation transcripts, removed with the parent session transcript when it ages out                                                                                                                                        |
| `projects/<project>/<session>/tool-results/` | Large tool outputs spilled to separate files                                                                                                                                                                                                            |
| `file-history/<session>/`                    | Pre-edit snapshots of files Claude changed, used for [checkpoint restore](./code-checkpointing.md). Holds snapshots for the 100 most recent checkpoints; snapshot files that no retained checkpoint references are deleted, except each file's first snapshot |
| `plans/`                                     | Plan files written during [plan mode](./code-permission-modes.md#analyze-before-you-edit-with-plan-mode)                                                                                                                                                      |
...
| `shell-snapshots/`                           | Aliases, functions, and shell options captured at startup and applied by the [Bash tool](./code-tools-reference.md#bash-tool-behavior) to each command. Removed on clean exit. The sweep clears any left after a crash.                                       |
...
| `remote-settings.json`  | Cached copy of [server-managed settings](./code-server-managed-settings.md) for your organization. Only present when your organization has configured them. Refreshed on each launch. |
...
* Set the [`CLAUDE_CODE_SKIP_PROMPT_HISTORY`](./code-env-vars.md) environment variable to skip writing transcripts and prompt history in any mode. In non-interactive mode, you can instead pass `--no-session-persistence` alongside `-p`, or set `persistSession: false` in the Agent SDK.
* Use [permission rules](./code-permissions.md) to deny reads of credential files
