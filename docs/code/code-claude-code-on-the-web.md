...
<Tip>
  New to Claude Code on the web? Start with [Get started](./code-web-quickstart.md) to connect your GitHub account and submit your first task.
</Tip>
...
| :--------------- | :------------------------------------------------------------------------------------------ | :---------------------------------------------------------------------- |
| **GitHub App**   | Authorize the Claude GitHub App during [web onboarding](./code-web-quickstart.md).                | Browser onboarding; teams that want [Auto-fix](#auto-fix-pull-requests) |
...
Either method works. [`/schedule`](./code-routines.md) checks for either form of access and prompts you to run `/web-setup` if neither is configured. See [Connect from your terminal](./code-web-quickstart.md#connect-from-your-terminal) for the `/web-setup` walkthrough.
...
<Note>
  Organizations with [Zero Data Retention](./code-zero-data-retention.md) enabled can't use `/web-setup` or other cloud session features.
</Note>
...
Cloud sessions start from a fresh clone of your repository. Anything committed to the repo is available. Anything you've installed or configured only on your own machine isn't available in the session. Your organization's policy arrives separately through [server-managed settings](./code-server-managed-settings.md).
...
| Plugins declared in `.claude/settings.json`                                                                                                                                               | Yes                         | Installed at session start from the [marketplace](./code-plugin-marketplaces.md) you declared. Requires network access to reach the marketplace source                                                                                                                                                                     |
| Your organization's [server-managed settings](./code-server-managed-settings.md)                                                                                                                | Yes                         | Fetched from Anthropic's servers when the session starts. See [Surface coverage](./code-model-config.md#surface-coverage) for how `availableModels` is enforced in cloud sessions. Settings deployed to your device through MDM or managed settings files don't apply, because the session runs on an Anthropic-managed VM |
...
| MCP servers you added with `claude mcp add`                                                                                                                                               | No                          | Those write to your local user config, not the repo. Declare the server in [`.mcp.json`](./code-mcp.md#project-scope) instead                                                                                                                                                                                              |
| Transport variables in your repo's `.claude/settings.json` `env` block, such as `NODE_EXTRA_CA_CERTS` and the [mTLS client certificate variables](./code-network-config.md#mtls-authentication) | No                          | The hosting environment manages the session's API connection, so Claude Code ignores these keys and notes each ignored key in the session's debug log                                                                                                                                                                |
...
To make your own configuration available in cloud sessions, commit it to the repo; organization policy arrives separately through [server-managed settings](./code-server-managed-settings.md).
...
Tasks requiring significantly more memory, such as large build jobs or memory-intensive tests, may fail or be terminated. For workloads beyond these limits, use [Remote Control](./code-remote-control.md) to run Claude Code on your own hardware.
...
SessionStart hooks can also be defined in your user-level `~/.claude/settings.json` locally, but user-level settings don't carry over to cloud sessions. In the cloud, hooks come from the repo and from your organization's [server-managed settings](./code-server-managed-settings.md).
...
To persist environment variables for subsequent Bash commands, write to the file at `$CLAUDE_ENV_FILE`. See [SessionStart hooks](./code-hooks.md#sessionstart) for details.
...
To change an environment's network access, [open it for editing](#configure-your-environment) and use the **Network access** selector in the dialog. There is no separate Environments page. The cloud icon appears wherever you start a cloud session or configure a [routine](./code-routines.md#environments-and-network-access).
...
Allowed domains are configured per environment. There's no organization-level allowlist that Owners can push to all users' environments; [server-managed settings](./code-server-managed-settings.md) can restrict cloud sessions but can't add allowed domains.
...
<Note>
  `--cloud` creates cloud sessions. `--remote-control` is unrelated: it exposes a local CLI session for monitoring from the web. See [Remote Control](./code-remote-control.md).
</Note>
...
Cloud sessions support [built-in commands](./code-commands.md) that produce text output. Commands that only run in the terminal interface, such as `/plugin` or `/resume`, aren't available. Commands that open a picker or panel in the terminal behave differently in cloud sessions:

* {/* min-version: 2.1.205 */}**`/model`, `/effort`, `/fast`, `/color`, and `/rename`**: pass the value as an argument, for example `/model sonnet`, instead of opening the terminal picker or slider. The argument forms require Claude Code v2.1.205 or later in the session's environment and follow each command's [availability notes](./code-commands.md#all-commands): `/effort` reports `Not applied` while a model's [launch-default effort hold](./code-model-config.md#adjust-effort-level) is in force, and `/fast` works only in a session that started with fast mode turned on.
* **`/config`**: on the web, opens the Claude Code section of your settings instead of setting a value, and text after the command, including `key=value`, is ignored. To change settings for a cloud session, use [environment variables](#configure-your-environment) or commit [settings files](./code-settings.md) to the repository.
...
Auto-compaction runs automatically when the context window approaches capacity. To trigger it earlier, set [`CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`](./code-env-vars.md) in your [environment variables](#configure-your-environment). For example, `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=70` compacts at 70% capacity instead of waiting until the window is nearly full. To change the effective window size for compaction calculations, use [`CLAUDE_CODE_AUTO_COMPACT_WINDOW`](./code-env-vars.md).

[Subagents](./code-sub-agents.md) work the same way they do locally. Claude can spawn them with the Task tool to offload research or parallel work into a separate context window, keeping the main conversation lighter. Subagents defined in your repo's `.claude/agents/` are picked up automatically.

[Agent teams](./code-agent-teams.md) are off by default but can be enabled by adding `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` to your [environment variables](#configure-your-environment).

### Review changes

Each session shows a diff indicator with lines added and removed, like `+42 -18`. Select it to open the diff view, leave inline comments on specific lines, and send them to Claude with your next message. See [Review and iterate](./code-web-quickstart.md#review-and-iterate) for the full walkthrough including PR creation. To have Claude monitor the PR for CI failures and review comments automatically, see [Auto-fix pull requests](#auto-fix-pull-requests).
...
For Enterprise and Team accounts, the two visibility options are **Private** and **Team**. Team visibility makes the session visible to other members of your claude.ai organization. [Claude in Slack](./code-slack.md) sessions are automatically shared with Team visibility.
...
<Note>
  Auto-fix requires the Claude GitHub App to be installed on your repository. If you haven't already, install it from the [GitHub App page](https://github.com/apps/claude) or when prompted during [setup](./code-web-quickstart.md#connect-github-and-create-an-environment).
</Note>
...
* **From your terminal**: run [`/autofix-pr`](./code-commands.md) while on the PR's branch. Claude Code detects the open PR with `gh`, spawns a web session, and turns on auto-fix in one step
...
For runtime API errors that appear in the conversation such as `API Error: 500`, `529 Overloaded`, `429`, or `Prompt is too long`, see the [Error reference](./code-errors.md). Those errors and their fixes are shared with the CLI and Desktop app. The sections below cover issues specific to cloud sessions.
...
* **Platform restrictions**: repository cloning and pull request creation require GitHub. Self-hosted [GitHub Enterprise Server](./code-github-enterprise-server.md) instances are supported for Team and Enterprise plans. GitLab, Bitbucket, and other non-GitHub repositories can be sent to cloud sessions as a [local bundle](#send-local-repositories-without-github), but the session can't push results back to the remote
* **Organization IP allowlist**: cloud sessions call the Anthropic API from Anthropic-managed infrastructure, not your network. If your organization has [IP allowlisting](https://support.claude.com/en/articles/13200993-restrict-access-to-claude-with-ip-allowlisting) enabled, every cloud session fails with an authentication error. The same applies to [Code Review](./code-code-review.md) and [Routines](./code-routines.md). Contact [Anthropic support](https://support.claude.com/) to exempt Anthropic-hosted services from your organization's IP allowlist.

## Related resources

* [Ultraplan](./code-ultraplan.md): draft a plan in a cloud session and review it in your browser
* [Ultrareview](./code-ultrareview.md): run a deep multi-agent code review in a cloud sandbox
* [Routines](./code-routines.md): automate work on a schedule, via API call, or in response to GitHub events
* [Hooks configuration](./code-hooks.md): run scripts at session lifecycle events
* [Settings reference](./code-settings.md): all configuration options
* [Security](./code-security.md): isolation guarantees and data handling
* [Data usage](./code-data-usage.md): what Anthropic retains from cloud sessions
