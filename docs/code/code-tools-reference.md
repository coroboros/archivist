...
| :--------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------ |
| `Agent`                | Spawns a [subagent](./code-sub-agents.md) with its own context window to handle a task. See [Agent tool behavior](#agent-tool-behavior)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | No                  |
| `Artifact`             | Publishes an HTML or Markdown file as an [artifact](./code-artifacts.md): a private, interactive page on claude.ai. On Team and Enterprise plans, you can share it inside your organization. {/* plan-availability: feature=artifacts plans=pro,max,team,enterprise providers=anthropic */}Requires a Pro, Max, Team, or Enterprise plan and `/login` authentication; see [Availability](./code-artifacts.md#availability)                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Yes                 |
| `AskUserQuestion`      | Asks multiple-choice questions to gather requirements or clarify ambiguity. {/* min-version: 2.1.198 */}As of v2.1.198, if you don't respond within 60 seconds the dialog closes on its own: it submits any options you'd already selected and tells Claude you may be away from your keyboard, so Claude proceeds on its own judgment and can re-ask later. A countdown appears for the last 20 seconds. Any keypress keeps the dialog open, and so does a focused window on terminals that report focus. Set the [`CLAUDE_AFK_TIMEOUT_MS`](./code-env-vars.md#variables) environment variable to change how long Claude Code waits, or to a large value like `86400000`, 24 hours, to keep questions open while you're away. This timeout applies only to `AskUserQuestion`'s multiple-choice questions; permission prompts, including plan approval, never auto-resolve on idle | No                  |
| `Bash`                 | Executes shell commands in your environment. See [Bash tool behavior](#bash-tool-behavior)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Yes                 |
| `CronCreate`           | Schedules a recurring or one-shot prompt within the current session. Tasks are session-scoped and restored on `--resume` or `--continue` if unexpired. See [scheduled tasks](./code-scheduled-tasks.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | No                  |
...
| `EnterWorktree`        | Creates an isolated [git worktree](./code-worktrees.md) and switches into it. Pass a `path` to switch into an existing worktree of the current repository instead of creating a new one. From within a worktree session, or from a subagent with a pinned working directory such as [`isolation: worktree`](./code-sub-agents.md#supported-frontmatter-fields), only the `path` form is available and the target must be under `.claude/worktrees/`                                                                                                                                                                                                                                                                                                                                                                                                                                      | No                  |
| `ExitPlanMode`         | Presents a plan for approval and exits plan mode                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Yes                 |
| `ExitWorktree`         | Exits a worktree session and returns to the original directory. Not available to subagents that already run in their own working directory, such as with [`isolation: worktree`](./code-sub-agents.md#supported-frontmatter-fields)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | No                  |
...
| `ScheduleWakeup`       | Reschedules the next iteration of a [self-paced `/loop`](./code-scheduled-tasks.md#let-claude-choose-the-interval). Claude calls this at the end of each iteration to pick when the next one runs, between one minute and one hour out; you don't call it directly. The pending wakeup appears in `session_crons` in [Stop hook input](./code-hooks.md#stop-input). {/* plan-availability: feature=loop-dynamic providers=anthropic */}Not available on Amazon Bedrock, Google Vertex AI, or Microsoft Foundry, where a `/loop` prompt with no interval runs on a fixed schedule instead                                                                                                                                                                                                                                                                                                 | No                  |
| `SendMessage`          | Sends a message to an [agent team](./code-agent-teams.md) teammate, or [resumes a subagent](./code-sub-agents.md#resume-subagents) by its agent ID or name. Stopped subagents auto-resume in the background. Structured team-protocol messages require agent teams. A receiver never treats a message from another agent as your consent or approval. {/* min-version: 2.1.198 */}As of v2.1.198, a subagent treats a message from the agent that launched it as normal task direction rather than as a peer request. {/* min-version: 2.1.199 */}As of v2.1.199, a send to a name that now resolves to a different agent than it did earlier in the conversation is refused instead of delivered; see [Resume subagents](./code-sub-agents.md#resume-subagents)                                                                                                                               | No                  |
| `SendUserFile`         | Sends files from the session to you with an optional caption, so a generated report, diagram, screenshot, or built artifact reaches your device instead of only being mentioned in the transcript. {/* min-version: 2.1.196 */}As of v2.1.196, the optional `display` input controls presentation: `render` opens the file inline in the client, `attach` shows a download card only, and when unset the client decides by file type. Available when a [Remote Control](./code-remote-control.md) client is connected or the session runs in a managed cloud environment such as [Claude Code on the web](./code-claude-code-on-the-web.md). Delivery runs through Anthropic-hosted infrastructure, so the tool is not available on Amazon Bedrock, Google Vertex AI, or Microsoft Foundry                                                                                               | No                  |
...
| `TaskStop`             | Stops a running background task by ID. {/* min-version: 2.1.198 */}As of v2.1.198, also accepts an [agent-team teammate](./code-agent-teams.md) or a named background agent by agent ID or name                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | No                  |
...
| `WaitForMcpServers`    | Waits for one or more [MCP servers](./code-mcp.md) that are still connecting in the background, so a request can use their tools without restarting the session. Claude calls it when a needed server isn't connected yet. Only appears when [tool search](./code-mcp.md#scale-with-mcp-tool-search) is disabled, since `ToolSearch` handles the wait when it's enabled                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | No                  |
...
| `Workflow`             | Runs a [dynamic workflow](./code-workflows.md): a script that orchestrates many subagents in the background and returns one consolidated result                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Yes                 |
...
* in [`permissions.allow` and `permissions.deny`](./code-settings.md#available-settings) in settings, and the `/permissions` interface
* in the `--allowedTools` and `--disallowedTools` [CLI flags](./code-cli-reference.md)
* in the Agent SDK's [`allowedTools` and `disallowedTools`](./code-agent-sdk/permissions.md#allow-and-deny-rules) options
* in a [subagent's `tools` or `disallowedTools`](./code-sub-agents.md#supported-frontmatter-fields) frontmatter
* in a [skill's `allowed-tools`](./code-skills.md#frontmatter-reference) frontmatter
* in a hook's [`if` condition](./code-hooks-guide.md#filter-by-tool-name-and-arguments-with-the-if-field)
...
| :----------------------------- | :------------------------ | :--------------------------------------------------------------- |
| `Bash(npm run *)`              | Bash, Monitor             | [Command pattern matching](./code-permissions.md#bash)                 |
| `PowerShell(Get-ChildItem *)`  | PowerShell                | [Command pattern matching](./code-permissions.md#powershell)           |
| `Read(~/secrets/**)`           | Read, Grep, Glob, LSP     | [Path pattern matching](./code-permissions.md#read-and-edit)           |
| `Edit(/src/**)`                | Edit, Write, NotebookEdit | [Path pattern matching](./code-permissions.md#read-and-edit)           |
| `Skill(deploy *)`              | Skill                     | [Skill name matching](./code-skills.md#restrict-claude’s-skill-access) |
| `Agent(Explore)`               | Agent                     | [Subagent type matching](./code-permissions.md#agent-subagents)        |
| `WebFetch(domain:example.com)` | WebFetch                  | [Domain matching](./code-permissions.md#webfetch)                      |
...
Hook `matcher` fields use bare tool names, not the parenthesized rule format. See [matcher patterns](./code-hooks.md#matcher-patterns) for the matching rules. For the field names each tool passes to `tool_input` in hooks, see the [PreToolUse input reference](./code-hooks.md#pretooluse-input).
...
To cap how many turns a subagent runs, set `maxTurns` in the [subagent definition](./code-sub-agents.md#supported-frontmatter-fields).

The same Agent tool also launches [forked subagents](./code-sub-agents.md#fork-the-current-conversation) when fork mode is enabled. A fork inherits the full parent conversation instead of starting fresh, always runs in the background, and still surfaces permission prompts in your terminal. The rest of this section describes named subagents.

Which tools a named subagent can use depends on the `tools` and `disallowedTools` fields in the [subagent definition](./code-sub-agents.md):
...
To limit what a subagent can reach in the first place, narrow its `tools` field, leave Bash off the list, or set deny rules in your settings, as described in [Control subagent capabilities](./code-sub-agents.md#control-subagent-capabilities). For more on choosing between foreground and background, see [Run subagents in foreground or background](./code-sub-agents.md#run-subagents-in-foreground-or-background).

## Bash tool behavior

The Bash tool runs each command in a separate process with the following persistence behavior:

* When Claude runs `cd` in the main session, the new working directory carries over to later Bash commands as long as it stays inside the project directory or an [additional working directory](./code-permissions.md#working-directories) you added with `--add-dir`, `/add-dir`, or `additionalDirectories` in settings. Subagent sessions never carry over working directory changes.
...
* **Timeout**: two minutes by default. Claude can request up to 10 minutes per command with the `timeout` parameter. Override the default and ceiling with [`BASH_DEFAULT_TIMEOUT_MS` and `BASH_MAX_TIMEOUT_MS`](./code-env-vars.md).
* **Output length**: 30,000 characters by default. When a command produces more than that, Claude Code saves the full output to a file in the session directory and gives Claude the file path plus a short preview from the start. Claude reads or searches that file when it needs the rest. Raise the limit with [`BASH_MAX_OUTPUT_LENGTH`](./code-env-vars.md), up to a hard ceiling of 150,000 characters.
...
This affects edit eligibility only, not permissions. [Read and Edit deny rules](./code-permissions.md#tool-specific-permission-rules) also apply to file commands Claude Code recognizes in Bash, such as `cat`, `head`, `tail`, `sed`, and `grep`, but not to arbitrary subprocesses that read or write files indirectly, like a Python or Node script that opens files itself. The set of commands recognized for deny rules is not the same as the read-before-edit list above: for example, `egrep` and `fgrep` count for read-before-edit but are not checked against Read deny rules. For OS-level enforcement that covers every process, [enable the sandbox](./code-sandboxing.md).
...
When Monitor runs a command, it uses the same [permission rules as Bash](./code-permissions.md#tool-specific-permission-rules), so `allow` and `deny` patterns you have set for Bash apply here too. The [WebSocket source](#websocket-source) has its own approval prompt.
...
Plugins can declare monitors that start automatically when the plugin is active, instead of asking Claude to start them. See [plugin monitors](./code-plugins-reference.md#monitors).
...
You configure [sandbox](./code-sandboxing.md) network rules separately, so a domain you want a sandboxed process to reach still needs an explicit sandbox permission rule.
...
The search backend is not configurable. To search with a different provider, add an [MCP server](./code-mcp.md) that exposes a search tool.

WebSearch permission rules take no specifier. A bare `WebSearch` entry in `allow` or `deny` is the only form.

<Note>
  WebSearch is available on the Claude API, [Claude Platform on AWS](./code-claude-platform-on-aws.md), and Microsoft Foundry. On Google Cloud Vertex AI it works with Claude 4 and later models, including Opus, Sonnet, and Haiku. Amazon Bedrock doesn't expose the server-side web search tool.
</Note>
...
<Note>
  The [advisor tool](./code-advisor.md) is a [server tool](https://platform.claude.com/docs/en/agents-and-tools/tool-use/advisor-tool) that the API runs, rather than a tool that Claude Code implements. It has no name you can reference in permission rules or hook matchers.
</Note>
