...
<Note>
  The [Security](./code-security.md) page covers the MCP threat model and how to evaluate a server before approving it. [Decide what to enforce](./code-admin-setup.md#decide-what-to-enforce) covers MCP restrictions alongside the other administrative controls.
</Note>
...
| **Plugin servers only** | Servers can only come from plugins; users can't add their own                              | [`strictPluginOnlyCustomization`](./code-settings.md#strictpluginonlycustomization) with `mcp` in the list |
...
<Note>
  Claude Code doesn't have a built-in MCP server registry that users can browse and install from. For the approved-catalog pattern, share the approved list and its `claude mcp add` commands somewhere your users will find them, such as an internal wiki, or distribute the servers as plugins through a [managed plugin marketplace](./code-plugin-marketplaces.md#managed-marketplace-restrictions) so users can browse and install them from `/plugin`.
</Note>
...
`managed-mcp.json` is a standalone file, so it cannot be delivered through [server-managed settings](./code-server-managed-settings.md). Any process that can write to a system path with administrator privileges can deploy it. At scale, that's usually through device management tooling, such as Jamf or a configuration profile on macOS, Group Policy or Intune on Windows, or your fleet management of choice on Linux. Claude Code looks for the file at one of these paths:
...
The file uses the same format as a project [`.mcp.json`](./code-mcp.md#project-scope) file:
...
* [`${VAR}` expansion](./code-mcp.md#environment-variable-expansion-in-mcp-json) to read secrets from each user's environment.
* [OAuth or per-user headers](./code-mcp.md#authenticate-with-remote-mcp-servers) so each user authenticates as themselves.
* [`headersHelper`](./code-mcp.md#use-dynamic-headers-for-custom-authentication) to generate credentials at connection time.
...
Deploying `managed-mcp.json` suppresses [claude.ai connectors](./code-mcp.md#use-mcp-servers-from-claude-ai) by default, including connectors an administrator configured for the organization in the claude.ai admin console. To load those connectors alongside the servers in `managed-mcp.json`, set `"allowAllClaudeAiMcps": true` in a [managed settings source](./code-admin-setup.md#decide-how-settings-reach-devices). Requires Claude Code v2.1.149 or later.
...
See [Invalid entries in managed settings](./code-settings.md#invalid-entries-in-managed-settings) for what happens when an entry fails schema validation.
...
To turn off all claude.ai connectors, see [`disableClaudeAiConnectors`](./code-mcp.md#disable-claude-ai-connectors).
...
When [OpenTelemetry export](./code-monitoring-usage.md) is configured, Claude Code can record which MCP servers and tools users invoke. Set `OTEL_LOG_TOOL_DETAILS=1` to include MCP server and tool names in tool events, then aggregate them in your collector to see which servers your users actually connect to. See [Monitoring](./code-monitoring-usage.md) to set up the exporter and for the full event schema.
...
| `allowedMcpServers`          | Allowlist of permitted servers                                                      | Any [settings file](./code-settings.md#settings-files); entries from every source merge unless `allowManagedMcpServersOnly` is set | For enforcement, a [managed settings source](./code-admin-setup.md#decide-how-settings-reach-devices): server-managed settings, `managed-settings.json`, MDM profile, or registry |
...
* [Decide what to enforce](./code-admin-setup.md#decide-what-to-enforce): MCP restrictions alongside permission rules, sandboxing, and the other admin controls
* [Connect Claude Code to tools via MCP](./code-mcp.md): the full MCP reference, including transports, scopes, and authentication
* [Settings](./code-settings.md): the settings hierarchy and how managed settings take precedence
* [Server-managed settings](./code-server-managed-settings.md): deliver `allowedMcpServers` and `deniedMcpServers` from the Claude.ai admin console
* [Security](./code-security.md): the threat model these controls defend against
