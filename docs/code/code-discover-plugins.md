...
`/plugin` opens an interactive panel in the terminal CLI. If Claude replies that `/plugin` isn't available in this environment, use the [plugin browser](./code-desktop.md#install-plugins) in the Claude desktop app, or declare the plugin under [`enabledPlugins`](./code-settings.md#enabledplugins) in `.claude/settings.json` for cloud sessions.
...
The `security-guidance` plugin reviews each change Claude makes for common vulnerabilities and instructs Claude to fix what it finds in the same session. See [Catch security issues as Claude writes code](./code-security-guidance.md) for what it checks and how to add project-specific rules.
...
To submit your own plugin to the community marketplace, see [Submit your plugin to the community marketplace](./code-plugins.md#submit-your-plugin-to-the-community-marketplace) in the create-plugins guide.
...
The command opens that plugin's details, where you choose an [installation scope](./code-settings.md#configuration-scopes). You see the same choices when you run `/plugin`, go to the **Discover** tab, and press **Enter** on a plugin:
...
To install without an interactive step, use the [`claude plugin install`](./code-plugins-reference.md#plugin-install) shell command, which installs to user scope unless you pass `--scope`.

You may also see plugins with **managed** scope. These are installed by administrators via [managed settings](./code-settings.md#settings-files) and can't be modified.
...
The **Not used recently** header and the **Last used** line are both hidden when your organization restricts marketplaces with [`strictKnownMarketplaces`](./code-settings.md#strictknownmarketplaces).

A plugin's [language server](./code-plugins.md#add-lsp-servers-to-your-plugin) counts as used when it delivers diagnostics or answers a code navigation request, so an LSP plugin whose server is active in your sessions isn't listed as unused. Before v2.1.203, language server activity couldn't be counted as use, so plugins that contribute an LSP server were exempt from the group entirely, the same way theme and output style plugins still are.
...
In these identifiers, `plugin-name` is the plugin's `name` in the [marketplace entry](./code-plugin-marketplaces.md#plugin-entries), which can differ from the `name` in the plugin's own `plugin.json`.
...
Reloading has a token cost on the next request: newly loaded components announce themselves in content appended to the conversation, while the existing history still reads from the prompt cache. A plugin that provides MCP servers costs more when its tools aren't deferred by [tool search](./code-mcp.md#scale-with-mcp-tool-search): the change invalidates the cache and the next request re-reads the entire conversation. {/* min-version: 2.1.163 */}In that case `/reload-plugins` shows a warning and does not apply the reload; pass `--force` to apply anyway. See [enabling or disabling a plugin](./code-prompt-caching.md#enabling-or-disabling-a-plugin) for details.
...
Administrators can also set `"autoUpdate": true` on each [`extraKnownMarketplaces`](./code-settings.md#extraknownmarketplaces) entry in managed settings to enable auto-update for an organization marketplace without requiring each user to toggle it.
