...
The `"Edit|Write"` matcher fires only when Claude uses the `Edit` or `Write` tool, not when it uses `Bash`, `Read`, or any other tool. See [Matcher patterns](./code-hooks.md#matcher-patterns) for how plain names and regular expressions are evaluated.
