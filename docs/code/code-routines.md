...
Create a routine from the web, the Desktop app, or the CLI. All three surfaces write to the same cloud account, so a routine you create in the CLI shows up at claude.ai/code/routines immediately. In the Desktop app, click **New task** and choose **New remote task**; choosing **New local task** instead creates a [local Desktop scheduled task](./code-desktop-scheduled-tasks.md), which runs on your machine and is not a routine.
...
<Steps>
  ...
  <Step title="Select an environment">
    ...
    * **Setup script**: install dependencies and tools the routine needs. The result is [cached](./code-claude-code-on-the-web.md#environment-caching), so the script doesn't re-run on every session
    ...
    A **Default** environment is provided. To use a custom environment, [create one](./code-claude-code-on-the-web.md#the-cloud-environment) before creating the routine.
  </Step>
  ...
  <Step title="Review connectors">
    All of your connected [MCP connectors](./code-mcp.md) are included by default. Remove any that the routine doesn't need. Connectors give Claude access to external services like Slack, Linear, or Google Drive during each run.
  </Step>
...
Open the **Schedule** page in the Desktop app, click **New task**, and choose **New remote task**. The Desktop app shows both local scheduled tasks and routines in the same grid. See [Desktop scheduled tasks](./code-desktop-scheduled-tasks.md) for details on the local option.
...
Each routine runs in a [cloud environment](./code-claude-code-on-the-web.md#the-cloud-environment) that controls network access, environment variables, and setup scripts. Configure environments before creating a routine to give Claude access to APIs, install dependencies, or restrict network scope. See [cloud environment](./code-claude-code-on-the-web.md#the-cloud-environment) for the full setup guide.
