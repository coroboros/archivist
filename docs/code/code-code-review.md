...
Lower [effort levels](./code-model-config.md#adjust-effort-level) return fewer, higher-confidence findings, while `high` through `max` give broader coverage and may include uncertain findings. Without an effort argument, the review uses the session's current effort. Pass a path or PR reference to review a specific target instead of the current diff.

`/code-review ultra --fix` runs the deeper [ultrareview](./code-ultrareview.md) in the cloud, then applies its findings to your working tree when they arrive back in your session.
