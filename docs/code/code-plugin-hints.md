...
This page is for CLI and SDK maintainers. If you are looking to install plugins, see [Discover and install plugins](./code-discover-plugins.md).

## How it works

Claude Code sets the [`CLAUDECODE`](./code-env-vars.md) environment variable to `1` for every command it runs through the Bash and PowerShell tools, and for [hook](./code-hooks.md) commands. {/* min-version: 2.1.172 */}From v2.1.172 it also sets [`CLAUDE_CODE_CHILD_SESSION`](./code-env-vars.md) to `1` in those same subprocesses. When your CLI sees one of these variables, it writes a self-closing `<claude-code-hint />` tag to stderr. In hook commands the hint tag is stripped and ignored. Only Bash and PowerShell tool output triggers the install prompt.
...
* {/* min-version: 2.1.172 */}`CLAUDE_CODE_CHILD_SESSION`: set only in subprocesses Claude Code itself spawns, such as tool calls, hook commands, and [status line](./code-statusline.md) commands, so the tag does not normally reach a human terminal. A long-lived process that was started inside a session, such as a tmux server, captures the variable, so shells later launched from that process still show the raw tag. Requires Claude Code v2.1.172 or later, so sessions on older versions miss the hint.
...
* **Telemetry opt-outs**: sessions where analytics are disabled never show hint prompts. This includes sessions with `DISABLE_TELEMETRY` or `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` set, and sessions on third-party providers such as Amazon Bedrock or Google Cloud's Agent Platform where the [automatic telemetry opt-out](./code-data-usage.md#default-behaviors-by-api-provider) applies.
...
The hint protocol only takes effect for plugins listed in the official Anthropic marketplace, `claude-plugins-official`. Anthropic curates that marketplace at its discretion, and the in-app submission forms add plugins to the [community marketplace](./code-plugins.md#submit-your-plugin-to-the-community-marketplace) instead, which the hint protocol does not check. If you are working with an Anthropic partner contact, reach out to them to coordinate an official-marketplace listing.

## See also

* [Create plugins](./code-plugins.md): build the plugin your CLI recommends
* [Create and distribute a plugin marketplace](./code-plugin-marketplaces.md): host plugins outside the official marketplace
* [Environment variables](./code-env-vars.md): full reference for `CLAUDECODE` and related variables
