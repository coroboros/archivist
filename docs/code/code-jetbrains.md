...
* **Selection context**: the current selection or tab in the IDE is automatically shared with Claude Code. [`Read` deny rules](./code-permissions.md#read-and-edit) block this sharing for matching files
...
<Steps>
  <Step title="Install the Claude Code CLI">
    Follow the [quickstart](./code-quickstart.md) to install the CLI if you haven't already. The plugin shows a "Cannot launch Claude Code" notification when `claude` isn't on your PATH.
  </Step>
...
Claude Code works with any paid Claude subscription (Pro, Max, Team, or Enterprise) or a Claude Console account, and no API key is required. You'll be prompted to [log in](./code-authentication.md#log-in-to-claude-code) the first time you run `claude`.
...
The **Diff tool** entry appears in `/config` only when Claude Code is connected to the IDE, so run `claude` from the JetBrains terminal or run [`/ide`](./code-commands.md) first from an external terminal. See [`diffTool`](./code-settings.md#global-config-settings) for the underlying setting.
...
When Claude Code runs in a JetBrains IDE in [`acceptEdits` permission mode](./code-permission-modes.md#auto-approve-file-edits-with-acceptedits-mode), it may be able to modify IDE configuration files that can be automatically executed by your IDE. This may increase the risk of running Claude Code in `acceptEdits` mode and allow bypassing Claude Code's permission prompts for bash execution.
...
For Claude Code installation or login problems outside the IDE, see [Troubleshoot installation and login](./code-troubleshoot-install.md).
...
The server is named `ide` and is hidden from `/mcp` because there's nothing to configure. If your organization uses a [`PreToolUse` hook](./code-hooks.md#pretooluse) to allowlist MCP tools, though, you'll need to know it exists.
