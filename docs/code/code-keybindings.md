...
| `app:toggleTodos`      | Ctrl+T    | Toggle visibility of Claude's to-do checklist. This is not the [`/tasks`](./code-commands.md) background-task view |
...
| `chat:clearInput`     | Ctrl+L                            | Force a full screen redraw, preserving input. In [fullscreen rendering](./code-fullscreen.md#clear-the-conversation), press twice within two seconds to run `/clear` |
| `chat:clearScreen`    | Cmd+K                             | In [fullscreen rendering](./code-fullscreen.md#clear-the-conversation), press twice within two seconds to run `/clear`                                               |
| `chat:killAgents`     | Ctrl+X Ctrl+K                     | Stop all running [background subagents](./code-sub-agents.md#run-subagents-in-foreground-or-background) in this session                                              |
...
| `confirm:toggleExplanation` | Ctrl+E    | Toggle a model-generated [explanation of the command](./code-permissions.md#permission-system) on Bash and PowerShell permission prompts |
...
`transcript:toggleShowAll` applies in the default renderer only; in [fullscreen rendering](./code-fullscreen.md), the transcript viewer doesn't offer a show-all toggle.
...
Actions available in the `Scroll` context when [fullscreen rendering](./code-fullscreen.md) is enabled:
...
* Vim keys aren't remappable through the keybindings file. To map a two-key INSERT-mode sequence such as `jj` to Escape, use the [`vimInsertModeRemaps`](./code-interactive-mode.md#remap-insert-mode-key-sequences) setting
...
Claude Code reports warnings when the file loads and writes each one to the debug log. Start Claude Code with [`--debug`](./code-cli-reference.md#cli-flags) to see the details.
