...
This guide is for plugin authors who declare dependencies in `plugin.json` and for marketplace maintainers who tag releases. To install plugins that have dependencies, see [Discover and install plugins](./code-discover-plugins.md). For the full manifest schema, see the [Plugins reference](./code-plugins-reference.md).
...
To roll bundles out across an organization, add the bundle plugin to `enabledPlugins` in [managed settings](./code-settings.md#enabledplugins).
...
This holds even when a dependency sets [`defaultEnabled: false`](./code-plugins-reference.md#default-enablement) in its manifest, because Claude Code writes an explicit `true` for it. The same applies at install: a dependency pulled in to satisfy an active plugin installs with `true` regardless of its own default.
...
* [Create plugins](./code-plugins.md): build plugins with skills, agents, and hooks
* [Create and distribute a plugin marketplace](./code-plugin-marketplaces.md): host plugins for your team
* [Plugins reference](./code-plugins-reference.md#plugin-manifest-schema): the full `plugin.json` schema
* [Version management](./code-plugins-reference.md#version-management): how a plugin's own version is resolved and used as the cache key
