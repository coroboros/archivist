...
Each refresh is one short Haiku-class request through your normal provider, billed and handled under the same [data usage terms](./code-data-usage.md) as the session itself. On third-party providers such as Bedrock, Vertex AI, Microsoft Foundry, and custom gateways, the request falls back to the session's main model when no Haiku model is configured. Set [`ANTHROPIC_DEFAULT_HAIKU_MODEL`](./code-model-config.md#environment-variables) to choose the model for these summaries on those providers.
