...
From v2.1.145, with [voice dictation](./code-voice-dictation.md) enabled, hold or tap your push-to-talk key while the reply input is focused to dictate a reply instead of typing it. The same works in the dispatch input at the bottom of agent view.
...
`--agent` sets the [subagent](./code-sub-agents.md) used when a dispatch prompt does not name one, either with `@name` or as the first word. It defaults to the [`agent` setting](./code-settings.md#available-settings) if one is set, otherwise the built-in catch-all `claude` agent. Naming a subagent in the dispatch input overrides both.
...
See the [error reference](./code-errors.md#could-not-resolve-authentication-method) for the full list of causes and fixes. Before v2.1.174, a pre-warmed worker that sat idle could surface this error when it was assigned to a dispatch even when your credentials were valid. Upgrade to recover.
