...
Each refresh is one short Haiku-class request through your normal provider, billed and handled under the same [data usage terms](./code-data-usage.md) as the session itself.
...
To make a subagent always run in its own worktree regardless of how it was started, set [`isolation: worktree`](./code-sub-agents.md#supported-frontmatter-fields) in its frontmatter.
