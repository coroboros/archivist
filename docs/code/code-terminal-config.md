...
This page is about getting your terminal to send the right signals to Claude Code. To change which keys Claude Code itself responds to, see [keybindings](./code-keybindings.md) instead.
...
In VS Code, Cursor, and Devin Desktop, `/terminal-setup` also updates two editor settings: it sets `terminal.integrated.gpuAcceleration` to `"off"` to prevent garbled text in the integrated terminal, and it sets `terminal.integrated.mouseWheelScrollSensitivity` for smoother scrolling in [fullscreen mode](./code-fullscreen.md). To undo the GPU acceleration change, set it back to `"auto"` and reload the editor window.
...
To bind newline to a different key, or to swap behavior so Enter inserts a newline and Shift+Enter submits, map the `chat:newline` and `chat:submit` actions in your [keybindings file](./code-keybindings.md).
...
By default Claude Code sends a desktop notification only in Ghostty, Kitty, and iTerm2. In other terminals, set [`preferredNotifChannel`](./code-settings.md#available-settings) to `"terminal_bell"` to ring the terminal bell instead, or configure a [Notification hook](#play-a-sound-with-a-notification-hook) for a custom sound or command. The following settings entry turns on the terminal bell:
...
When Claude Code runs inside tmux, two things break by default: Shift+Enter submits instead of inserting a newline, and desktop notifications and the [progress bar](./code-settings.md#available-settings) never reach the outer terminal. Add these lines to `~/.tmux.conf`, then run `tmux source-file ~/.tmux.conf` to apply them to the running server:
...
To customize what appears at the bottom of the interface, configure a [custom status line](./code-statusline.md) that shows the current model, working directory, git branch, or other context.
...
In addition to the built-in presets, `/theme` lists any custom themes you have defined and any themes contributed by installed [plugins](./code-plugins-reference.md#themes). Select **New custom theme…** at the end of the list to create one interactively: you name the theme, then pick individual color tokens to override. Press `Ctrl+E` while a custom theme is highlighted to edit it.
...
<Accordion title="Color token reference">
  ...
  Apply only in [fullscreen rendering mode](./code-fullscreen.md), where messages have a background fill.
  ...
  * `fastMode` and `fastModeShimmer`
  ...
  Each [subagent](./code-sub-agents.md) and parallel task is shown in one of eight named colors so you can tell them apart in the transcript. The token names follow the pattern `<color>_FOR_SUBAGENTS_ONLY`, where `<color>` is `red`, `blue`, `green`, `yellow`, `purple`, `orange`, `pink`, or `cyan`. Override these to change what each named color looks like. For example, a subagent with `color: blue` in its definition is drawn using the `blue_FOR_SUBAGENTS_ONLY` value.

  The [`ultrathink`](./code-model-config.md#use-ultrathink-for-one-off-deep-reasoning) and [`ultraplan`](./code-ultraplan.md) keywords in the prompt input are rendered with a seven-color rainbow gradient. The token names follow the pattern `rainbow_<color>` and `rainbow_<color>_shimmer`, where `<color>` is `red`, `orange`, `yellow`, `green`, `blue`, `indigo`, or `violet`.
</Accordion>

## Switch to fullscreen rendering

If the display flickers or the scroll position jumps while Claude is working, switch to [fullscreen rendering mode](./code-fullscreen.md). It draws to a separate screen the terminal reserves for full-screen apps instead of appending to your normal scrollback, which keeps memory usage flat and adds mouse support for scrolling and selection. In this mode you scroll with the mouse or PageUp inside Claude Code rather than with your terminal's native scrollback; see the [fullscreen page](./code-fullscreen.md#search-and-review-the-conversation) for how to search and copy.

If flicker is the only problem and your terminal supports synchronized output but isn't auto-detected, such as Emacs `eat`, set [`CLAUDE_CODE_FORCE_SYNC_OUTPUT=1`](./code-env-vars.md) to stop the flicker without changing renderers.
...
Claude Code includes a Vim-style editing mode for the prompt input. Enable it through `/config` → Editor mode, or by setting [`editorMode`](./code-settings.md#available-settings) to `"vim"` in `~/.claude/settings.json`. Set Editor mode back to `normal` to turn it off.

Vim mode supports a subset of NORMAL- and VISUAL-mode motions and operators, such as `hjkl` navigation, `v`/`V` selection, and `d`/`c`/`y` with text objects. See the [Vim editor mode reference](./code-interactive-mode.md#vim-editor-mode) for the full key table.

Vim motions aren't remappable through the keybindings file. To map a two-key INSERT-mode sequence such as `jj` to Escape, set [`vimInsertModeRemaps`](./code-interactive-mode.md#remap-insert-mode-key-sequences) in your user settings.
...
* [Interactive mode](./code-interactive-mode.md): full keyboard shortcut reference and the Vim key table
* [Keybindings](./code-keybindings.md): remap any Claude Code shortcut, including Enter and Shift+Enter
* [Fullscreen rendering](./code-fullscreen.md): details on scrolling, search, and copy in fullscreen mode
* [Hooks guide](./code-hooks-guide.md): more Notification hook examples for Linux and Windows
* [Troubleshooting](./code-troubleshooting.md): fixes for issues outside terminal configuration
