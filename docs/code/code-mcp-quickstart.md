...
For every way to connect and configure MCP servers in Claude Code, see the [MCP reference](./code-mcp.md).

## Before you begin

Make sure you have:

* [Claude Code installed](./code-quickstart.md) and authenticated
...
The steps are the same for any server: add it, check the connection status, then use it in a session, with an optional cleanup step at the end. Some servers add a step, like a browser sign-in, shown in [Additional MCP server examples](#additional-mcp-server-examples). For more servers to connect, browse the [Anthropic Directory](./code-mcp.md#find-and-build-mcp-servers).
...
<Steps>
  ...
  <Step title="Remove the server">
    ...
    <Note>
      Each connected server takes some space in [Claude's context window](./code-how-claude-code-works.md#the-context-window) because its tool names and server instructions load into every session. Removing servers you no longer use keeps that space free.
    </Note>
  </Step>
</Steps>
...
On Windows, `~/.claude.json` resolves to `%USERPROFILE%\.claude.json`, typically `C:\Users\YourName\.claude.json`. If you've set [`CLAUDE_CONFIG_DIR`](./code-env-vars.md), Claude Code reads `.claude.json` from inside that directory instead.

Run `claude mcp get claude-code-docs` to see which scope holds a server's definition. For how the scopes interact when the same server is defined in more than one, see [MCP installation scopes](./code-mcp.md#mcp-installation-scopes).
...
The steps below use Sentry as the example. To connect a different service, substitute its URL, which you can find in the [Anthropic Directory](./code-mcp.md#find-and-build-mcp-servers) or the service's documentation.
...
Servers that authenticate with a static token instead of OAuth take the token at add time with `--header "Authorization: Bearer <token>"`. See the [GitHub example](./code-mcp.md#example-connect-to-github-for-code-reviews) for a worked version.
...
* **Claude Code desktop app**: add servers through the [Connectors UI](./code-desktop.md#connect-external-tools).
...
* **VS Code**: see [Connect to external tools with MCP](./code-vs-code.md#connect-to-external-tools-with-mcp).
* **Claude Code on the web**: reads `.mcp.json` from your repository. See [Edit .mcp.json directly](#edit-mcp-json-directly).
* **Claude.ai**: connectors you add at [claude.ai/customize/connectors](https://claude.ai/customize/connectors) load automatically in the CLI when you sign in with that account. See [Use MCP servers from Claude.ai](./code-mcp.md#use-mcp-servers-from-claude-ai).
...
<AccordionGroup>
  ...
  <Accordion title="Connection timed out at startup">
    The server took longer than the default 30-second startup timeout. A stdio server's first run can be slow while `npx` downloads the package. Increase the limit with the [`MCP_TIMEOUT`](./code-env-vars.md) environment variable, in milliseconds:
  ...
  <Accordion title="OAuth sign-in fails or browser doesn't open">
    Run `/mcp`, select the server, and choose `Authenticate` again. If the browser doesn't open automatically, copy the URL shown in the terminal and open it manually. See [Authenticate with remote MCP servers](./code-mcp.md#authenticate-with-remote-mcp-servers) for fixed callback ports and pre-configured credentials.
  </Accordion>
</AccordionGroup>
...
* [Find more MCP servers](./code-mcp.md#find-and-build-mcp-servers) in the Anthropic Directory
* [Share servers with your team](./code-mcp.md#mcp-installation-scopes) using installation scopes
* [Manage MCP access for an organization](./code-managed-mcp.md) with managed settings and policy controls
* [Reference MCP resources](./code-mcp.md#use-mcp-resources) in prompts with @ mentions
* [Run MCP prompts as commands](./code-mcp.md#use-mcp-prompts-as-commands) from the `/` menu
