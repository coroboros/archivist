...
These five types cover the full agent loop lifecycle in both SDKs. The TypeScript SDK also yields additional observability events (hook events, tool progress, rate limits, task notifications) that provide extra detail but are not required to drive the loop. See the [Python message types reference](./code-agent-sdk/python.md#message-types) and [TypeScript message types reference](./code-agent-sdk/typescript.md#message-types) for the complete lists.
...
See [Session management](./code-agent-sdk/sessions.md) for the full guide on resume, continue, and fork patterns.
