...
| ------------ | ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scope.type` | `user`, `rbac_group`, `organization`        | `user` targets one developer by their OpenID Connect (OIDC) `sub`, the stable user ID your identity provider assigns; pass it as `scope.user_id`. `rbac_group` targets an [IdP group](./code-claude-apps-gateway-config.md#managed) by name; pass it as `scope.rbac_group_id`. `organization` is the org-wide default. The gateway accepts all three; Anthropic's public `POST` is user-only today. |
...
A group or organization cap is a per-seat default that each member inherits, not a shared pool. Per period, a developer's effective cap resolves in this order: a per-user override, then the most restrictive of their group caps, then the org default, then unlimited. [`admin.group_limit_mode: max`](./code-claude-apps-gateway-config.md#admin) flips the multi-group tie-break to least-restrictive instead.

### Authenticate to the admin API

Send one of:

* An `x-api-key` header matching a key in [`admin.write_keys`](./code-claude-apps-gateway-config.md#admin) for full access, or `admin.read_keys` for `GET`-only access. Each key carries an `id` that appears in the audit log as `admin-key:<id>`, so give Terraform, CI, and each automation its own.
* A gateway bearer token whose `groups` claim includes one of [`admin.admin_groups`](./code-claude-apps-gateway-config.md#admin). This is full access and audits as `oidc:<sub>`, so prefer it for human admins.

## How enforcement works

On each `/v1/messages` request, the gateway resolves the developer's caps and period-to-date spend in one Postgres query. If they're over any cap, the request returns `429` with `error.type: billing_error` and the header `x-should-retry: false`. The message is `spend limit reached`, followed by your [`admin.blocked_message`](./code-claude-apps-gateway-config.md#admin) if set.
...
The pre-check queries Postgres with a two-second timeout. If the store is unreachable or times out, enforcement fails open by default: the request proceeds and the gateway logs a warning. Set [`enforcement.fail_closed_on_error: true`](./code-claude-apps-gateway-config.md#enforcement) to fail closed instead, which returns the same `429 billing_error` with the message `spend limit unavailable`. Fail-open keeps a store outage from becoming an inference outage; fail-closed guarantees no unmetered spend.
...
| ------------------ | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `spend`            | Per-principal period-to-date counters in cents                                | [`admin.spend_retention_months`](./code-claude-apps-gateway-config.md#admin), default 13                      |
| `spend_limits`     | The configured caps                                                           | Until deleted via the API                                                                               |
| `admin_audit`      | The mutation trail                                                            | [`admin.audit_retention_days`](./code-claude-apps-gateway-config.md#admin), default 365                       |
| `principal_emails` | Each principal's last-seen email, display name, and IdP groups. Contains PII. | [`admin.identity_retention_days`](./code-claude-apps-gateway-config.md#admin) since last activity, default 90 |
...
* [`admin` and `enforcement` configuration](./code-claude-apps-gateway-config.md#admin): enabling the admin API and tuning retention
* [Deployment guide](./code-claude-apps-gateway-deploy.md#postgres): Postgres schema and backup guidance
