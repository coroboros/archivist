...
On Amazon Bedrock, Google Cloud's Agent Platform, and Microsoft Foundry, Claude Code doesn't send metrics from your cloud. A self-hosted [Claude apps gateway](./code-claude-apps-gateway.md) provides per-user usage attribution, OTLP metrics with token counts, and [per-user spend limits](./code-claude-apps-gateway-spend-limits.md) on these providers. Organizations that route Claude Code through a different [LLM gateway](./code-llm-gateway.md) can track spend at the gateway instead, since it sees every request.
