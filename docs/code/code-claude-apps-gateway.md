...
<Steps>
  ...
  <Step title="Run it">
    Build a container image around the `claude` binary that meets the [image requirements](./code-claude-apps-gateway-deploy.md#container-image), then run it alongside Postgres:
...
| Auto mode                                                                                                                  | Available with opt-in | Follows the [third-party provider rules](./code-permission-modes.md#enable-auto-mode-on-bedrock-agent-platform-or-foundry): set `CLAUDE_CODE_ENABLE_AUTO_MODE=1`, deliverable through the managed policy `env` block, and only the models eligible on third-party providers can use it                                                                                                                                                 |
