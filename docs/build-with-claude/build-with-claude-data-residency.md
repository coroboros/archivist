...
These settings can be configured through the Console or the [Admin API](../manage-claude/manage-claude-admin-api.md) under the `data_residency` field.
...
* **Older models:** Don't support `inference_geo` (see [Model availability](../manage-claude/manage-claude-data-residency.md#model-availability)); standard pricing applies. Requests that include the parameter return a 400 error.
...
The same multiplier applies to [Claude Managed Agents](../managed-agents/managed-agents-overview.md): when an agent's [model configuration](../managed-agents/managed-agents-agent-setup.md) pins `inference_geo` to `"us"`, model requests in sessions running that agent are priced at 1.1x the standard rate.

<Note>
  If you have a [Priority Tier](../api/api-service-tiers.md) commitment, the 1.1x multiplier for US-only inference also affects how tokens are counted against your Priority Tier capacity. Each token consumed with `inference_geo: "us"` draws down 1.1 tokens from your committed TPM, consistent with how other pricing multipliers (such as prompt caching) affect burndown rates.
</Note>
...
Legacy models are unaffected by this migration. For current pricing on newer models, see [Pricing](../manage-claude/manage-claude-data-residency.md#pricing).
...
<CardGroup>
  ...
  <Card title="Workspaces" icon="building" href="../manage-claude/manage-claude-workspaces.md">
    Learn about workspace configuration.
  </Card>

  <Card title="Usage and Cost API" icon="chart" href="../manage-claude/manage-claude-usage-cost-api.md">
