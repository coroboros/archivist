...
* `"omitted"`: thinking blocks are returned with an empty `thinking` field. The `signature` field still carries the encrypted full thinking for multi-turn continuity (see [Thinking encryption](./build-with-claude-thinking.md#thinking-encryption)). This is the default on Claude Fable 5.1, Claude Mythos 5.1, Claude Fable 5, Claude Mythos 5, Claude Opus 5, Claude Sonnet 5, Claude Opus 4.8, Claude Opus 4.7, and [Claude Mythos Preview](https://anthropic.com/glasswing).
...
* When streaming with `display: "omitted"`, no `thinking_delta` events are emitted. With `display: "updates"`, only [progress-update blocks](./build-with-claude-thinking.md#progress-updates) stream `thinking_delta` events. See [Streaming thinking](./build-with-claude-thinking.md#streaming-thinking) for the event sequence.
...
* These models write fewer progress updates at higher [effort](./build-with-claude-effort.md) and in long tool chains. If your interface depends on them, see [Ask for user-facing progress updates](./build-with-claude-prompt-engineering-prompting-claude-fable-5-1.md#ask-for-user-facing-progress-updates).
...
* **For the model that produced it, or a newer one.** An earlier model can't use the block, and the API drops it from that request. See [Only for the model that produced it, or a newer one](./build-with-claude-thinking.md#preserved-for-model).
* **In the conversation that produced it (Claude Fable 5.1 only).** If the `system` prompt, the `tools`, or any earlier message changes, the block is no longer valid, and the API rejects the request or drops the block. See [Only in the conversation that produced it](./build-with-claude-thinking.md#preserved-in-conversation).
...
**A block the receiving model can't read is dropped.** The API removes it before the prompt reaches the model. It doesn't count toward `input_tokens` and isn't billed. When you fall back from Claude Fable 5.1 to an older model mid-conversation, for example after a [classifier refusal fallback](./build-with-claude-refusals-and-fallback.md), the older model reasons again from the visible conversation. With the [controls beta header](./build-with-claude-thinking.md#preserved-thinking-controls) the drop is reported in `input_transformations` as `model_binding_mismatch`. Without it the drop is silent. A [server-side fallback](./build-with-claude-refusals-and-fallback.md#server-side-fallback) drops unreadable blocks the same way.
