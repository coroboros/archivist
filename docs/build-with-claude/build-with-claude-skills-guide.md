...
<Note>
  ...
  * [Skill Management API Reference](../api/api-skills-list.md) - CRUD operations for Skills
  * [Skill Versions API Reference](../api/api-skills-versions-list.md) - Version management
</Note>
...
<CardGroup cols={2}>
  <Card title="Get started with Agent Skills in the API" icon="rocket" href="../agents-and-tools/agents-and-tools-agent-skills-quickstart.md">
    Learn how to use Agent Skills to create documents with the Claude API in under 10 minutes.
  </Card>

  <Card title="Skill authoring best practices" icon="hammer" href="../agents-and-tools/agents-and-tools-agent-skills-best-practices.md">
...
| **Management**     | Pre-built and maintained by Anthropic      | Upload and manage through the [Skills API](../api/api-skills-create.md) |
| **Availability**   | Available to all users                     | Private to your workspace                                                                         |

Both skill sources are returned by the [List Skills endpoint](../api/api-skills-list.md) (use the `source` parameter to filter). The integration shape and execution environment are identical. The only difference is where the Skills come from and how they're managed.
...
2. **[Code execution tool](../agents-and-tools/agents-and-tools-tool-use-code-execution-tool.md)** enabled in your requests

Skills require the code execution tool, so use a model from its [model compatibility list](../agents-and-tools/agents-and-tools-tool-use-code-execution-tool.md#model-compatibility).
...
2. The response includes a `file_id` for each created file, inside code-execution tool result blocks (see [Response format](../agents-and-tools/agents-and-tools-tool-use-code-execution-tool.md#response-format)).
3. Use the Files API to download the actual file content.
4. Save locally or process as needed.

To provide input files for Skills to work on, [upload them with the Files API](./build-with-claude-files.md#uploading-a-file) and reference them in your request with a [container upload block](./build-with-claude-files.md#container-upload-blocks).
...
The response's `container` object carries the container's `id` and `expires_at` timestamp (see [Container reuse](../agents-and-tools/agents-and-tools-tool-use-code-execution-tool.md#container-reuse) for lifetime details). Reuse the same container across multiple messages by specifying the container ID:
...
For complete request/response schemas, see the [Create Skill API reference](../api/api-skills-create.md).
...
See the [List Skills API reference](../api/api-skills-list.md) for pagination and filtering options.
...
See the [Create Skill Version API reference](../api/api-skills-versions-create.md) for complete details.
...
See [Code execution tool](../agents-and-tools/agents-and-tools-tool-use-code-execution-tool.md) for available packages.
...
**For production:** pin a specific version, so Skill updates never change your deployed behavior. If you omit `version` or set it to `"latest"`, requests use the newest version of the Skill, so a version uploaded by anyone in the [workspace](./build-with-claude-skills-guide.md#workspace-scoped-access) immediately changes what your production agents run. The version ID comes from the create-version response in [Versioning](./build-with-claude-skills-guide.md#versioning) or from the [List Skill Versions API](../api/api-skills-versions-list.md). The ID is always a string, so quote it in JSON or YAML even when it looks numeric.
...
If your organization has the [Compliance API](../manage-claude/manage-claude-compliance-api.md) enabled, its [Activity Feed](../manage-claude/manage-claude-compliance-activity-feed.md) records the creation and deletion of Skills and Skill versions made with a Claude API key or from the Claude Console. Operations that occur while the Compliance API is off are not recorded and cannot be recovered later, so [set up the Compliance API](../manage-claude/manage-claude-compliance-api-access.md) before you rely on this audit trail.

## Next steps

<CardGroup cols={3}>
  <Card title="API reference" icon="book" href="../api/api-skills-create.md">
    Complete API reference with all endpoints
  </Card>

  <Card title="Skill authoring best practices" icon="edit" href="../agents-and-tools/agents-and-tools-agent-skills-best-practices.md">
    Learn how to write effective Skills that Claude can discover and use successfully.
  </Card>

  <Card title="Code execution tool" icon="terminal" href="../agents-and-tools/agents-and-tools-tool-use-code-execution-tool.md">
