...
Note that `input_type="document"` and `input_type="query"` are used for embedding the document and query, respectively. More specification can be found in the [Voyage Python package section](./build-with-claude-embeddings.md#voyage-python-package).
