...
For broader prompting guidance with thinking, see [leverage thinking and interleaved thinking capabilities](./build-with-claude-prompt-engineering-claude-prompting-best-practices.md#leverage-thinking-and-interleaved-thinking-capabilities).
...
`output_tokens` remains the inclusive, authoritative total used for billing. `output_tokens_details` is a read-only breakdown for observability. For complete pricing information including base rates, cache writes, cache hits, and output tokens, see [Pricing](../about-claude/about-claude-pricing.md).
