...
## Compatibility
- Status: Beta
- [Beta header](../api/api-beta-headers.md): `cache-diagnosis-2026-04-07`
...
The comparison is about request structure, independent of whether the cache actually hit. See [Reading diagnostics alongside usage](./build-with-claude-cache-diagnostics.md#reading-diagnostics-alongside-usage) for how to combine the `diagnostics` result with `usage.cache_read_input_tokens`.
...
The `message_start` event carries the full `diagnostics` field; see [Response format](./build-with-claude-cache-diagnostics.md#response-format) for the possible values.
...
<Tabs>
  <Tab title="cURL">
    <Info>
      This workflow doesn't translate well to a one-off shell command. See the SDK tabs for the loop pattern; the per-turn HTTP request is identical to [Basic usage](./build-with-claude-cache-diagnostics.md#basic-usage).
    </Info>
  </Tab>

  <Tab title="CLI">
    <Info>
      This workflow doesn't translate well to a one-off shell command. See the SDK tabs for the loop pattern; the per-turn CLI invocation is identical to [Basic usage](./build-with-claude-cache-diagnostics.md#basic-usage).
    </Info>
  </Tab>
...
* **Same workspace:** The previous request must have been made with an API key from the same organization and workspace. To check, compare the `anthropic-workspace-id` [response header](../api/api-overview.md#response-headers) on the two responses.
...
* [Beta headers](../api/api-beta-headers.md)
