...
### Memory tools
When memory stores are attached to a session, the agent automatically gains access to memory tools. The agent's interactions with memory stores are registered as `agent.tool_use` events in the [event stream](./developer-managed-agents-events-and-streaming.md).
