...
<Note>
The CLI does not include a `tool_runner` helper. Use [server-side compaction](./developer-build-with-claude-compaction.md) instead, which handles compaction on Anthropic's servers without SDK-side integration.
</Note>
