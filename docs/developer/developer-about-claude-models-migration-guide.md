...
<Note>
This guide covers migrating [Messages API](./developer-build-with-claude-working-with-messages.md) code. If you use [Claude Managed Agents](./developer-managed-agents-overview.md), no changes beyond updating model name are required.
</Note>
...
- [1M token context window](./developer-build-with-claude-context-windows.md) at standard API pricing with no long-context premium
- [128k max output tokens](./developer-about-claude-models-overview.md)
...
- [Files API](./developer-build-with-claude-files.md)
- [PDF support](./developer-build-with-claude-pdf-support.md)
- [Vision](./developer-build-with-claude-vision.md)
- The full set of server-side and client-side [tools](./developer-agents-and-tools-tool-use-overview.md) ([bash](./developer-agents-and-tools-tool-use-bash-tool.md), [code execution](./developer-agents-and-tools-tool-use-code-execution-tool.md), [computer use](./developer-agents-and-tools-tool-use-computer-use-tool.md), [text editor](./developer-agents-and-tools-tool-use-text-editor-tool.md), [web search](./developer-agents-and-tools-tool-use-web-search-tool.md), [web fetch](./developer-agents-and-tools-tool-use-web-fetch-tool.md), [MCP connector](./developer-agents-and-tools-mcp-connector.md), [memory](./developer-agents-and-tools-tool-use-memory-tool.md))

<Tip>
  **Automate this migration with the Claude API skill.** In Claude Code, run `/claude-api migrate` to invoke the bundled [Claude API skill](./developer-agents-and-tools-agent-skills-claude-api-skill.md#migrating-to-a-newer-claude-model):
...
1. **Extended thinking removed:** `thinking: {type: "enabled", budget_tokens: N}` is no longer supported on Claude Opus 4.7 or later models and returns a 400 error. Switch to [adaptive thinking](./developer-build-with-claude-adaptive-thinking.md) (`thinking: {type: "adaptive"}`) and use the [effort parameter](./developer-build-with-claude-effort.md) to control thinking depth. Adaptive thinking is **off by default** on Claude Opus 4.7: requests with no `thinking` field run without thinking, matching Opus 4.6 behavior. Set `thinking: {type: "adaptive"}` explicitly to enable it.
    ...
    Adaptive thinking is steerable through prompting. For guidance on tuning when the model over- or under-thinks, see [Calibrating effort and thinking depth](./developer-build-with-claude-prompt-engineering-claude-prompting-best-practices.md#calibrating-effort-and-thinking-depth).
...
4. **Updated token counting:** Claude Opus 4.7 uses a new tokenizer, contributing to its improved performance on a wide range of tasks. The new tokenizer may use roughly 1x to 1.35x as many tokens when processing text compared to previous models (up to ~35% more, varying by content).
    ...
    [`/v1/messages/count_tokens`](./developer-build-with-claude-token-counting.md) will return a different number of tokens for Claude Opus 4.7 than it did for Claude Opus 4.6. Token efficiency can vary by workload shape.
...
6. **Stricter effort calibration:** Meaningfully changing from Claude Opus 4.6, Claude Opus 4.7 respects [effort levels](./developer-build-with-claude-effort.md) strictly, especially at the low end. At `low` and `medium`, the model scopes its work to what was asked rather than going above and beyond.
    ...
    If you need to keep effort at `low` for latency, add targeted guidance: "This task involves multi-step reasoning. Think carefully through the problem before responding." See [Recommended effort levels for Claude Opus 4.7](./developer-build-with-claude-effort.md#recommended-effort-levels-for-claude-opus-4-7).
...
- [ ] Replace `thinking: {type: "enabled", budget_tokens: N}` with `thinking: {type: "adaptive"}` plus the [effort parameter](./developer-build-with-claude-effort.md).
...
- [ ] If your application sends images, re-budget for [high-resolution image support](./developer-build-with-claude-vision.md#high-resolution-image-support-on-claude-opus-4-7) (up to approximately 3x more image tokens per full-resolution image). Downsample before sending if you do not need the additional fidelity.
...
- [ ] **BREAKING on Opus 4.7:** Replace `thinking: {type: "enabled", budget_tokens: N}` with `thinking: {type: "adaptive"}` plus the [effort parameter](./developer-build-with-claude-effort.md) (returns 400 on Opus 4.7)
