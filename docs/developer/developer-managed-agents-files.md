...
First, upload a file using the [Files API](./developer-build-with-claude-files.md):
...
A new `file_id` will be created that references the instance of the file in the session. These copies do not count against your [storage limits](./developer-build-with-claude-files.md).
...
Use the [Files API](./developer-build-with-claude-files.md) to list files scoped to a session and download them.
