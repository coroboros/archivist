...
| `pause_turn`                                                     | A dangling advisor call ends the response with `stop_reason: "pause_turn"` and the `server_tool_use` block as the last content block. The advisor executes on resumption. See [Server tools](./developer-agents-and-tools-tool-use-server-tools.md#the-server-side-loop-and-pause-turn). |
