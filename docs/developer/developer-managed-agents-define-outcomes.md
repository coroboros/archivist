...
Progress on an outcome-oriented session is surfaced on the events [stream](./developer-managed-agents-events-and-streaming.md).
...
- You can also send `user.message` [events](./developer-managed-agents-events-and-streaming.md#user-events) to an outcome-oriented session, to direct the agent's work as it progresses, but these are not as necessary; the agent knows to work until it has exhausted its iterations or achieved the outcome.
...
You can either listen on the [event stream](./developer-managed-agents-events-and-streaming.md) for `span.outcome_evaluation_end`, or poll `GET /v1/sessions/:id` and read `outcome_evaluations[].result`:
...
The agent writes output files to `/mnt/session/outputs/` inside the container. Once the session is idle, fetch them via the [Files API](./developer-build-with-claude-files.md) scoped to the session:
